...
})) %>% dplyr::select(patient, Samples, everything())

max.bin<-function(bin15,bin50,bin100) {
  if (!is.na(bin100))
    return(100)
  if (!is.na(bin50))
    return(50)
...
per.sample.bin = per.sample.var %>% group_by(Samples) %>% dplyr::summarise( max.bin = max.bin(`15kb`,`50kb`,`100kb`) ) %>% arrange(max.bin)

per.sample.bin %>% group_by(max.bin) %>% tally() %>% spread(max.bin,n) %>% kable(caption = 'Number of samples per QDNAseq bin size') %>% kable_styling('striped', full_width = F)
```

```{r}

per.sample.bin = val.info %>% dplyr::select(matches('Hospital|Status|Block|Pathology|Samplename')) %>% 
  left_join(per.sample.bin, by=c('Samplename'='Samples'))

per.sample.bin %>% group_by(max.bin, Status) %>% tally() 

per.sample.bin %>% filter(is.na(max.bin))
