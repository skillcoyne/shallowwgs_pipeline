...
crossvalidate.by.patient<-function(x,y,lambda,pts,a=1,nfolds=10, splits=5, fit=NULL, minR=0.2, select='performance') {
  ...
  for (n in 1:nfolds) {  
    ...
    for (i in 1:splits) { 
      ...
      predmat = predict(fitCV, test, type='response')
      prob_min = 1e-05; prob_max = 1 - prob_min

      predmat = pmin(pmax(predmat, prob_min), prob_max)

      yT = as.factor(y[test.rows])
      dev = apply(predmat, 2, function(p)  -2*((yT==1)*log(p)+(yT==0)*log(1-p)) )
...
plot.patient.cv<-function(df, fit=NULL) {
  ...
  if (nrow(subset(df, lambda.1se == T)) > 0) {
    gp = gp + geom_point(data=subset(df, lambda.1se == T), aes(y=mean, x=log.lambda, colour="lambda.min+1se"), size=2 ) +
    ...
    gpD = gpD + geom_point(data=subset(df, lambda.1se == T), aes(y=mean.b.dev, x=log.lambda, colour="lambda.min+1se"), size=2 ) +
      geom_vline(xintercept = subset(df, lambda.1se == T)$log.lambda, colour='grey') +
      annotate("text", x=subset(df, lambda.1se == T)$log.lambda, y=min(df$mean)+sd(df$mean), 
               label=round(df[subset(df, lambda.1se == T)$lambda.at,'mean'],3)) 
  }
  ...
  if (!is.null(fit)) {
    ...
    coef.m.1se = subset(df, `lambda-1se` == T)$nzcoef
    
    d = coef.1se-coef.min
    
    coef.text = arrange(subset(df,nzcoef %in% c(0,  coef.min, coef.1se, coef.m.1se, max(nzcoef) )), -lambda.min, -lambda.1se)
...
performance.plots = list(); coefs = list(); deviance.plots = list()
plots = list()
folds = 10; splits=5
...
if (file.exists('/tmp/all.pt.alpha.Rdata')) {
  ...
  for (a in alpha.values) {
    ...
    cv.patient = crossvalidate.by.patient(x=dysplasia.df, y=labels, lambda=fit0$lambda, pts=pts, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance')
    ...
    #performance.plots[[as.character(a)]] = cv.patient$plot + ggtitle(paste('alpha=',a,sep=''))
    #deviance.plots[[as.character(a)]] = cv.patient$deviance.plot + ggtitle(paste('alpha=',a,sep=''))
    
    plots[[as.character(a)]] = arrangeGrob(cv.patient$plot+ggtitle('CV Performance'), cv.patient$deviance.plot+ggtitle('Binomial Deviance'), top=paste('alpha=',a,sep=''), ncol=2)
  }
  save(plots, coefs, file='/tmp/all.pt.alpha.Rdata')
}
...
if (file.exists('/tmp/nohgd.Rdata')) {
  ...
  for (a in alpha.values) {
    ...
    cv.nohgd = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=lambda1, pts=subset(pts, Samplename %in% samples), a=a, nfolds=folds, splits=splits, fit=fit1, select='deviance')
    
    #plots[[as.character(a)]] = cv.nohgd$plot + ggtitle(paste('alpha=',a,sep=''))
    
    no.hgd.plots[[as.character(a)]] = arrangeGrob(cv.patient$plot+ggtitle('CV Performance'), cv.patient$deviance.plot+ggtitle('Binomial Deviance'), top=paste('alpha=',a,sep=''), ncol=2)
  ...
  save(no.hgd.plots, coefs, file='/tmp/nohgd.Rdata')
}
...
if (file.exists('/tmp/nolgd.Rdata')) {
  ...
  for (a in alpha.values) {
    ...
    cv.nolgd = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=lambda2, pts=subset(pts, Samplename %in% samples), a=a, nfolds=folds, splits=splits, fit=fit2, select='deviance')
    
    #plots[[as.character(a)]] = cv.nolgd$plot + ggtitle(paste('alpha=',a,sep=''))
    
    nolgd.plots[[as.character(a)]] = arrangeGrob(cv.nolgd$plot+ggtitle('CV Performance'), cv.nolgd$deviance.plot+ggtitle('Binomial Deviance'), top=paste('alpha=',a,sep=''), ncol=2)
  ...
  save(nolgd.plots, coefs, file='/tmp/nolgd.Rdata')
}
...
few.plots = list()
for (a in alpha.values) {
  fit0 <- glmnet(dysplasia.df[samples,], labels[samples], alpha=a, nlambda=500, family='binomial') # all patients
  cv.back.few = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fit0$lambda, pts=subset(pts, Samplename %in% samples), splits=5, nfolds=20, a=a, fit=fit0, minR=0.1, select='deviance')
  #few.plots[[as.character(a)]] = cv.back.few$plot + ggtitle(paste('alpha=',a,sep=''))
  
  few.plots[[as.character(a)]] = arrangeGrob(cv.back.few$plot+ggtitle('CV Performance'), cv.back.few$deviance.plot+ggtitle('Binomial Deviance'), top=paste('alpha=',a,sep=''), ncol=2)
}
...
for (a in alpha.values) {
  ...
  cv.back.many = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fit0$lambda, pts=subset(pts, Samplename %in% samples), splits=5, nfolds=20, a=a, fit=fit0, select='deviance')
  #many.plots[[as.character(a)]] = cv.back.many$plot + ggtitle(paste('alpha=',a,sep=''))
  
  many.plots[[as.character(a)]] = arrangeGrob(cv.back.many$plot+ggtitle('CV Performance'), cv.back.many$deviance.plot+ggtitle('Binomial Deviance'), top=paste('alpha=',a,sep=''), ncol=2)
}
...
if (file.exists('/tmp/loo.Rdata')) {
  ...
  for (pt in names(pg.samp)) {
    ...
    cv = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fitLOO$lambda, 
                                  pts=subset(pts, Samplename %in% samples), a=a, nfolds=10, splits=5, fit=fitLOO, select='deviance')
    #plots[[pt]] = cv$plot + ggtitle(pt)
    
    plots[[pt]] = arrangeGrob(cv$plot+ggtitle('CV Performance'), cv$deviance.plot+ggtitle('Binomial Deviance'), top=pt, ncol=2)
    ...
    if ( length(cv$lambda.1se) > 0 ) {
      ...
      pg.samp[[pt]]$Prediction = predict(fitLOO, dysplasia.df[pg.samp[[pt]]$Samplename,], s=cv$lambda.1se, type='response')[,1]
    } else {
...
# do.call(grid.arrange, c(lapply(fits[sample(subset(sum.patient.data, Status == 'NP')$Patient, 4)], function(f) {
#   plt = plot.patient.cv(f$lambdas)   
#   arrangeGrob(plt$performance +ggtitle('CV Performance'), plt$deviance+ggtitle('Binomial Deviance'), ncol=2)
# })))


# do.call(grid.arrange, c(lapply(fits[sample(subset(sum.patient.data, Status == 'P')$Patient, 4)], function(f) {
#   plt = plot.patient.cv(f$lambdas)   
#   arrangeGrob(plt$performance +ggtitle('CV Performance'), plt$deviance+ggtitle('Binomial Deviance'), ncol=2)
# })))
...
predicted.progression<-function(x) { if(is.na(x)) return(NA); as.integer(length(which(x == 1)) > 0)  }
...
ggplot(p.predictions, aes(y=`%.Predicted`,x=n.Endoscopies)) + geom_bar(stat="identity", fill='dodgerblue3') + #geom_text(aes(label=`%.Predicted`), nudge_y=0.005) + 
