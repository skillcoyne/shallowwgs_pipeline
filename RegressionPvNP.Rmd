...
ggplot(performance.at.1se, aes(alpha, mean)) + geom_point() + geom_text( aes(label=round(mean, 3)), hjust=-0.5) +
...
if (file.exists(file)) {
  ...
  for (pt in names(pg.samp)) {
    ...
    a = 1
    fitLOO <- glmnet(training, labels[train.rows], alpha=a, family='binomial', nlambda=1000) # all patients
...
adj.pred = as.data.frame(all.predictions %>% rowwise() %>% summarise( 
  'pred'=round(correctly.predicted/(sum(correctly.predicted, total.samples )),2),
  'total'=round(total.samples/(sum(correctly.predicted, total.samples )),2),
  'Status'=Status))
adj.pred$Patient = rownames(all.predictions)                              


adj.pred = transform(adj.pred, Patient=reorder(Patient, -total) ) 

m = melt(adj.pred, id.vars=c('Status', 'Patient'))
m = arrange(m, Status, variable, -value)
m$Status = as.factor(m$Status)

grid.arrange(
  ggplot(subset(m, Status == 'P') , aes(x=Patient, y=value, fill=variable)) + geom_bar(stat = 'identity') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip() + labs(title='Progressors',x='', y='ratio'),
  ggplot(subset(m, Status == 'NP') , aes(x=Patient, y=value, fill=variable)) + geom_bar(stat = 'identity') +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip() + labs(title='Non-Progressors',x='', y='ratio')
)


pg = subset(adj.pred, Status == 'P')
np = subset(adj.pred, Status == 'NP')
...
When predicting each sample individually `r round(nrow(subset(pg, pred == 0.5))/nrow(pg), 2)*100`% patients were correctly predicted in all samples. `r round(nrow(subset(pg, pred < 0.5 & pred > 0))/nrow(pg),2)*100`% of patients were correctly predicted in some samples

`r nrow(subset(pg, pred == 0))` patients had no correctly predicted samples, however, both had only two samples:
```{r, echo=F, warning=F, message=F}
x = do.call(rbind, lapply(pg.samp[subset(pg, pred == 0)$Patient], function(df) df[,c('Status','Patient','Path.ID','Endoscopy.Year', 'Pathology')] ))
row.names(x) = NULL
pander(x)
```
...
In non-progressors `rnrow(subset(np, pred == 0.5))` patients were predicted to be non-progressors in _all_ of their samples, ranging from `r paste(range(all.predictions[subset(np, pred == 0.5)$Patient, 'total.samples']), collapse='-')`. `r nrow(subset(np, pred >= 0.3))` patients were predicted correctly in 70% or more of their samples. Only `r nrow(subset(np, pred < 0.3))` patients had fewer than 70% of their samples predicted correctly, and both of these have consistently been predicting _progression_ across multiple endoscopies.

`r pander(all.predictions[subset(np, pred < 0.3)$Patient,], caption='Non-progressors that have consistently predicted progression')`
...
## What clinicicians really want 

Backwards from the final sample.
...
back = lapply(pg.samp[subset(sum.patient.data, Status == 'P')$Patient], function(pt) {
  pt = ddply(pt, .(PID, Endoscopy.Year), summarise, 'pred'=sum(as.integer(Prediction > 0.5)), 'samples'=length(PID), 
...
endos = do.call(rbind, lapply(1:max(sapply(back,nrow)), function(i) {
  df = do.call(rbind, lapply(back, function(x) {
    ...
    cbind(x[i, c('Endoscopy.Year','pred','samples','final.endo')], i)
  }))
...
endos = arrange(endos, Patient)
endos = endos %>% group_by(Patient) %>% mutate('n'=length(Patient))
endos = transform(endos, Patient=reorder(Patient, -n) ) 
endos$brks <- cut(endos$pred.ratio, include.lowest=T,
                   breaks=c(0, 0.1, 0.25, 0.5, 0.75, 0.9, 1), 
                   labels=c('0', '0.1-.25', '.25-.5', '.5-.75','.75-0.9', '1'))
                     

## TODO This doesn't seem like the most useful way to display this idea
m = melt(endos[,c('Patient','pred.ratio', 'i','n', 'brks')], id.vars = c('Patient', 'i', 'n', 'brks'))

ggplot(m, aes(Patient, i, fill=brks)) + geom_bar(color='grey', stat='identity', position='dodge') + 
  scale_fill_brewer(type='div', palette = "Set2", "Predicted Ratio") + labs(x='', y='Endoscopy from final') + 
