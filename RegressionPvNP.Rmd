...
```{r noHGD, echo=T, message=F, warning=F, fig.height=12, fig.width=12}
`%nin%` <- Negate(`%in%`)
...
for (a in alpha.values) {
  ...
  plots[[as.character(a)]] = arrangeGrob(cv.nolgd$plot+ggtitle('CV Performance'), cv.nolgd$deviance.plot+ggtitle('Binomial Deviance'), top=paste('alpha=',a,sep=''), ncol=2)
...
for (a in alpha.values) {
  ...
  cv.back.few = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fit0$lambda, pts=subset(pts, Samplename %in% samples), splits=5, nfolds=20, a=a, fit=fit0, minR=0.1)
...
for (a in alpha.values) {
  ...
  cv.back.many = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fit0$lambda, pts=subset(pts, Samplename %in% samples), splits=5, nfolds=20, a=a, fit=fit0)
...
for (pt in names(pg.samp)) {
  ...
  if ( length(cv$lambda.1se) > 0 ) {
    ...
    coef.1se = as.data.frame(non.zero.coef(fit1, cv$lambda.1se))
...
save(plots, performance.at.1se, coefs, fits, file='/tmp/loo.Rdata')
}
...
When predicting each sample individually `r length(pg[pg == 1])` patients were correctly predicted in all samples. The remaining patients were predicted in 80% or fewer samples.   
...
```{r timepoints2, echo=T, message=F, warning=F, eval=F, fig.height=400, fig.width=100}
