...
## TODO score complexity on the seg.vals rather than merged data frame
...
if (file.exists(file)) {
  ...
  save(plots, coefs, performance.at.1se, dysplasia.df, models, file=file)
}
...
```{r hazards, echo=F, warning=F, message=F,eval=T, fig.height=12, fig.width=12}
...
ggplot(ch, aes(coef, hazards, col=hazards >= qq[length(qq)])) + 
  ...
  labs(title='Top features by hazard ratio') + theme(legend.position = 'bottom') + theme(legend.position='none')
```
...
### How do coefficients related to the raw data?
...
plot.endo<-function(df, minM=NULL, maxM=NULL, bin=F) {
  ...
  if (bin) {
    ...
    ggplot(df, aes(months.before.final, endo.pred, group=1,color=Patient)) + facet_grid(Patient ~.) + 
      ...
      geom_label_repel(aes(label=max.path, fill=Patient, color=Patient), color='white', fontface='bold', size=4, label.size=0, label.r=unit(0.25, 'lines'), label.padding=unit(0.15, 'lines')) +
      labs(x='Months before diagnosis', y='Predictive Endoscopy') + 
      theme(text=element_text(size=14, face="bold"), axis.title=element_text(size=16), legend.position='none')
  } else {
  ggplot(df, aes(months.before.final, pred.ratio, group=1, color=Patient, fill=Patient)) + facet_grid(Patient ~.) +
      ylim(0,1) + geom_abline(slope=0, intercept=seq(.25,.75,.25), color='grey', linetype='dashed') +
      geom_col(position='dodge', aes(y=pred.ratio)) + geom_line() +
      scale_x_continuous(breaks=seq(minM, maxM, by = 12)) + 
      geom_label(aes(y=ifelse(pred.ratio==1, 0.75, pred.ratio), label=max.path, fill=Patient), color='white', fontface='bold', size=4, label.size=0, label.r=unit(0.25, 'lines'), label.padding=unit(0.15, 'lines'), nudge_y=0.1) +
...
```{r validation, message=F, warning=F, echo=F, fig.height=8, fig.width=6, eval=F}
...
if (nrow(subset(np,pred.ratio == 0)) > 0) {
  p2 = p2 +  geom_text( aes(x=subset(np,pred.ratio == 0)$Patient[1], y=0.5, label=paste(round(nrow(pn[pn$pred.ratio <= 0,])/nrow(np),3)*100,'%',sep='')), size=8, fontface='bold' ) 
}
...
```{r, echo=F, message=F, warning=F, fig.width=8, fig.height=12, eval=F}
...
by.path = do.call(rbind.data.frame, lapply(vpd, function(pt) {
  final.endo = subset(pt, Endoscopy.Year == max(Endoscopy.Year))
  final.endo = final.endo[nrow(final.endo),]
