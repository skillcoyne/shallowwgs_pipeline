...
if (file.exists(file)) {
  ...
  for (a in alpha.values) {
    ...
    cv.patient = crossvalidate.by.patient(x=dysplasia.df, y=labels, lambda=l, pts=pts, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance')
...
stability = do.call(rbind, lapply(coef.stable, function(cf) {
  cbind('>=75%'=length(which(cf >= 0.75)),  '>=50%'=length(which(cf >= 0.5)),  '>=25%'=length(which(cf >= 0.25)), 'Total'=length(cf))
}))
rownames(stability) = names(coef.stable)

pander(stability, justify='left', caption='Number of features stable in n% or more folds')
```
...
#autoplot(fitV, xvar='lambda', main=paste('fit0, all samples, alpha=',a,sep='')) + theme(legend.position='none') 
cv.patient = crossvalidate.by.patient(x=dysplasia.df, y=labels, lambda=fitV$lambda, pts=pts, a=select.alpha, nfolds=folds, splits=splits, fit=fitV)
lambda.opt = ifelse( length(cv.patient$lambda.1se) > 0, cv.patient$lambda.1se, cv.patient$lambda.min )

pred = predict(fitV, newx=validation.df, s=lambda.opt, type='response')


statusV = unlist(sapply(validation.patient.data, function(df) {
  df$info = arrange(df$info, Endoscopy.Year, Pathology)
  label = as.integer(df$info$Status == 'P') #as.integer(df$info$Pathology %in% c('HGD', 'IMC'))
  names(label) = df$info$Samplename
  return(label)
}))
names(statusV) = sub('.*\\.', '',  names(statusV))

# Progressors - by sample
length( which(pred[ names(which(statusV == 1)), ] > 0.5) )/length(which(statusV == 1))

# Non Progressors - by sample
length( which(pred[ names(which(statusV == 0)), ] < 0.5) )/length(which(statusV == 0))
...
Using the features from the alpha=0.9 models
...
grid.arrange(ggplot(topFRegions, aes(reorder(chr, -adj.info.content), totalbp/length, fill=ai.cut)) + ylim(0, 0.5) +
  ...
  bottom='Ratio of base pair coverage per chromosome in the top non-zero features selected from the model.\nChromosomes are sorted and binned by their gene (e.g. information) content rather than by overall length.')
...
# Exons correlate with the adjusted information content measure, expected
...
write.table(containedCC$ensembl_gene_id, quote=F, row.names = F, col.names = F, file='feature_cosmic.txt')
...
grid.arrange(ggplot(topFRegions, aes(reorder(chr, -adj.info.content), COSMIC.cont/COSMIC.all, fill=ai.cut)) + geom_bar(stat='identity') +
  ...
  scale_fill_discrete(name='Length adjusted information content') + theme(legend.position = 'bottom'), 
  bottom='Again, but this time looking only at COSMIC genes, the ratio of COSMIC genes\nwithin the selected feature vs all COSMIC genes on that chromosome.\nChromosomes are ordered and colored by information content.')


colSums(topFRegions[,c('COSMIC.all','COSMIC.cont')],na.rm=T)
colSums(topFRegions[,c('protein_coding','gene.cnt')],na.rm=T)

#table(topFRegions$ai.cut)
...
if (file.exists(file)) {
  ...
  #hist(colSums(gain.df))
  #hist(colSums(loss.df))
  #hist(colSums(all.binary))

  gp = run.cv(x=as.matrix(gain.df), y=labels, pts=pts, nfolds=folds, splits=splits, alphas=c(0,0.5,0.8,1))
  lp = run.cv(x=as.matrix(loss.df), y=labels, pts=pts, nfolds=folds, splits=splits, alphas=c(0,0.5,0.8,1))
  ...
  both = run.cv(x=as.matrix(gl.df), y=labels, pts=pts, nfolds=folds, splits=splits, alphas=c(0,0.5,0.8,1))

  save(gp, lp, all, file=file)
}
...
```{r, echo=F, warning=F, message=F, fig.height=6, fig.width=4}
...
length(which(pilot$Prediction > 0.5))/nrow(pilot)

p2 = subset(pilot, Patient %nin% c('AD0591', 'AH0329'))
length(which(p2$Prediction > 0.5))/nrow(p2)
...
length(which(exome$Prediction > 0.5))/nrow(exome)

e2 = subset(exome, Patient %nin% c('AD0591', 'AH0329'))
length(which(e2$Prediction > 0.5))/nrow(e2)

```

The false positive rate in both the exome and NP_pilot batches is driven by two patients that we predict to be progressors in nearly every sample: AD0591 and AH0329.  Both patients had samples in the exome batch, while AH0329 also had samples in the NP_Pilot batch.  Without those patients the FPR in the exome batch was `r round(length(which(e2$Prediction > 0.5))/nrow(e2), 2)*100`% for `r length(unique(e2$Patient))` patient(s), and in the NP_Pilot batch the FPR was `r round(length(which(p2$Prediction > 0.5))/nrow(p2), 2)*100`% for `r length(unique(p2$Patient))` patient(s).

The somewhat lower Batch_1 prediction rate is driven by the samples from AHM0277 as only `r length(which(pg.samp$AHM0277$Prediction > 0.5))/nrow(pg.samp$AHM0277)*100`% of this patient's samples were predicted to progress.  Batch_3 is driven by three other patients that have low prediction rates: PR1_ADH_076, PR1_HIN_046, and PR1_WSH_026.

### Look at these specific patients
...
wilcox.test(subset(prog, Prediction > 0.5)$SampleSD, subset(prog, Prediction < 0.5)$SampleSD)
wilcox.test(subset(nonp, Prediction > 0.5)$SampleSD, subset(nonp, Prediction < 0.5)$SampleSD)

wilcox.test(prog$SampleSD, nonp$SampleSD) # Appears to be a small difference in this measure, where is it from?

## Progressors
tprows = which(prog$Prediction > 0.5)
...
tprowsNP = which(nonp$Prediction < 0.5)
...
ggplot(prog, aes(x=SampleSD)) + 
  geom_histogram(data=subset(prog, Prediction > 0.5), aes(SampleSD, fill='TP'), alpha=0.5) +
  geom_histogram(data=subset(prog, Prediction < 0.5), aes(SampleSD, fill='FN'), alpha=0.5) + 
  labs(title='SD of Samples vs True positive ~ False negative Progressors')

ggplot(nonp, aes(x=SampleSD)) + 
  geom_histogram(data=subset(nonp, Prediction < 0.5), aes(SampleSD, fill='TN'), alpha=0.5) + 
  geom_histogram(data=subset(nonp, Prediction > 0.5), aes(SampleSD, fill='FP'), alpha=0.5) +
  labs(title='SD of Samples vs False positive ~ True negative Non-Progressors')
