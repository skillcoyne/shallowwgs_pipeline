...
includeDemo = FALSE # demographics
...
#dysplasia.df = apply(dysplasia.df, 2, function(x) x-median(x))
...
```{r addDemoCols, echo=T, message=F, warning=F}
  ...
  # t.test(
  # complexity.measures[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID],
  # complexity.measures[subset(sum.patient.data, Status == 'NP')$Hospital.Research.ID]
  # )
  ...
  score.cx <-function(pd) {
    ...
    #extremes = apply(as.matrix(pd$seg.vals[,samples]), 2, function(col) {
    #  length(which(col >= (median(col)+2*sd(col)))) + length(which(col <= (median(col)-2*sd(col)))) 
    #}) 

    num.var.reg = vector(mode='integer', length=length(samples))
    if (!is.null(pd$variable.regions) && nrow(pd$variable.regions) > 0) {
      num.var.reg = apply(as.matrix(pd$variable.regions[,samples]), 2, function(col) {
        length(which(col >= 1.1 | col <= 0.9))
      })
    }
    
    sample.var = apply(as.matrix(pd$seg.vals[, samples]), 2, var)
    t(cbind(len.var, num.var.reg, sample.var))
  }

  complexity.measures = lapply(patient.data, score.cx)

  dysplasia.df = cbind(dysplasia.df, 'segment.length'=0, 'num.var.regions'=0, 'sample.variance'=0)
  for (pt in names(complexity.measures)) {
    for (sample in colnames(complexity.measures[[pt]])) {
      dysplasia.df[sample,c('segment.length', 'num.var.regions', 'sample.variance')] = as.numeric(complexity.measures[[pt]][,sample])
    }
    #for (sample in rownames(dysplasia.df)) {
      #dysplasia.df[sample, ] = 
        #complexity.measures[subset(patient.info, Samplename == sample)$Hospital.Research.ID]
    #}
  }
  # norm to 1 to keep within the magnitude of the genomic data
  scale.to.1<-function(x) {
    (x-min(x))/(max(x)-min(x))
  }
  
  dysplasia.df[,c('segment.length', 'num.var.regions', 'sample.variance')] = apply(dysplasia.df[,c('segment.length', 'num.var.regions', 'sample.variance')], 2, scale)
  
  #cx = dysplasia.df[,'cx.measure'] 
  #dysplasia.df[,'cx.measure'] = (log(cx)/log(max(cx)))
  #dysplasia.df[,'cx.measure'] = dysplasia.df[,'cx.measure'] - median(dysplasia.df[,'cx.measure'])
...
# dysplasia.df = cbind(dysplasia.df, 'sl.nvr'=0)
# dysplasia.df[, 'sl.nvr'] = scale(dysplasia.df[, "segment.length"] + dysplasia.df[, 'num.var.regions'])
# 
# dysplasia.df = cbind(dysplasia.df, 'sl.nvr.sv'=0)
# dysplasia.df[, 'sl.nvr.sv'] = scale(dysplasia.df[, "segment.length"] + dysplasia.df[, 'num.var.regions'] + dysplasia.df[, 'sample.variance'])
# 
# dysplasia.df = cbind(dysplasia.df, 'sl.sv'=0)
# dysplasia.df[, 'sl.sv'] = scale(dysplasia.df[, "segment.length"] + dysplasia.df[, 'sample.variance'])
# 
# dysplasia.df = cbind(dysplasia.df, 'nvr.sv'=0)
# dysplasia.df[, 'sl.sv'] = scale(dysplasia.df[, "num.var.regions"] + dysplasia.df[, 'sample.variance'])
...
ggplot(ch, aes(coef, hazards, col=hazards > quantile(hazards)[['75%']])) + 
  ...
  geom_text_repel(aes(x=coef, label=ifelse(hazards > quantile(hazards)[['75%']], label, '')),show.legend=F) +
  labs(title='Top features coef vs hazard') + theme(legend.position = 'bottom')
```



```{r echo=F, warning=F, message=F,eval=F, fig.height=12, fig.width=8}
...
#    labs(x='seg. value', title=r, subtitle=paste('haz=',x[['hazards']], ' coef=', x[['coef']], ' MW pval: ',pv,sep=''))
  ggplot(reg, aes(x=labels, y=value, group=labels, fill=labels) ) + geom_jitter(color='grey', show.legend = F) + geom_violin(show.legend = F, alpha=0.5) +
...
do.call(grid.arrange, c(lapply(regions[1:15], function(reg) {
  print(reg$region[1])
  cols = c('indianred3','slateblue2')
  if ( as.numeric(reg$coef[1]) < 0) cols = c('green3', 'darkviolet')

  ggplot(reg, aes(labels, value*coef, fill=labels, color=labels)) + 
    geom_jitter(aes(labels, value), width=0.05, color='yellow4', alpha=0.3) +
    geom_jitter(aes(labels, value*coef, color=labels), width=0.05, alpha=0.5) +
    geom_text_repel(data=subset(reg, value >= quantile(reg$value)[['100%']]), aes(labels, value, label=round(value, 3)), color='yellow4') +
    geom_text_repel(data=subset(reg, value >= quantile(reg$value)[['100%']]), aes(labels, value*coef, label=round(value, 3),color=labels)) + geom_violin(alpha=0.2) + 
    scale_fill_manual(values=cols) + scale_color_manual(values=cols) +
    theme(legend.position = 'none') + labs(title=unique(reg$region)[1], x='')
})))
      
    
    #geom_violin(aes(labels, value, fill=labels), alpha=0.5) + theme(legend.position = 'none')
  
#  x = rbind.data.frame('P'= cbind( 'gain' = length(which(with(subset(reg, labels == 'P'), value > 0))),
#                               'loss' = length(which(with(subset(reg, labels == 'P'), value < 0))) ),

#                        'NP' = cbind( 'gain' = length(which(with(subset(reg, labels == 'NP'), value > 0))),
#                                'loss' = length(which(with(subset(reg, labels == 'NP'), value < 0))) ) )
  ...
  #ggplot(melt(as.matrix(x)), aes(Var2, value, fill=Var1)) + facet_grid(~Var1) +
  #  geom_col( show.legend = F) + labs(x='', y='', title=unique(reg$region)[1]) + scale_fill_manual(values=cols)
...
```{r validation, message=F, warning=F, echo=F, fig.height=8, fig.width=6}

score.cx <-function(pd) { ## OLD method
#  print(pd$info)
...
for (sample in rownames(validation.df)) {
  validation.df[sample, 'cx.measure'] = complexity.measures[subset(validation.patient.info, Samplename == sample)$Hospital.Research.ID]
}
# 
# # norm to 1 to keep within the magnitude of the genomic data
