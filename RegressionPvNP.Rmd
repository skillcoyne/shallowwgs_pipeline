...
## update the info as it has changed
patient.data = lapply(patient.data, function(df) {
  df$info = subset(patient.info, Patient == unique(df$info$Patient))
  df
})

validation.patient.data = lapply(validation.patient.data, function(df) {
  df$info = subset(validation.patient.info, Patient == unique(df$info$Patient))
  df
})
...
```{r, rawvalues, echo=F, eval=F}


p = ggplot(chr.lengths, aes(x=genome.length))

for (pt in subset(sum.patient.data, Status == 'P')$Hospital.Research.ID) {
  df = merge(patient.data[[pt]]$seg.vals, chr.lengths[,c('chrom','chr.length','genome.length')])
  df$g.start = df$start.pos; df$g.end = df$end.pos
  df[which(df$chrom != 1), 'g.start'] = rowSums(df[which(df$chrom != 1), c('start.pos', 'genome.length')])
  df[which(df$chrom != 1), 'g.end'] = rowSums(df[which(df$chrom != 1), c('end.pos', 'genome.length')])
  
  df = melt(df, measure.vars=grep('^D',colnames(df),value=T))

  p = p + geom_segment(data=df, aes(x=g.start,xend=g.end,y=value, yend=value,color=variable),show.legend=F)  
}

p1 = ggplot(chr.lengths, aes(x=genome.length)) + ylim(0,3)
for (pt in subset(sum.patient.data, Status == 'NP')$Hospital.Research.ID) {
  df = merge(patient.data[[pt]]$seg.vals, chr.lengths[,c('chrom','chr.length','genome.length')])
  df$g.start = df$start.pos; df$g.end = df$end.pos
  df[which(df$chrom != 1), 'g.start'] = rowSums(df[which(df$chrom != 1), c('start.pos', 'genome.length')])
  df[which(df$chrom != 1), 'g.end'] = rowSums(df[which(df$chrom != 1), c('end.pos', 'genome.length')])
  ...
  p1 = p1 + geom_segment(data=df, aes(x=g.start,xend=g.end,y=value, yend=value,color=variable),show.legend=F)  
}
...
ggplot(patient.info, aes(Age.at.diagnosis, fill=Status)) + geom_histogram(binwidth=3) + labs(title='Age at diagnosis')

ggplot(patient.info, aes(BMI, fill=Status)) + geom_histogram(binwidth=1) + labs(title='BMI')

ggplot(patient.info, aes(Maximal,Circumference, color=Status)) + geom_point() + labs(title='C vs M')


grid.arrange(ggplot(patient.info, aes(p53.Status, fill=Status)) + geom_bar(position = position_dodge()) + labs(title='p53 Staining'),
  tableGrob(  rbind( 'Non-progressor'=summary(subset(patient.info, Status == 'NP')$p53.Status),
...
`r pander(p53Path/table(subset(patient.info, Status == 'P' & !is.na(p53.Status))$Pathology), caption='Ratio of progressors predicted by p53 staining per pathology where p53 staining was done (excluding N/As)')`
...
ggplot(as.data.frame(performance.at.1se), aes(y=performance.at.1se, x='LOO')) + ylim(0.5,0.9) +
  ...
  labs(y='performance at lambda.1se', x='', title=paste('Performance for Leave One (patient) Out, at alpha=',select.alpha,sep=''))
