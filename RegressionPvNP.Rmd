...
patient.info = read.patient.info(patient.file, demo.file, set=dataset)$info
head(patient.info)
...
sum.patient.data = as.data.frame(subset(sum.patient.data, Hospital.Research.ID %in% names(patient.data))) ## For now
...
crossvalidate.by.patient<-function(x,y,lambda,pts,a=1,nfolds=10, splits=5, fit=NULL, minR=0.2, select='deviance', ...) {
  ...
  if (select == 'classification') {
    ...
    lambdas = subset(df, log.lambda <= log(lmin) + log(sd(lambda))/2 )
    n = 3
    while (nrow(lambdas) <= 0 & n < 10) {
      lambdas = subset(df, log.lambda <= log(lmin) + log(sd(lambda))/n )
      n = n+1
    }
    if (nrow(lambdas) <= 0) {
      df[subset(df, log.lambda <= log(lmin) )[1, 'lambda.at'], 'lambda.1se'] = T  
    } else {
      df[subset(df, log.lambda <= log(lmin) + log(sd(lambda))/n )[1, 'lambda.at'], 'lambda.1se'] = T  
    }
  }
...
coefs = list(); plots = list(); performance.at.1se = list(); models = list();
folds = 10; splits = 5 
...
if (file.exists(file)) {
  ...
  for (a in alpha.values) {
    ...
    cv.patient = crossvalidate.by.patient(x=dysplasia.df, y=labels, lambda=l, pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', standardize=F)
    #cv.patient$deviance.plot
  ...
  save(plots, coefs, performance.at.1se, dysplasia.df, models, labels, file=file)
}
...
```{r featureinfo, warning=F, message=F, eval=T}
...
gl = cbind.data.frame(
  'P.gain' = apply(features.in.prog, 2, function(x) sum( as.integer(x > 0) ) ), 
  'P.loss' = apply(features.in.prog, 2, function(x) sum( as.integer(x < 0) ) ),
  'NP.gain' = apply(features.nonprog, 2, function(x) sum( as.integer(x > 0) ) ),
  'NP.loss' = apply(features.nonprog, 2, function(x) sum( as.integer(x < 0) ) ) ) 

allgl = cbind.data.frame(
  'ALL.P.gain' = apply(dysplasia.df[names(which(labels == 1)), genomeCols], 2, function(x) sum( as.integer(x > 0) ) ), 
  'ALL.P.loss' = apply(dysplasia.df[names(which(labels == 1)),genomeCols], 2, function(x) sum( as.integer(x < 0) ) ),
  'ALL.NP.gain' = apply(dysplasia.df[names(which(labels == 0)),genomeCols], 2, function(x) sum( as.integer(x > 0) ) ), 
  'ALL.NP.loss' = apply(dysplasia.df[names(which(labels == 0)),genomeCols], 2, function(x) sum( as.integer(x < 0) ) ) )
...
# Gains/losses enriched in the top features?  Appears not
...
```{r bplen, echo=F, warning=F, message=F}
...
topFSegments$chr = factor(topFSegments$chr, levels=droplevels(chromInfo$chr[1:22]), ordered=T)
...
grid.arrange(
  ...
  ggplot(topFRegions, aes(chr, totalbp/length, fill=ai.cut)) + ylim(0, 0.5) +
...
#head(bm)
  ...
  pander(with(topFSegments, cor.test(genes, (end-start))), caption="Number of genes per segment may be related to the segment length.")
}, error = function(e) {
...
```{r hazards, echo=F, warning=F, message=F,eval=T, fig.height=6, fig.width=6}
...
ggplot(ch, aes(coef, hazards, col=hazards >= qq[length(qq)])) + 
  ...
  labs(title='Top features by hazard ratio') + theme(legend.position = 'none', text = element_text(size=14, face='bold')) 
```
...
```{r cosmic, echo=F, warning=F, message=F}
if (!is.null(bm)) {
  ...
  coefGR$stability = rowSums(coefs[[select.alpha]][grcfs,-1])/(folds*splits)
...
write.table(as.data.frame(coefGR[order(-coefGR$stability)])[, c('seqnames', 'start','end','coef','stability','hazard', 'COSMIC')], sep='\t', quote=F, row.names=F, file='cosmic_genes.txt')
...
```{r, echo=F}
pander(as.data.frame(coefGR[coefGR$COSMIC != ""])[c(1:3, 11)], caption='COSMIC genes mapped to coefficients. Only a few regions have mapped COSMIC genes, but several regions neighbor those with important tumour supressors (TP53) and may have not been selected through an artifact of the GLM process.')
```


### How do coefficients relate to the raw data?

```{r warning=F, message=F, fig.height=8, fig.width=8}
ch = arrange(ch, -stability, -hazards, -coef)
ch$hazards = round(ch$hazards, 3)
ch$coef = round(ch$coef, 3)
write.table(ch[c('label', 'stability','hazards','coef')], row.names = F, quote=F, sep="\t", file='features.txt')
...
melt.by.reg<-function(x) {
  ...
  reg$labels = factor(reg$labels, levels=c(1,0), ordered=T)
  reg$labels = revalue(reg$labels, c("1"="P", "0"="NP"))

  #reg = factor(reg$labels, levels=c('P','NP'), ordered=T)
  
  tt = wilcox.test(dysplasia.df[ which(labels == 0) , r ], dysplasia.df[ which(labels == 1) , r ])

  reg$pval = tt$p.value
  reg$region = r
  reg$hazard = as.numeric(x[['hazards']])
  reg$coef = as.numeric(x[['coef']])
  return(reg)
}
...
#   reg = melt.by.reg(x)
#   
#   pv = ifelse(reg$pval[1] < .001, '< .001', format.pval(unique(reg$pval[1])))
# 
#   ggplot(reg, aes(x=labels, y=value, group=labels, color=labels, fill=labels) )  + geom_violin(alpha=0.5) + geom_jitter(width=0.3, alpha=0.8) +
#     geom_hline(yintercept=with(reg, c(mean(value)+sd(value)*2, mean(value)-sd(value)*2)), color='darkgrey', linetype='dashed') +
#     #geom_hline(yintercept = mean(reg$value) + sd(reg$value)) + geom_hline(yintercept = mean(reg$value) - sd(reg$value)) +
#     labs(x='', y='seg. value', title=x[['label']], subtitle=paste('haz= ',x[['hazards']], ' coef= ',x[['coef']], '\nMW pval ',pv,sep='')) + theme(legend.position = 'none')
# }

regions = apply(ch, 1, melt.by.reg)
names(regions) = ch$label

plot.features<-function(reg) {
  se = 2
  cols = c('indianred3','slateblue2')
  if ( as.numeric(reg$coef[1]) < 0) cols = c('green3', 'darkviolet')
  ...
  p = ggplot(reg, aes(labels, value, group=labels, fill=labels, color=labels)) + geom_violin(alpha=0.2) +
    geom_jitter(width=0.05) +
    geom_hline(yintercept=with(reg, c(mn+sd(value)*se, mn-sd(value)*se)), color='darkgrey', linetype='dashed') +
    scale_fill_manual(values=cols) + scale_color_manual(values=cols) + theme(legend.position = 'none') + labs(x='', subtitle=reg$coef[1])

   x = rbind.data.frame('P'= cbind( 'gain' = length(which(with(subset(reg, labels == 'P'), value >=  mn+sd(value)*se))),
                                    'loss' = -length(which(with(subset(reg, labels == 'P'), value <=  mn-sd(value)*se))) ),
   
                        'NP' = cbind( 'gain' = length(which(with(subset(reg, labels == 'NP'), value >=  mn+sd(value)*se))),
                                      'loss' = -length(which(with(subset(reg, labels == 'NP'), value <=  mn-sd(value)*se))) ) )

   x = x*sign(reg$coef[1])

  p2 = ggplot(melt(as.matrix(x)), aes(Var2, value, fill=Var1, 
                                      color=ifelse(value > 0, 'red', 'white'))) + #facet_grid(~Var1) +
        geom_bar(stat='identity', width=0.5, position='dodge') + 
        geom_hline(yintercept = 0, color='black', size=1.5) +
        labs(x='', y='Counts', subtitle=reg$coef[1]) + 
        scale_fill_manual(values=cols) + theme(legend.position = 'none')

  return(list('plot'=arrangeGrob(p, p2, ncol=2, top=unique(reg$region)[1]), 'counts'=x))     
}


pf = lapply(regions, plot.features)


do.call(grid.arrange, c(lapply(pf[1:12], function(x) x$plot), ncol=3) )

```

### Examine a few example regions

```{r, echo=F, eval=F}
# p53 regions
chr17 = get.chr.lengths()[17,]

samples = names(which(dysplasia.df[,'17:1-4776188'] <= -1 ) )

#patients = subset(patient.info, Samplename %in% samples)
#table(patients$Pathology)
#nrow(patients)/nrow(subset(patient.info, Patient %in% patients$Patient))

values17 = dysplasia.df[samples, grep('^17:', colnames(dysplasia.df), value=T)]

values17 = merge(values17, subset(patient.info, Samplename %in% samples , select=c('Patient','Samplename')), by.x='row.names', by.y='Samplename')
values17 = melt(values17, measure.vars = grep('^17:',colnames(values17), value=T))
colnames(values17) = c('sample','Patient','region','value')
head(values17)

values17$selected = as.integer( as.character(values17$region) == '17:1-4776188' )

loc = do.call(rbind.data.frame, strsplit(as.character(values17$region), ':|-'))
colnames(loc) = c('chr','start', 'end')
loc[] = lapply(loc[], function(x) as.numeric(as.character(x)))

values17 = cbind(values17, loc)


genes = ccgenes[grep("BRCA1|SMARCE1|STAT3|STAT5B|TP53", ccgenes$Gene.Symbol),c('Gene.Symbol','Genome.Location')]

loc = unlist(strsplit(ccgenes[which(ccgenes$Gene.Symbol == 'TP53'),'Genome.Location'], ':|-'))
genes = cbind(genes, 'start'=loc[1], 'value'=-7, 'Row.names'='', 'Patient'='', 'selected'=as.factor(0))

#loc = do.call(rbind.data.frame, strsplit(ccgenes[grep("TP53|BRCA1|SMARCE1|STAT3|STAT5B", ccgenes$Gene.Symbol),'Genome.Location'], ':|-'))
#colnames(loc) = c('chr','start','end')
genes = cbind.data.frame('chr'=as.numeric(loc[1]), 'start'=as.numeric(loc[2]), 'end'=as.numeric(loc[3]), 'Gene.Symbol'='TP53', 'selected'=as.factor(0), 'sample'='', value=-7)

values17$sample = as.factor(values17$sample)
values17$selected = as.factor(values17$selected)

arms = cbind.data.frame('x'=rbind( with(chr17, chr.cent-cent.gap*3), with(chr17, chr.cent+cent.gap*3)), 'y'=rbind(10,10), 'label'=c('p arm','q arm'))
head(values17)


p = ggplot() + xlim(1,chr17$chr.length) + 
    #geom_rect(data=values17, aes(xmin=start, xmax=end, ymin=0, ymax=value, fill=Patient), alpha=0.2, show.legend=F) +
    geom_segment(data=values17, aes(x=start, xend=end, y=value, yend=value, color=Patient), show.legend=F, size=1.5) +
    geom_segment(data=unique(subset(values17, selected == 1, select=c('chr','start','end'))), aes(x=start, xend=end, y=0, yend=0), size=2, color='red') +
    geom_point(data=genes, aes(x=start,y=value), color='red', size=3, show.legend = F) +
    geom_text_repel(data=genes, aes(x=end, y=value, label=Gene.Symbol), color='red', show.legend = F, nudge_y=-0.1) +
    geom_vline(xintercept=chr17$chr.cent, color='red', linetype='longdash') +
    geom_label(data=arms, aes(x,y, label=label), fill='white') +
    geom_hline(yintercept = -1, color='grey39', linetype='longdash') + geom_hline(yintercept = 1, color='grey39', linetype='longdash') +
    geom_label(data=cbind('x'=chr17$chr.cent/2, 'y'=0), aes(x, y), label='Losses', color='white', fill='grey39') + 
    labs(y='Segment value', x='Chr position', title='Chromosome 17', subtitle=paste(length(table(values17$Patient)), 'patients,', length(samples), 'samples')) +
  theme(text=element_text(size=16), axis.text=element_text(size=12))

png(filename='chr17.png', width=600, height=600)
print(p)
dev.off()
...
```{r, eval=F}
# BRCA etc
regions = subset(ch, grepl('^17', label) & coef > 0)
samples = which(dysplasia.df[,regions$label[1]] > 1 ) 
samples = union(names(samples), names(which(dysplasia.df[,regions$label[2]] > 1 )) )


cols = grep(paste(regions$label, collapse='|'), colnames(dysplasia.df))

values17 = cbind(dysplasia.df[samples, c(cols-2,cols-1,cols,cols+1,cols+2)], 'labels'=labels[samples])
values17 = merge(values17, subset(patient.info, Samplename %in% samples , select=c('Hospital.Research.ID','Patient','Samplename')), by.x='row.names', by.y='Samplename')

values17 = melt(values17)

values17$selected = as.integer(values17$variable %in% ch[grepl('^17', ch$label),'label'])

loc = do.call(rbind.data.frame, strsplit(as.character(values17$variable), ':|-'))
colnames(loc) = c('chr','start', 'end')
loc[] = lapply(loc[], function(x) as.numeric(as.character(x)))

values17 = cbind(values17, loc)

genes = ccgenes[grep("BRCA1|SMARCE1|STAT3|STAT5B", ccgenes$Gene.Symbol),c('Gene.Symbol','Genome.Location')]
loc = do.call(rbind.data.frame, strsplit(genes$Genome.Location, ':|-'))
colnames(loc) = c('chr','start','end')
loc[] = lapply(loc[], function(x) as.integer(as.character(x)))

genes = cbind.data.frame(genes, loc, 'value'=as.integer(5), 'Row.names'='', 'Patient'='', 'selected'=as.factor(0))

values17$Row.names = as.factor(values17$Row.names)
values17$selected = as.factor(values17$selected)
head(values17)
ggplot(values17, aes(start, value)) + # xlim(1,chr17$chr.length) + #facet_wrap(~Patient, ncol=1) +
    geom_segment(aes(x=start, xend=end, y=value, yend=value, fill=Patient, color=Patient), show.legend=F) + 
    geom_segment(data=unique(subset(values17, selected == 1, select=c('chr','start','end'))), aes(x=start, xend=end, y=0, yend=0), size=2, color='red') +
    geom_point(data=genes, aes(x=start,y=value), color='red', size=3, show.legend = F) +
    geom_text_repel(data=genes, aes(x=end, y=value, label=Gene.Symbol), color='red', show.legend = F) +
    geom_hline(yintercept = 1, color='grey39', linetype='longdash') +
    geom_label(x=median(values17$start, na.rm=T), y=7, label='Gains', color='white', fill='grey39') + 
    labs(y='Segment value', x='chr17', title='chr17') 
  
png(filename='chr17.png', width=800, height=1000)
print(p)
dev.off()

  
# FGFR2 region
chr10 = get.chr.lengths()[10,]

samples = which(dysplasia.df[,'10:121013167-125853693'] > 1)

cols = grep('^10:121013167-125853693', colnames(dysplasia.df))
values10 = cbind(dysplasia.df[samples, c(cols-1, cols, cols+1)], 'labels'=labels[samples])
values10 = merge(values10, subset(patient.info, Samplename %in% names(samples) , select=c('Hospital.Research.ID','Patient','Samplename')), by.x='row.names', by.y='Samplename')

values10 = melt(values10)

loc = do.call(rbind.data.frame, strsplit(as.character(values10$variable), ':|-'))
colnames(loc) = c('chr','start', 'end')
loc[] = lapply(loc[], function(x) as.numeric(as.character(x)))

values10 = cbind(values10, loc)

loc = unlist(strsplit(ccgenes[which(ccgenes$Gene.Symbol == 'FGFR2'),'Genome.Location'], ':|-'))
fgfr2 = cbind.data.frame('chr'=as.numeric(loc[1]), 'start'=as.numeric(loc[2]), 'end'=as.numeric(loc[3]), 'Row.names'='FGFR2', 'Patient'='', value=5)

values10$Row.names = as.factor(values10$Row.names)
head(values10)
ggplot(values10, aes(start, value, fill=Patient, color=value)) + #xlim(1,chr10$chr.length) + #facet_wrap(~Patient, ncol=1) +
    geom_segment(aes(x=start, xend=end, y=value, yend=value), show.legend = F) + 
    geom_point(data=fgfr2, aes(x=start,y=value), color='red', size=3, show.legend = F) +
    geom_text_repel(data=fgfr2, aes(x=end, y=value, label=Row.names), show.legend = F, color='red') +
    #geom_vline(xintercept=chr10$chr.cent, color='red', linetype='longdash') +
    #geom_label(x=chr10$chr.cent-chr10$cent.gap*3, y=10, label='p arm', color='white') + 
    #geom_label(x=chr10$chr.cent+chr10$cent.gap*3, y=10, label='q arm', color='white') +
    geom_hline(yintercept = 1, color='grey39', linetype='longdash') +
    #geom_label(x=chr10$chr.cent/2, y=0, label='Losses', color='white', fill='grey39') + 
    labs(y='Segment value', x='chr10', title='chr10')  + scale_color_continuous(low='red',high='green')
...
### Is there a difference in P~NP for gains/losses

Based on the relationship between the coefficients and the values of the data we would expect one.  
```{r}
features.binom <- function(df) {
  p.adjust(apply(df, 1, function(x) {
    binom.test(x[2], sum(x), 0.5)$p.value
  }))  
}
...
gain = do.call(rbind, lapply(gains$label, function(reg) {
  values = dysplasia.df[,reg] 
  table(labels[which(values > sd(values))])
}))

# There is a difference in the gains NP~P in the positive coefficients
wilcox.test(gain[,1], gain[,2])
summary(aov(gain[,1]~gain[,2]))

which(features.binom(gain) < 0.05)

fisher.test(gain, simulate.p.value = T)

loss = do.call(rbind, lapply(gains$label, function(reg) {
  values = dysplasia.df[,reg] 
  table(labels[which(values < -sd(values))])
}))

# There may be on in the losses as well, but less clear
wilcox.test(loss[,1], loss[,2])
summary(aov(loss[,1]~loss[,2]))

which(features.binom(loss) < 0.05)

# Compared to randomly selected regions (not the coefficients)
sample.reg = sample(setdiff(colnames(dysplasia.df), ch$label), nrow(ch))
...
# P~NP may be different across randomly selected regions as well, again less clear
wilcox.test(randomGains[,1], randomGains[,2])
summary(aov(randomGains[,1]~randomGains[,2]))
fisher.test(randomGains, simulate.p.value = T)

which(features.binom(randomGains) < 0.05)
...
# In this case losses show a definite difference
wilcox.test(randomLosses[,1], randomLosses[,2])
summary(aov(randomLosses[,1]~randomLosses[,2]))

which(features.binom(randomLosses) < 0.05)
...
If gains or losses are primarily driving this, can a model that only looks at gains or losses predict better? 
...
run.cv<-function(x, y, alphas=c(0,0.2,0.5,1), ...) {
  ...
  for (a in alphas) {
    fitg <- glmnet(x, y, alpha=a, nlambda=nl, family='binomial', standardize = F) # all patients
    ...
    cvp = crossvalidate.by.patient(x, y, fit=fitg, lambda=l, a=a, pts=sets, standardize=F)
...
Gains alone are pretty much the same as the raw data.
```{r warning=F, message=F, fig.height=8, fig.width=8}
...
When we try to put them together, so each region has two columns where gains and losses are considered separately the classification drops to < 50%. So we're better off using the merged data we started with and gains/losses alone aren't simply driving the prediction.
```{r warning=F, message=F, fig.height=8, fig.width=8}
...
```{r manyfew, echo=T, message=F, warning=F, fig.height=16, fig.width=16, eval=F}
## Compare progressors with many samples vs few
m = median(subset(sum.patient.data, Status == 'P')$total.samples)
nonps = as.vector(unlist(lapply(patient.data[subset(sum.patient.data, Status == 'NP')$Hospital.Research.ID], function(df) df$info$Samplename)))
...
if (file.exists(file)) {
  ...
  # Few
  samples = c(nonps, as.vector(unlist(lapply(patient.data[subset(sum.patient.data, Status == 'P' & total.samples < m)$Hospital.Research.ID], function(pt) pt$info$Samplename))))
  
  few.plots = list()
  for (a in alpha.values) {
    fitF <- glmnet(dysplasia.df[samples,], labels[samples], alpha=a, nlambda=nl, family='binomial', standardize = F) # all patients
    cv.back.few = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fitF$lambda, pts=subset(pts, Samplename %in% samples), splits=5, nfolds=folds, a=a, fit=fitF, minR=0.1, standardize = F)
...
if (file.exists(file)) {
  ...
  many.plots = list()
  for (a in alpha.values) {
    fitM <- glmnet(dysplasia.df[samples,], labels[samples], alpha=a, nlambda=nl, family='binomial', standardize = F) # all patients
    
    cv.back.many = crossvalidate.by.patient(x=dysplasia.df[samples,], y=labels[samples], lambda=fitM$lambda, pts=subset(pts, Samplename %in% samples), splits=5, nfolds=20, a=a, fit=fitM, standardize = F)
...
if (file.exists(file)) {
  ...
  r.labels = unlist(apply(rand.patients[,c('Hospital.Research.ID','Status')],1, function(st) {
    info = patient.data[[ st['Hospital.Research.ID'] ]]$info
  ...
  # sort in label order
  r.dysplasia.df = dysplasia.df[names(r.labels),]

  r.coefs = list(); r.plots = list()
  folds = 10; splits=5
  for (a in c(0,0.5,1)) {
    fitR <- glmnet(r.dysplasia.df, r.labels, alpha=a, nlambda=nl, family='binomial', standardize = F) # all patients
    
    cv.R = crossvalidate.by.patient(x=r.dysplasia.df, y=r.labels, lambda=fitR$lambda, pts=pts, a=a, nfolds=folds, splits=splits, fit=fitR, standardize = F)
...
df = cbind.data.frame('Specificity'=rev(roc50$specificities), 'Sensitivity'=rev(roc50$sensitivities))
g1 = ggplot(df, aes(Specificity, Sensitivity)) + geom_line() +
  scale_x_reverse() + 
  geom_label(data=t(as.data.frame(round(pROC::coords(roc50, "best"),2))), 
             aes(x=specificity, y=sensitivity, label=paste('prob=',threshold, ' (FPR ', specificity*100, '%, TPR ', sensitivity*100,'%)\nAUC:', sprintf("%.2f",roc50$auc*100), '%', sep='')), nudge_x=.25) +
...
g2 = ggplot(df, aes(Specificity, Sensitivity)) + geom_line() +
  scale_x_reverse() + 
  geom_label(data=t(as.data.frame(round(pROC::coords(roc, "best"),2))), aes(x=specificity, y=sensitivity, label=paste('prob=',threshold, ' (FPR ', specificity*100, '%, TPR ', sensitivity*100,'%)\nAUC:', sprintf("%.2f",roc$auc*100), '%', sep='')), nudge_x=.25) +
...
This argues that we should set the prediction probability cutoff to `r pROC::coords(roc, 'best')[['threshold']]`. 
...
hgd = sapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
  hgd = subset(pt, Pathology %in% c('HGD', 'IMC'))
  sum(as.integer(hgd$Prediction > cutoff))/nrow(hgd)
})

hgd = do.call(rbind, lapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
  hgd = subset(pt, Pathology %in% c('HGD', 'IMC'))
  cbind.data.frame('correctly.predicted'=sum(as.integer(hgd$Prediction > cutoff)), 'total.hgd.samples'=nrow(hgd))
}))
rownames(hgd) = subset(sum.patient.data, Hospital.Research.ID %in% rownames(hgd))$Patient
...
by.path = do.call(rbind.data.frame, lapply(pg.samp[subset(sum.patient.data, Status == 'NP')$Hospital.Research.ID], function(pt) {
...
sum(by.path$NDBE)/sum(by.path$total.be)
sum(by.path$LGD)/sum(by.path$total.l)
sum(by.path$ID)/sum(by.path$total.id)
sum(by.path$HGD)/sum(by.path$total.h)



global.by.path = do.call(rbind.data.frame,
                         lapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
  final.endo = subset(pt, Endoscopy.Year == max(Endoscopy.Year))
  final.endo = final.endo[nrow(final.endo),]
...
ggplot(melt(global.path[,c('ratio','total','path')], measure.vars='ratio', id.vars=c('path', 'total')), aes(path, value, group=path)) + 
  ...
  geom_text(aes(y=3,label=paste('n=', total, sep='')), color='white') +
  ylim(0,100) + labs(title='Progressor samples predicted by pathology', x='Pathology', y='Percent identified') + 
  theme(text=element_text(size=14, face='bold'), panel.background=element_blank(), axis.line=element_line(color='black'), panel.grid.major.x=element_blank(),
        panel.grid.major.y=element_line(color='grey'))
```


```{r echo=F, message=F, warning=F}
lgd = do.call(rbind, lapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
  e = subset(pt, Pathology  == 'LGD')
  cbind.data.frame('predicted'=sum(as.integer(e$Prediction > cutoff)), 'total.lgd'=nrow(e))
}))
rownames(lgd) = subset(sum.patient.data, Hospital.Research.ID %in% rownames(lgd))$Patient
...
# Only BE, Progressors
ndbe.only = do.call(rbind, lapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
...
rownames(ndbe.only) = subset(sum.patient.data, Hospital.Research.ID %in% rownames(ndbe.only))$Patient
...
total = do.call(rbind, lapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
...
rownames(total) = subset(sum.patient.data, Hospital.Research.ID %in% rownames(total))$Patient
...
pre.hgd = sapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], function(pt) {
...
for (i in 1:nrow(p.predictions)) {
  p = sapply(pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID], pred.by.endo, i)
  pp = sapply(p, predicted.progression)
  pp = pp[!is.na(pp)]
  
  fpr = sapply(pg.samp[subset(sum.patient.data, Status == 'NP')$Hospital.Research.ID], pred.by.endo, i)
...
ggplot(m, aes(brks, pred.r*100, group=brks)) + ylim(0,105) +
  geom_bar(stat='identity', fill='dodgerblue3') + 
  geom_text(aes(y=25, label=paste(round(pred.r*100), '%', sep='')), nudge_y=-0.02, color='white') + 
  ...
  theme(text=element_text(size=14, face='bold'), axis.text.x=element_text(angle=45,hjust=1), panel.grid.major.x=element_blank(),   panel.grid.major.y=element_line(color='grey') , axis.line=element_line(color='black'), panel.background=element_blank()) 
...
plot.endo<-function(df, minM=NULL, maxM=NULL, bin=F) {
  ...
  if (bin) {
    ...
    ggplot(df, aes(months.before.final, endo.pred, group=1, color=endo.pred)) + facet_grid(Patient ~.) + 
      ...
      scale_color_manual(values=c('red4', 'green4'), name='Endoscopy Prediction') +
      labs(x='Months before diagnosis', y='Predictive Endoscopy') + 
      theme(text=element_text(size=14, face="bold"), axis.title=element_text(size=16), legend.position='bottom')
  } else {
  ggplot(df, aes(months.before.final, pred.ratio, group=1, color=pred.ratio, fill=pred.ratio)) + facet_grid(Patient ~.) +
      ...
      scale_fill_continuous(low='orangered4', high='green4', name='Sample Prediction Ratio') +
      theme(text=element_text(size=14, face="bold"), axis.title=element_text(size=16), legend.position='bottom')
  }
}
...
p = plot.endo( subset(endos, Patient %in% subset(me, months <= -72)$Patient), bin=T) + 
  labs(title="Progressors", subtitle=paste(72/12, "years or more follow-up"))
#print.png(p, 'progressors_1.png', width=900, height=1200)
grid.arrange(p)

p1a = plot.endo( subset(endos, Patient %in% subset(me, months <= -96)$Patient), bin=T) + 
  labs(title="Progressors", subtitle=paste(96/12, "years or more follow-up"))
#print.png(p1a, 'progressors_1a.png', width=600, height=800)

p1b = plot.endo( subset(endos, Patient %in% subset(me, months <= -72 & months > -96)$Patient), bin=T) + 
  labs(title="Progressors", subtitle=paste(72/12, "-",96/12,"years or more follow-up"))
#print.png(p1b, 'progressors_1b.png', width=600, height=800)

p1 = plot.endo( subset(endos, Patient %in% subset(me, months <= -72)$Patient), bin=F) +
  labs(title="Progressors", subtitle=paste(72/12, "years or more follow-up"))
#print.png(p1, 'progressors_1_bar.png', width=900, height=1200)

```

```{r, echo=F, warning=F, message=F, fig.height=22, fig.width=10}
p1 = plot.endo( subset(endos, Patient %in% subset(me, months <= -48 & months > -72)$Patient), bin=T) +
  labs(title="Progressors", subtitle=paste(48/12, "-",72/12,  "years or more follow-up"))
#print.png(p1, 'progressors_2.png', width=400, height=600)

p1 = plot.endo( subset(endos, Patient %in% subset(me, months <= -48 & months > -72)$Patient), bin=F) +
  labs(title="Progressors", subtitle=paste(48/12, "-",72/12,  "years or more follow-up"))
#print.png(p1, 'progressors_2_bar.png', width=400, height=600)

p2 = plot.endo( subset(endos, Patient %in% subset(me, months <= -24 & months > -48)$Patient), bin=T) +
  labs(title="Progressors", subtitle=paste(24/12, "-",48/12,  "years or more follow-up"))
#print.png(p2, 'progressors_3.png', width=400, height=600)

p2 = plot.endo( subset(endos, Patient %in% subset(me, months <= -24 & months > -48)$Patient), bin=F) +
  labs(title="Progressors", subtitle=paste(24/12, "-",48/12,  "years or more follow-up"))
#print.png(p2, 'progressors_3_bar.png', width=400, height=600)

p3 = plot.endo( subset(endos, Patient %in% subset(me, months >= -12)$Patient), bin=T) + 
  labs(title="Progressors", subtitle=paste("1 year follow-up"))
#print.png(p3, 'progressors_4.png', width=400, height=600)

p3 = plot.endo( subset(endos, Patient %in% subset(me, months >= -12)$Patient), bin=F) +
  labs(title="Progressors", subtitle=paste("1 year follow-up"))
#print.png(p3, 'progressors_4_bar.png', width=400, height=600)
...
plot.endo(subset(nonp.endos, Patient %in% subset(nonp.endos, pred.ratio > 0)$Patient), bin=F)

p = plot.endo( subset(nonp.endos, Patient %in% subset(nonp.endos, pred.ratio > 0)$Patient), bin=T) + 
  labs(title="Non-Progressors", subtitle=paste("Patients with predictive endoscopies, max", abs(min(me$months)/12), 'years follow-up'))
#print.png(p, 'non-progressors.png', width=600, height=800)

p1 = plot.endo( subset(nonp.endos, Patient %in% subset(nonp.endos, pred.ratio > 0)$Patient), bin=F) + 
  labs(title="Non-Progressors", subtitle=paste("Patients with predictive endoscopies, max", abs(min(me$months)/12), 'years follow-up'))
#print.png(p1, 'non-progressors_bar.png', width=600, height=800)
...
pg.samp = lapply(pg.samp, function(df){ df$Batch.Name = sub('_', '', df$Batch.Name ); df })


all = do.call(rbind, pg.samp)

prog = do.call(rbind, pg.samp[subset(sum.patient.data, Status == 'P')$Hospital.Research.ID])
nonp = do.call(rbind, pg.samp[subset(sum.patient.data, Status == 'NP')$Hospital.Research.ID])
...
pilot = pg.samp[subset(patient.info, grepl('nppilotstudy', Batch.Name, ignore.case=T))$Hospital.Research.ID]
...
exome =  pg.samp[subset(patient.info, grepl('exomesubcohort', Batch.Name, ignore.case=T))$Hospital.Research.ID]
...
```{r validation, message=F, warning=F, echo=F, fig.height=8, fig.width=6, eval=T}
...
p1 = ggplot(data=subset(v.predictions, Status == 'P'), aes(Patient, pred.ratio)) + geom_col(fill='darkblue', color='grey') + coord_flip() +
    geom_label( aes(x=p[nrow(subset(p, pred.ratio == 1))/2,'Patient'], y=0.5, label=paste(round(nrow(p[p$pred.ratio == 1,])/nrow(p),2)*100,'%',sep='')), size=8, fill='darkblue', color='white', fontface='bold' ) +
    geom_label( aes(x='77', y=0.5, label=paste(round(nrow(p[p$pred.ratio < 1 & p$pred.ratio > 0,])/nrow(p),3)*100,'%',sep='')), size=8, fill='darkblue', color='white', fontface='bold' ) +
    theme(legend.position='none', axis.text.x = element_text(angle = 45, hjust = 1)) + 
    labs(title='Progressors', x='', y='Ratio predicted samples')
if (nrow(subset(p,pred.ratio == 0)) > 0) {
  p1 = p1 +  geom_text( aes(x=subset(p,pred.ratio == 0)$Patient[1], y=0.5, label=paste(round(nrow(p[p$pred.ratio <= 0,])/nrow(p),3)*100,'%',sep='')), size=8, fontface='bold' ) 
}


p2 = ggplot(data=subset(v.predictions, Status == 'NP'), aes(Patient, pred.ratio)) + geom_col(fill='darkgreen', color='grey') + coord_flip() +
    geom_label( aes(x=np[nrow(subset(np, pred.ratio == 1))/2,'Patient'], y=0.5, 
                    label=paste(round(nrow(np[np$pred.ratio == 1,])/nrow(np),2)*100,'%',sep='')), size=8, fill='darkgreen', color='white', fontface='bold' ) +
    geom_label( aes(x='41', y=0.5, label=paste(round(nrow(np[np$pred.ratio < 1 & np$pred.ratio > 0,])/nrow(np),3)*100,'%',sep='')), size=8, fill='darkgreen', color='white', fontface='bold' ) +
    theme(legend.position='none', axis.text.x = element_text(angle = 45, hjust = 1)) + 
    labs(title='Non Progressors', x='', y='Ratio predicted samples')
if (nrow(subset(np,pred.ratio == 0)) > 0) {
  p2 = p2 +  geom_text( aes(x=subset(np,pred.ratio == 0)$Patient[1], y=0.5, label=paste(round(nrow(np[np$pred.ratio <= 0,])/nrow(np),3)*100,'%',sep='')), size=8, fontface='bold' ) 
}

grid.arrange(p1, p2, ncol=2, top='Validation data set')

```



```{r, echo=F, message=F, warning=F, fig.width=8, fig.height=12, eval=T}
...
p1 = plot.endo(subset(vpi, Patient %in% subset(sum.validation.info, Status == 'P')$Patient), bin=T) + labs(title="Validation - Progressors")
#print.png(p1, 'validation_progressors.png', width=600, height=800)

#print.png(plot.endo(subset(vpi, Patient %in% subset(sum.validation.info, Status == 'P')$Patient), bin=F) + labs(title="Validation - Progressors"), 'validation_progressors_bar.png', width=600, height=800)

p2 = plot.endo(subset(vpi, Patient %in% subset(sum.validation.info, Status == 'NP')$Patient), bin=T) + labs(title="Validation - Non Progressors")

#print.png(p2,'validation_Nonprogressors.png', width=600, height=800)
#print.png(plot.endo(subset(vpi, Patient %in% subset(sum.validation.info, Status == 'NP')$Patient), bin=F) + labs(title="Validation - Non Progressors"),'validation_Nonprogressors_bar.png', width=600, height=800)

grid.arrange(p1, p2,ncol=1)

global.by.path = do.call(rbind.data.frame,
                         lapply(vpd[subset(sum.validation.info, Status == 'P')$Hospital.Research.ID], function(pt) {
  final.endo = subset(pt, Endoscopy.Year == max(Endoscopy.Year))
  final.endo = final.endo[nrow(final.endo),]
...
ggplot(melt(global.path[,c('ratio','total','path')], measure.vars='ratio', id.vars=c('path', 'total')), aes(path, value, group=path)) + 
  ...
  geom_text(aes(y=value+2,label=paste(round(value, 1), '%', sep=''))) +
  geom_text(aes(y=3,label=paste('n=', total, sep='')), color='white') +
  ylim(0,100) + labs(title='Validation samples predicted by pathology', x='Pathology', y='Percent identified') + 
  theme(text=element_text(size=14, face='bold'), panel.background=element_blank(), axis.line=element_line(color='black'), panel.grid.major.x=element_blank(),
        panel.grid.major.y=element_line(color='grey'))
...
# random
df11 = dysplasia.df[,grep('^11:', colnames(dysplasia.df))]
fit0 <- glmnet(df11, labels, alpha=a, nlambda=nl, family='binomial', standardize=F) # all patients
cv11 = crossvalidate.by.patient(x=df11, y=labels, lambda=more.l(fit0$lambda), pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', standardize=F)
plot.cv(cv11,"Chr11")

df17 = dysplasia.df[,grep('^17:', colnames(dysplasia.df))]
fit0 <- glmnet(df17, labels, alpha=a, nlambda=nl, family='binomial', standardize=F) # all patients
cv17 = crossvalidate.by.patient(x=df17, y=labels, lambda=more.l(fit0$lambda), pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', standardize=F)
plot.cv(cv17, 'Chr17')

## No HGD
df17 = dysplasia.df[subset(patient.info, Pathology %in% c('BE','ID','LGD'))$Samplename,grep('^17:', colnames(dysplasia.df))]
fit0 <- glmnet(df17, labels[subset(patient.info, Pathology %in% c('BE','ID','LGD'))$Samplename], alpha=a, nlambda=5000, family='binomial', standardize=F)
cv17 = crossvalidate.by.patient(x=df17, y=labels[subset(patient.info, Pathology %in% c('BE','ID','LGD'))$Samplename], lambda=fit0$lambda, pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', standardize=F)
plot.cv(cv17, 'Chr17, no HGD')



df3 = dysplasia.df[,grep('^3:', colnames(dysplasia.df))]
fit0 <- glmnet(df3, labels, alpha=a, nlambda=nl, family='binomial', standardize=F) # all patients
cv3 = crossvalidate.by.patient(x=df3, y=labels, lambda=fit0$lambda, pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', standardize=F)
plot.cv(cv3, 'Chr3')
...
df = mergedDf[,c('chr','start','end')]
df = cbind(df, t(dysplasia.df[,grep('^\\d+:', colnames(dysplasia.df), value=T)]))
df = merge(df, chr.lengths[,c('chrom','chr.length')], by.x='chr', by.y='chrom')
df = melt(df, id.vars=c('chr','start','end', 'chr.length'))
df$Status = ifelse(df$variable %in% names(which(labels == 1)), 'Progressor', 'Non-Progressor')

df = merge(df, patient.info[,c('Hospital.Research.ID','Patient','Samplename','Pathology')], by.x='variable', by.y='Samplename')
...
# annotation = as.data.frame(bmGR[which(bmGR$COSMIC | bmGR$hgnc_symbol == 'TP53')])
# colnames(annotation) = c('chr', colnames(annotation)[-1])
# head(df)

df$cn = factor(sapply(df$value, cn), levels=c('loss','neutral','gain'), ordered = T)
df$cn = factor(sapply(df$value, cn), levels=c('loss','neutral','gain'), ordered = T)
df$cnB = factor(as.integer(df$cn != 'neutral'))

ggplot(df, aes(x=chr.length)) + facet_grid(Status~chr, scales='free_x') + 
  geom_rect(data=df, aes(xmin=start, xmax=end, ymin=0, ymax=value, fill=cn), alpha=0.5, show.legend = T) + 
  #scale_fill_manual(values=c('grey88','green4')) +
  scale_color_manual(values=c('firebrick3','grey88','green4')) +
  scale_fill_manual(values=c('firebrick3','grey88','green4')) +
  #scale_fill_gradient2(high="green4", mid='grey88', low ="firebrick3", name='') +
  labs(title="Segment Values Across Samples", x='Chromosomes', y='Mean adjusted segmentation value',subtitle=paste('NonP:Prog - ', paste(table(patient.info$Status), collapse=':'))) +
  theme(text=element_text(face='bold', size=14), axis.text.x=element_blank(), legend.position='bottom', 
        ...
        panel.border = element_rect(color="grey", fill=NA, size=0.5), panel.spacing.x = unit(0.1, 'lines') )


ggplot(subset(df, Pathology == 'BE'), aes(x=chr.length)) + facet_grid(Status~chr, scales='free_x') + ylim(range(df$value)) +
  geom_rect(data=subset(df, Pathology == 'BE'), aes(xmin=start, xmax=end, ymin=0, ymax=value, fill=cn), alpha=0.5, show.legend = T) + 
  #geom_segment(data=subset(df, Pathology %in% c('BE','LGD','HGD')),aes(x=start, xend=end, y=value, yend=value, color=Pathology), size=1.5) +
  #scale_fill_manual(values=c('grey88','green4')) +
  scale_color_manual(values=c('firebrick3','grey88','green4')) +
  scale_fill_manual(values=c('firebrick3','grey88','green4')) +
  #scale_fill_gradient2(high="green4", mid='grey88', low ="firebrick3", name='') +
  labs(title="Segment Values Across BE Samples", x='Chromosomes', y='Mean adjusted segmentation value', subtitle=paste('NonP:Prog - ', paste(table(subset(patient.info, Pathology == 'BE')$Status), collapse=':'))) +
  theme(text=element_text(face='bold', size=14), axis.text.x=element_blank(), legend.position='bottom', 
        panel.grid = element_blank(), panel.background = element_blank(),strip.background =element_rect(fill="lightcyan2"),  
        panel.border = element_rect(color="grey", fill=NA, size=0.5), panel.spacing.x = unit(0.1, 'lines') )


patient_labeller <- function(string, prefix = "Patient") paste(prefix, string)
dfpts = subset(df, Patient %in% c('57','76'))
ggplot(dfpts, aes(x=chr.length)) + ylim(-5,10) +
  facet_grid(Patient~chr, scales='free_x', labeller = labeller(Patient = patient_labeller )) + 
  geom_rect(data=dfpts, aes(xmin=start, xmax=end, ymin=0, ymax=value, fill=cn), show.legend = F) + 
  scale_fill_manual(values=c('firebrick3','grey88','green4')) +
  labs(title="Segment Values Across Samples", x='Chromosomes', y='Mean adjusted segmentation value', subtitle=paste('NonP:Prog - ', paste(table(subset(patient.info, Patient %in% c('57','76'))$Status), collapse=':'))) +
  theme(text=element_text(face='bold', size=14), axis.text.x=element_blank(), legend.position='bottom', 
        panel.grid = element_blank(), panel.background = element_blank(),strip.background =element_rect(fill="lightcyan2"),  
        panel.border = element_rect(color="grey", fill=NA, size=0.5), panel.spacing.x = unit(0.1, 'lines') )

patient_labeller <- function(string, prefix = "Patient") paste(prefix, string)
dfpts = subset(df, Patient %in% c('57','76') & Pathology == 'BE')
ggplot(dfpts, aes(x=chr.length)) + ylim(-5,10) +
  facet_grid(Patient~chr, scales='free_x', labeller = labeller(Patient = patient_labeller )) + 
  geom_rect(data=dfpts, aes(xmin=start, xmax=end, ymin=0, ymax=value, fill=cn), show.legend = F) + 
  scale_fill_manual(values=c('firebrick3','grey88','green4')) +
  labs(title="Segment Values Across BE Samples", x='Chromosomes', y='Mean adjusted segmentation value', subtitle=paste('NonP:Prog - ', paste(table(subset(patient.info, Patient %in% c('57','76') & Pathology == 'BE')$Status), collapse=':'))) +
  theme(text=element_text(face='bold', size=14), axis.text.x=element_blank(), legend.position='bottom', 
        panel.grid = element_blank(), panel.background = element_blank(),strip.background =element_rect(fill="lightcyan2"),  
        panel.border = element_rect(color="grey", fill=NA, size=0.5), panel.spacing.x = unit(0.1, 'lines') )
...
x = endos %>% group_by(Patient, max.path) %>% summarise (  min(Endoscopy.Year)  )

## Skip "indeterminate" as a state?
x[x$max.path == 'ID', 'max.path'] = 'BE'

cols = c('Patient','start.path','end.path','start.year','end.year', 'year.diff')
state.changes = as.data.frame(matrix(ncol=length(cols), nrow=0, dimnames=list(c(), cols )))

x$max.path
for (p in unique(x$Patient)) {
  ep = subset(x, Patient == p)
  if (nrow(ep) <= 1) next
  
  pairs = cbind( c(1:nrow(ep))[-nrow(ep)], c(2:nrow(ep)) )
  df = do.call(rbind, apply(pairs, 1, function(pair) {
    a = ep[ pair[1], ]
    b = ep[ pair[2], ]
    
    cbind.data.frame(a$Patient, a$max.path, b$max.path, a$`min(Endoscopy.Year)`, b$`min(Endoscopy.Year)`, (b$`min(Endoscopy.Year)`-a$`min(Endoscopy.Year)`))
  }))
  colnames(df) = cols  
  state.changes = rbind(state.changes,df)
}
state.changes$start.path = ordered( state.changes$start.path, levels=levels(endos$max.path))
state.changes$end.path = ordered( state.changes$end.path, levels=levels(endos$max.path))


# Sanity check
as.integer(state.changes$end.path) > as.integer(state.changes$start.path)

state.changes = state.changes %>% group_by(Patient) %>% mutate( 'diagnosis'=ifelse(end.year == max(end.year), 1, 0) )

# Sometimes a later sample is at a lower pathology than the earlier one. This state change is due to several factors.  LGD is hard to diagnose accurately so ID would be even less accurate, and heterogenity in the BE segment itself means that one sample could be BE while another sample further down could be LGD and the samples for which we had enough to sequence can mean we miss out on the higher path ones
pander(as.data.frame(state.changes))
...
bl = subset(state.changes, start.path == 'BE' & end.path == 'LGD')

bh = subset(state.changes, start.path == 'BE' & end.path %in% c('IMC','HGD'))
summary(bh$year.diff)

lh = subset(state.changes, start.path == 'LGD' & end.path %in% c('IMC','HGD'))
summary(lh$year.diff)
...
```{r cox, warning=F, message=F, echo=T}
library(survival)
...
prog.pred = lapply(pg.samp, function(df) {
  print(df$Hospital.Research.ID[1])
...
p3 = ggplot(m, aes(Status, value, fill=Status)) + 
  geom_jitter(aes(color=Status), width=0.2, show.legend = F) +  geom_violin(alpha=0.8) +
...
#print.png(p3, 'validation_predictions.png', width=600, height=800)
