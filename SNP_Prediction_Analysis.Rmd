...
```{r rnormals, echo=F, fig.width=8, fig.height=8, eval=T}
...
sa = subtract.arms(adjust.cols(mergedSegs[sub('\\.LogR','',randBE),,drop=F], z.mean, z.sd),
              adjust.cols(mergedArms[sub('\\.LogR','',randBE),,drop=F], z.arms.mean, z.arms.sd))[,1:589]
ggplot(melt(as.matrix(sa)), aes(Var1, value)) + geom_point()
...
grid.arrange(
  ...
  top=randBE)
# So what if I don't subtract the arms?  
...
# grid.arrange(
#     ggplot(melt(arrayDf), aes(Var2, value)) + geom_point() + geom_point(data=melt(arrayDf[,589:634]+abs(mean(arrayDf[,589:634])*2)), col='red') + labs(title='SNP'),
#     ggplot(melt(allDf), aes(Var2, value)) + geom_point() +
#       geom_point(data=melt(allDf[,589:634]), col='red') + labs(title='sWGS'),
#   nrow=1)
...
## TEMP: I accidentally included normals by failing to account for difference in name
#preds = preds[-grep('_\\d+$', rownames(preds)),]
