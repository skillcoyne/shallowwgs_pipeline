...
grid.arrange(
  ggplot(melt(allDf[,1,drop=F]), aes(sample=value)) + stat_qq(),
  ggplot(melt(arrayDf[,1,drop=F]), aes(sample=value)) + stat_qq(),
  ncol=1)

for (i in 1:ncol(arrayDf)) {
  arrayDf[,i] = unit.var(arrayDf[,i], mean(allDf[,i]), sd(allDf[,i]))
}
...
pnr = grep('BLD|gastric', rownames(preds), ignore.case = T)
...
## First of all we have samples from patients that probably had a genome duplication, there's no way to measure that in our sWGS data
#subset(pred.qc, `mean(Ploidy)` > 2.7)
