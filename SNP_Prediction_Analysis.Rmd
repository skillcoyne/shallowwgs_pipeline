...
ggplot(subset(qcdata, SampleType == 'BE'), aes(round(Ploidy,2), Purity, color=Status)) + geom_point() + facet_grid(~Status) + 
  labs(title='BE only, purity vs ploidy')

ggplot(qcdata, aes(SCA.Ratio, color=Status, fill=Status)) + geom_density(alpha=0.5) + labs(x='SCA Ratio', title='SCA Ratio, NP vs P')
...
#copySegs = as.data.frame(normalize.quantiles.use.target(mergedSegs, as.vector(raw.segs)))
grid.arrange(
...
pred.qc = pred.qc %>% group_by(Status, Samplename) %>% dplyr::mutate(
  lowsca = (Status == 'P' & (`mean(SCA.Ratio)` < 0.03 | `mean(Ploidy)` < 1.5))
)

subset(pred.qc, `mean(Ploidy)` < 1.99)

subset(pred.qc, lowsca)
...
roc.plot(rocpp) + labs(title='Both endos - excluded low SCA P')
pROC::coords(rocpp, 'best')


# Exclude the tetraploid NP
