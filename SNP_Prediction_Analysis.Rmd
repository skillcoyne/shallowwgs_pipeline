...
# These samples are likely genome duplicated.
dup = subset(pred.qc, `mean(Ploidy)` > 2.7)
cor.test(dup$`mean(Ploidy)`, dup$Prob)
table(dup$Status)
...
table(subset(pred.qc, Risk == 'High' & !exclude)$Status)/table(subset(pred.qc, !exclude)$Status)
table(subset(pred.qc, Risk == 'Moderate' & !exclude)$Status)/table(subset(pred.qc, !exclude)$Status)
table(subset(pred.qc, Risk == 'Low' & !exclude)$Status)/table(subset(pred.qc, !exclude)$Status)
...
rocpp = pROC::roc(Status ~ Prob, data=subset(pred.qc, !exclude), auc=T, ci=T, of='thresholds')
roc.plot(rocpp) + labs(title='Max per patient (excluded low sca/NP tetra), Prob')
pROC::coords(rocpp, 'best')

as.data.frame(subset(pred.qc, Status == 'P' & Prob < 0.5 & !exclude))
...
per_pt_ex = subset(pred.qc, !exclude) %>% group_by(PatientID, Status) %>% dplyr::summarise(
...
subset(pred.qc, `mean(SCA.Ratio)` < 0.1 & Status == 'P' & !exclude & Prob < 0.5)

ggplot(subset(pred.qc, !exclude | ( `mean(SCA.Ratio)` > 0.1 & Status == 'P')) , aes(`mean(Purity)`,Prob, color=Risk, shape=Status)) + facet_grid(~Status) + geom_point()
...
```{r regression, echo=F, message=F}
slabels = as.data.frame(subset(pred.qc, !exclude, select=c('PatientID','Samplename','Status')))
#slabels = as.data.frame(pred.qc, select=c('PatientID','Samplename','Status'))
rownames(slabels) = slabels$Samplename

df = arrayDf[which(rownames(arrayDf) %in% slabels$Samplename),]
...
if (file.exists(file)) {
 ...
 load(file, verbose=T)
  ...
  save(plots, coefs, performance.at.1se, df, models, cvs, labels, file=file)
...
if (file.exists(file)) {
  ...
  for (pt in unique(slabels$PatientID)) {
    print(pt)
    samples = subset(slabels, PatientID != pt)$Samplename
