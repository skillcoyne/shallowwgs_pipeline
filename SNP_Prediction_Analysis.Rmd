...
subset(sample.list, PatientID == 74)
...
ggplot(sample.info, aes(SCA.Ratio)) + geom_histogram() + facet_grid(~Status)
...
ggplot(qcdata, aes(SCA.Ratio)) + geom_histogram() + 
  facet_wrap(~Status, scales='free_y', ncol=1) + labs(x='SCA Ratio', title='SCA Ratio, normal vs BE')
...
```{r}
## 43% of P samples have low/no CN with a ploidy of 2 or less
nrow(subset(qcdata, Status == 'P' & SampleType == 'BE' & SCA.Ratio <= 0.05 & Ploidy <= 2))/nrow(subset(qcdata, Status == 'P' & SampleType == 'BE'))

## <1% of P samples have no apparent CN
nrow(subset(qcdata, Status == 'P' & SCA.Ratio <= 0 & SampleType == 'BE'))/nrow(subset(qcdata, Status == 'P' & SampleType == 'BE'))

## 36% of P samples have < 5% CN and high purity ()
nrow(subset(qcdata, Status == 'P' &  Purity >= 0.95 & SCA.Ratio <= 0.05 & SampleType == 'BE'))/nrow(subset(qcdata, Status == 'P' & SampleType == 'BE'))
...
hist(round(lowscaSamples/totalSamples[names(lowscaSamples)], 2))
...
```{r normals, fig.width=6, fig.height=8}
...
```{r rnormals, echo=F, fig.width=8, fig.height=8}
randNormal = sample(grep('BLD|gastric', rownames(mergedSegs), ignore.case=T, value=T), 1)
...
grid.arrange(
  ggplot(subset(segments.list[[pt]], sample == paste(randNormal,'.LogR',sep='')), aes(y=medLRR)) + 
    facet_grid(~chr, scales='free_x',space='free_x') +
    geom_segment(aes(x=startpos, xend=endpos, y=medLRR, yend=medLRR), size=2) + ylim(-1,1) +
    theme(axis.text.x=element_blank(), panel.spacing.x=unit(0, 'lines')) +
    labs(title='Median LRR, ASCAT segments', subtitle=pt), 
  ggplot(subset(nm, Var1 == randNormal), aes(Var2, value)) + geom_point() + ylim(-1,1) + 
...
grid.arrange(
  ggplot(subset(segments.list[[pt]], sample == randBE), aes(y=medLRR)) + 
    facet_grid(~chr, scales='free_x',space='free_x') +
    geom_segment(aes(x=startpos, xend=endpos, y=medLRR, yend=medLRR), size=2) + ylim(1,-1) +
    theme(axis.text.x=element_blank(), panel.spacing.x=unit(0, 'lines')) +
    labs(title='Median LRR, ASCAT segments', subtitle=pt), 
  ggplot(subset(be, Var1 == sub('\\.LogR','',randBE)), aes(Var2, value)) + geom_point() + ylim(-1,1) + 
    theme(axis.text.x=element_blank()) + labs(title='Winsorized LRR, binned'),  
...
The variance gets much bigger after unit normalizing, but the difference between BE and normal is still clear.
...
bld = preds[grep('BLD', preds$Samplename, ignore.case=T),]
grid.arrange(
  ggplot(bld, aes(Prob)) + geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1) , show.legend = F) +
  scale_fill_gradientn(colors = myPal,  name='') + 
  labs(y='n Samples', x='Unadjusted Probability'),
ggplot(bld, aes(`Adj. Prob`)) + geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1) , show.legend = F) +
  scale_fill_gradientn(colors = myPal,  name='') + 
  labs(y='n Samples', x='Adjusted Probability'), top='Predictions on BLD samples only' )
...
grid.arrange(
  ...
  ggplot(be, aes(`Adj. Prob`)) + geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1) , show.legend = F) +
    scale_fill_gradientn(colors = myPal,  name='') + 
    labs(y='n Samples', x='Adjusted Probability'), top='Predictions on BE only'
)
...
Unajdusted `r round(unadj,2)*100`% of BE patients have a >50% probability of progression.  Adjusted, `r round(adj, 2)*100`% have a >50% probability.
...
rocNoLow = pROC::roc(Status ~ Prob, data=subset(pred.qc, !Samplename %in% lowsca$Samplename), auc=T, ci=T, of='thresholds')
roc.plot(rocNoLow)
...
roct1 = pROC::roc(Status ~ Prob, data=subset(pred.qc, Timepoint == 'T1'), auc=T, ci=T, of='thresholds')
roc.plot(roct1)

roct2 = pROC::roc(Status ~ Prob, data=subset(pred.qc, Timepoint != 'T1'), auc=T, ci=T, of='thresholds')
roc.plot(roct2)

qcpred = subset(pred.qc, Samplename %in% subset(qcdata, Purity > 0.8 & Goodnessoffit >= 0.9)$Samplename)
qc = pROC::roc(Status ~ Prob, data=qcpred, auc=T, ci=T, of='thresholds')
roc.plot(qc)
...
sample.info = patient.info[c('PatientID','Status')]
sample.info = base::merge(sampleInfo, sample.info, by='PatientID', all.x=T)

sample.info = subset(sample.info, !Level %in% c('BLD','GASTRIC'))
sample.info$Samplename = with(sample.info, paste(PatientID, SampleID, EndoscopyID, Level, sep='_'))

df = arrayDf
arrayDf = arrayDf[grep('BLD|GASTRIC',rownames(df), invert=T),]
rownames(arrayDf) = sub('\\.LogR','',rownames(arrayDf))

patient.info
status = vector(length=nrow(arrayDf), mode='character')
names(status) = rownames(arrayDf)

for (i in 1:length(status))
  status[i] = unique(subset(patient.info, PatientID == sub('_.*','',names(status)[i]))[,'Status'])

labels = as.integer(factor(labels))-1
names(labels) = names(status)
rm(status)
...
sets = create.patient.sets(sample.info[c('PatientID','Samplename','Status')], folds, splits, 0.2)  
...
if (file.exists(file)) {
  ...
  for (a in alpha.values) {
    fit0 <- glmnet(arrayDf, labels, alpha=a, nlambda=nl, family='binomial', standardize=F)    
    l = fit0$lambda

    cv.patient = crossvalidate.by.patient(x=arrayDf, y=labels, lambda=l, pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', opt=-1, standardize=F)
