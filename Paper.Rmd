...
```{r echo=F}
cohortSummary = as.data.frame(all.info %>% dplyr::group_by(Status) %>% summarise(
  'Total Patients'=length(unique(Patient)),
  'Total Samples'=sum(total.samples),
...
noHGD.pred.confidence = cbind(noHGD.pred.confidence, pg.sampNOHGD %>% dplyr::group_by(quants) %>% summarise ( 'mn'=mean(Prediction), 'sd'=sd(Prediction) ))
...
### Per Endoscopy ROCs
```{r, echo=F, warning=F, message=F,fig.height=4, fig.width=4}
pendo = do.call(rbind, pg.samp) %>% dplyr::group_by(Patient, Status, Hospital.Research.ID, PID ) %>% dplyr::summarise(
...
#ptendo = do.call(rbind, pg.samp) %>% dplyr::group_by(Patient, Hospital.Research.ID, Status) %>% summarise('max'=max(Prediction), 'avg'=mean(Prediction), 'n'=length(Patient))
...
transform.by.time<-function(pt, info) {
  #print(unique(pt$Hospital.Research.ID))
  pt = pt[,c('Status','Patient','Hospital.Research.ID','Path.ID','PID','Pathology','Samplename','Final.Endoscopy','Endoscopy.Year','Prediction', 'OR', 'Sample.Mean','Sample.SD','Sample.Var')] %>% dplyr::group_by(PID, Samplename) %>% dplyr::mutate(
...
qt = cbind(qt, back %>% dplyr::group_by(quants) %>% dplyr::summarise ( 'mn'=mean(Prediction), 'sd'=sd(Prediction) ))#, 'se'=(sd(Prediction)/sqrt(length(Prediction)))*1.96 ))

pred.confidence = qt %>% as.data.frame.matrix %>% dplyr::group_by(quant) %>% dplyr::mutate( 
...
annotation.table = arrange(pred.confidence %>% dplyr::group_by(Risk) %>% dplyr::summarise(
  'n(NP)'=sum(NP), 'n(P)'=sum(P), 'p.value'=format.pval(mean(p.value), 3), 
  'min'=min(r1), 'max'=max(r2)), min
)
...
back = back %>% dplyr::group_by(Risk) %>% dplyr::mutate('se(Risk)'=(sd(Prediction)/sqrt(length(Risk)))*1.96 )
...
pathTotal = back %>% dplyr::group_by(Status, Pathology) %>% dplyr::summarise( 'nPath'=length(Pathology))
...
p53Path = merge( subset(back, Status == 'P' & !is.na(p53.Status)) %>% dplyr::group_by(Pathology) %>% dplyr::summarise( 'nPath'=length(Pathology)), p53Path)
...
endos.m = subset(endos, Status == 'P') %>% dplyr::group_by(brks) %>% dplyr::summarise( 
...
tmp$brks = cut(tmp$months.before.final, include.lowest = T, ordered_result = T,breaks=sq,
     labels=c('Endpoint','1-12','12-24','24-48','48-72','72-96','96-120','>120'))
tmp = tmp %>% dplyr::group_by(brks) %>% dplyr::summarise( 'mean(P)'=mean(Prediction),'sem(P)'=sd(Prediction)/sqrt(length(brks)) )
...
# Average cn value
valCNdf = valCNdf %>% dplyr::group_by(Status, start, end) %>% dplyr::mutate( 'mean.value' = value/length(unique(variable)) )
...
m = melt(back.v %>% dplyr::group_by(Patient, Status) %>% dplyr::summarise( 'Low'=length(which(Risk == 'Low'))/length(Patient),
...
qt = cbind(qt, back.v %>% dplyr::group_by(quants) %>% dplyr::summarise ( 'mn'=mean(Prediction), 'sd'=sd(Prediction) )) 

pred.confidence.v = qt %>% as.data.frame.matrix %>% dplyr::group_by(quant) %>% dplyr::mutate( 'perc'=P/sum(NP,P) )
...
dirA = '~/Data/Ellie/Analysis/downsampled'
ds_info = read_excel('~/Data/Ellie/Analysis/downsampled_ids.xlsx')
predA = list.files(dirA, 'predictions.Rdata', full.names = T)
load(predA)

dspred = merge(dspred, ds_info, by.x='row.names', by.y='Illumina ID')

ggplot(dspred, aes(Type, RR, group=Type, fill=Type)) + geom_boxplot(outlier.colour = NA) + geom_jitter(width=0.2, aes(color=Status)) +
  labs(x='', y='Relative Risk', title='Predictions on downsampled patients') + 
  plot.theme 

adj = subset(dspred, grepl('Adjacent',Type))
