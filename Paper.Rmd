...
roc.plot(rocNOHGD, 'Model excluding HGD/IMC',incS = T) 

pROC::coords(rocNOHGD, 'best', transpose=F) %>% add_column('AUC'=pROC::auc(rocNOHGD)) %>% mutate_all(list(~round(.,2))) %>% 
  kable(caption = 'LOO excluding HGD/IMC') %>% kable_styling(full_width = F)

ggsave(filename='plots/auc/roc_nohgd_model.png', plot=roc.plot(rocNOHGD, 'Model excluding HGD/IMC',incS = T) , width=6, height=6, units='in', dpi=300)
```
...
roc.plot(rocNOLGD, 'Model excluding LGD/HGD/IMC',incS = T) 

pROC::coords(rocNOLGD, 'best', transpose=F) %>% add_column('AUC'=pROC::auc(rocNOLGD)) %>% mutate_all(list(~round(.,2))) %>% 
  kable(caption = 'LOO excluding LGD/HGD/IMC') %>% kable_styling(full_width = F)

ggsave(filename='plots/auc/roc_nolgd_model.png', plot=roc.plot(rocNOLGD, 'Model excluding LGD/HGD/IMC',incS = T) , width=6, height=6, units='in', dpi=300)
```
...
p = ggplot(m, aes(model, value, ymin=CI.min, ymax=CI.max, group=Measure)) + 
  ...
  labs(x='', y='ROC', title='Model comparisons') 

auc.leg = get.legend(p)

ggsave('plots/auc/loo_model_roc.png', plot=p + theme(legend.position = 'none'), height=6, width=4.5, units="in", limitsize=F, dpi = 300)
...
ggsave('plots/auc/avgendo_auc.png', plot=roc.plot(rocEndoA, title='Mean per Endoscopy'), width=6, height=6, units="in", limitsize=F, dpi = 300)
...
multi.roc.plot(list(rocEndoM, rocEndoA, rocLastEndo, rocFirstEndo, rocRandEndo)) + labs(title='Endoscopy prediction ROCs')
```
...
ggsave('plots/auc/avgpt_auc.png', plot=roc.plot(rocPtA, title='Mean per Patient (excl. final)'), width=6, height=6, units="in", limitsize=F, dpi = 300)

grid.arrange(roc.plot(rocPtM, title='Max') , roc.plot(rocPtA, title='Mean'), nrow=1, top='Per Patient')

multi.roc.plot(list(rocPtM, rocPtA)) + labs(title='Per-Patient ROCs')
...
p = ggplot(m, aes(model, value, ymin=CI.min, ymax=CI.max, group=Measure)) + 
  ...
  labs(x='', y='AUC', title='Aggregated ROC Comparison') 
