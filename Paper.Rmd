...
## Dysplasia only
dys.status <- c('Progressor'=paste('Progressor (n=',length(unique(subset(globalCNdf, Status == 'Progressor' & Pathology %in% c('LGD','HGD','IMC'))$variable)), ')', sep=''),
              'Non-Progressor'=paste('Non-Progressor (n=',length(unique(subset(globalCNdf, Status == 'Non-Progressor' & Pathology %in% c('LGD','HGD','IMC'))$variable)),')', sep=''))

globalnHGDCN.plot = cn.mtn.plot(subset(globalCNdf, seg.type != 'arms' & Pathology %in% c('LGD','HGD','IMC')), labeller(Status=dys.status, chr=label_value)) + labs(title='LGD/HGD/IMC Only')
...
# `%nin%` = Negate(`%in%`)
# 
# nhgd.status <- c('Progressor'=paste('Progressor (n=',length(unique(subset(globalCNdf, Status == 'Progressor' & Pathology %nin% c('HGD','IMC'))$variable)), ')', sep=''),
#               'Non-Progressor'=paste('Non-Progressor (n=',length(unique(subset(globalCNdf, Status == 'Non-Progressor' & Pathology %nin% c('HGD','IMC'))$variable)),')', sep=''))
# globalnHGDCN.plot = cn.mtn.plot(subset(globalCNdf, seg.type != 'arms' & Pathology %nin% c('HGD','IMC')), labeller(Status=nhgd.status, chr=label_value)) + labs(title='Excluding HGD/IMC')
...
globalnHGDCN.plot + theme(legend.position = 'none')

globalBECN.plot + theme(legend.position = 'none')
...
# 
# genomePtCN.plot + theme(legend.position = 'none')
# genomePtBECN.plot + theme(legend.position = 'none')

#cn.mtn.plot(dfpts,  labeller(Status = ptbe.status ), type='bar') + labs(title='NDBE samples only')

#a = sample( subset(subset(patient.info, Patient == 57),  Endoscopy.Year == min(Endoscopy.Year) )$Samplename, 1)
#b = sample( subset(subset(patient.info, Patient == 76), Endoscopy.Year == min(Endoscopy.Year) & Pathology %nin% c('HGD','IMC'))$Samplename, 1)

#ind.status <- c('Non-Progressor'='Patient 57 (n=1)','Progressor'='Patient 76 (n=1), excl. HGD/IMC')
#cn.mtn.plot(subset(globalCNdf, variable %in% c(a,b) & seg.type != 'arms'), labeller(Status = ind.status), type='bar') + labs(title="Individual samples")
```
...
# cx was generated on raw seg values
plot(dysplasia.df[names(labels) ,'cx'], col=labels+1)

# remove old cx...see what new one would look like
dysplasia.df = dysplasia.df[,-grep('cx',colnames(dysplasia.df))] 

# general variance per sample after the samples have been mean normalized, ignores segment length
cx = apply(dysplasia.df, 1, function(x) var(x))
...
ggplot(x, aes(Status, cx, fill=Status)) + 
  geom_jitter(width=0.2, color='grey39') + 
  geom_violin(outlier.colour = NA, alpha=0.8) +
  labs(x='', y='Variance', title='Per sample variance', subtitle=paste('p-value=', format.pval(tt$p.value, digits=2))) + theme(legend.position = 'none') +
  plot.theme

rocX = pROC::roc(Status ~ cx, data=x, auc=T, ci=T, of='thresholds')
roc.plot(rocX, title='Per sample variance') + plot.theme
...
mp = model.performance(performance.at.1se, coefs, lims=c(0.7,0.8))
...
mp = model.performance(performance.at.1se, coefs, lims=c(0.69,0.8))
mp$plot + labs(title='No HGD/IMC samples')
...
```{r, echo=F, message=F, fig.width=5, fig.height=5}
  ...
  ggplot(melt(perf), aes(x=variable,y=value, group=variable, fill=variable)) +
...
```{r, echo=F, fig.width=6, fig.height=6}
mp = model.performance(performance.at.1se, coefs, lims=c(0.69,0.8))
mp$plot + labs(title='No LGD/HGD/IMC samples')
...
globalCN.plot + geom_rect(data=subset(chh, gl == 'gain'), aes(xmin=start, xmax=end, ymin=0, ymax=max), color='goldenrod2', alpha=0, show.legend=F) + 
  geom_rect(data=subset(chh, gl == 'loss'), aes(xmin=start, xmax=end, ymin=0, ymax=max), color='blue3', alpha=0, show.legend=F)
...
# m = melt(insilico.rocs, measure.vars = 'auc')
# ggplot(m, aes(variable,value)) + ylim(0,1) +
#   geom_boxplot(fill='mintcream', color='grey55') + geom_jitter(width=0.2, color='darkblue', size=1.5, show.legend = F) + 
#   geom_text(data=subset(m, value %in% range(value)), aes(label=round(value,2)), vjust=0.02, hjust=0.2) +
#   plot.theme + labs(x='',y='AUC',title='AUC', subtitle='20 insilico 20:69 splits')
...
multi.roc.plot(list(roc), colors=brewer.pal(3,'Greys')[3]) + labs(title='Discovery cohort ROC')


insilico.rocs = rbind(insilico.rocs, c(pROC::ci.auc(roc), pROC::coords(roc, 'best')))

insilico.rocs[1:20,'split'] = paste('in-silico', 1:20)
insilico.rocs[21,'split'] = 'discovery'

ggplot(insilico.rocs, aes(split, auc)) + geom_point() + ylim(0,1) +
  geom_errorbar(aes(ymin=auc.ci.min, ymax=auc.ci.max), width=0.2) + 
  coord_flip() + plot.theme + labs(y='AUC', x='', title='AUC per split', subtitle='20 insilico 20:69 splits')



# To find the auc at a given spec or sens
#pROC::auc(roc, partial.auc=c(0.99,1), partial.acu.focus="sp", partial.auc.correct=T)
...
rocPtA = pROC::roc(Status ~ avg, data=ptendo, ci=T, of='thresholds')
...
ggplot(melt(subset(m, Measure == 'AUC'), measure.vars='value'), aes(x=model, y=value*100, ymin=CI.min*100, ymax=CI.max*100, group=Measure, fill=Measure)) + 
  geom_bar(stat='identity',position=dodge) + geom_errorbar(position=dodge, width=0.3, color='black' ) + 
  geom_text(aes(label=paste(round(value,2)*100,'%',sep=''), y=1, vjust=-1 ), position=dodge, color='white')  +
...
ggplot( arrange(mm, CI.min), aes(x=model, y=value*100, ymin=CI.min*100, ymax=CI.max*100, group=Measure, fill=model)) + 
  geom_bar(stat='identity',position=dodge) + geom_errorbar(position=dodge, width=0.3, color='black' ) + 
  geom_text(aes(label=paste(round(value,2)*100,'%',sep=''), y=1), position=dodge, color='white')  +
  scale_fill_manual(values=cols, name='') + plot.theme + 
  theme(legend.position = 'none', axis.text.x = element_text(angle=45, hjust=1)) + labs(x='', y='AUC', title='ROC Comparison') 
...
transform.by.time<-function(pt, info) {
  pt = pt[,c('Status','Patient','Hospital.Research.ID','Path.ID','PID','Pathology','Samplename','Final.Endoscopy','Endoscopy.Year','Prediction', 'OR')] %>% group_by(PID, Samplename) %>% mutate(
...
```{r probhist, echo=F, warning=F, message=F, fig.width=8, fig.height=6}
...
```{r indP, echo=F, warning=F, message=F, fig.width=8, fig.height=6}
...
ggplot(back, aes(OR)) + geom_histogram(aes(fill=..x..), bins=10, show.legend = F) +
...
ggplot(back, aes(Prediction)) + geom_histogram(aes(fill=..x..), breaks=cuts, show.legend = F) +
    scale_fill_gradientn(colors = myPal,  name='') + 
  #scale_fill_distiller(palette = 'RdYlBu', type='div',name='P(P)') +
    plot.theme + labs(title='Predictions, all samples model', y='n Samples', x='Probability') 

ggplot(back, aes(OR, Prediction, color=Prediction)) + geom_point(show.legend = F) + 
...
min.theme = theme(text=element_text(size=10), panel.background=element_blank(), strip.background=element_rect(fill="grey97"), 
                   strip.text = element_text(size=10), axis.text = element_blank(),
...
p1 = mtn.spatial(subset(globalCNdf, Patient == 48 & seg.type != 'arms'), back, ogjLimits, 'Patient 48 (P)')
p2 = mtn.spatial(subset(globalCNdf, Patient == 20 & seg.type != 'arms'), back, ogjLimits,'Patient 20 (P)' )
p3 = mtn.spatial(subset(globalCNdf, Patient == 60 & seg.type != 'arms'), back, ogjLimits,'Patient 60 (P)' )

p4 = mtn.spatial(subset(globalCNdf, Patient == 3 & seg.type != 'arms'), back,ogjLimits,'Patient 3 (NP)' )
p5 = mtn.spatial(subset(globalCNdf, Patient == 15 & seg.type != 'arms'), back,ogjLimits,'Patient 15 (NP)' )
...
col = brewer.pal(11, "PRGn")[c(9,3)]
ggplot(p53Path, aes(Pathology, ratio*100, group=Pathology, fill=p53.Status)) + geom_bar(stat='identity', position = position_stack()) +
...
p = pred.tiles(transform(subset(back, Status == 'NP'), Patient=reorder(Patient, pred.ratio)), cols=myPal, probs=T, nrow=2) + labs(title='Non-Progressors') 
riskLegend = get.legend(p)
p = p + theme(legend.position = 'none')
ggsave('npg_heatmap.png', plot = p, scale = 1, width = 52, height = 8, units = "in", limitsize = F, dpi = 300)
 
p = pred.tiles(transform(subset(back, Status == 'P'), Patient=reorder(Patient, -pred.ratio)), probs=T, cols=myPal,nrow=2) + labs(title='Progressors') + theme(legend.position = 'none')
ggsave('pg_heatmap.png', plot = p, scale = 1, width = 52, height = 8, units = "in", limitsize = F, dpi = 300)

ggsave('risk_legend.png', plot=grid.arrange(riskLegend), scale=1, dpi=300)
...
plot.cn.chr(globalCNdf, chrom='17', chr.lengths, genes=genes, haz=chh) + labs(
title="Segment Values Samples", subtitle=paste('NonP:Prog - ', paste(table(patient.info$Status), collapse=':'))) + theme(legend.position = 'none')
```
...
for (chr in unique(chh$chr)) 
  print(plot.cn.chr(subset(globalCNdf, seg.type != 'arms') , chrom=chr, chr.lengths, genes=genes, haz=chh))
...
globalCN.plot + #geom_rect(data=annoG, aes(xmin=start, xmax=end, ymin=ymin,ymax=ymax), color='red') +
  geom_point(data=annoG, aes(x=start, y=0), size=1.5, color='red') +
  geom_text_repel(data=annoG, aes(x=start, y=ymax/2, label=Gene.Symbol), color='red', size=3 )
...
```{r echo=F, warning=F, message=F, fig.height=5,fig.width=25}
pred.tiles(subset(back.v, Status == 'P'), cols=myPal, probs=T, nrow=1) + labs(title='Validation Progressors') + theme(legend.position = 'none')
pred.tiles(subset(back.v, Status == 'NP'), cols=myPal, probs=T, nrow=1) + labs(title='Validation Non-Progressors') + theme(legend.position = 'none')
```
...
ggplot(riskPath, aes(Pathology, ratio*100)) + geom_bar(aes(group=Risk, fill=Risk),stat='identity', position=position_stack()) +   scale_fill_manual(values=riskCols) + scale_color_manual(values=c('white','black','white')) + 
...
roc$model = 'Discovery'
multi.roc.plot(list(roc, vroc), title='ROC', palette='Dark2') 
```
...
vigLeg = get.legend(p)
p + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 56), subset(patient.info, Patient == 56),cols=riskCols) + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 20), subset(patient.info, Patient == 20),cols=riskCols) + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 86), subset(patient.info, Patient == 86),cols=riskCols) + theme(legend.position = 'none')


## NP
vig.plot(subset(back, Patient == 3), subset(patient.info, Patient == 3),cols=riskCols) + theme(legend.position = 'none')
vig.plot(df=subset(back, Patient == 23),info= subset(patient.info, Patient == 23),cols=riskCols) + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 33), subset(patient.info, Patient == 33),cols=riskCols) + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 49), subset(patient.info, Patient == 49),cols=riskCols) + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 51), subset(patient.info, Patient == 51),cols=riskCols) + theme(legend.position = 'none')
           
vig.plot(subset(back, Patient == 6), subset(patient.info, Patient == 6),cols=riskCols) + theme(legend.position = 'none')
vig.plot(subset(back, Patient == 54), subset(patient.info, Patient == 54),cols=riskCols) + theme(legend.position = 'none')
```

```{r, echo=F, warning=F, message=F, fig.width=4, fig.height=1}
grid.arrange(vigLeg)
```
...
cols = unique(colnames(back), colnames(back.v))
...
ggplot(rrPath, aes(Status, mean.RR)) + #scale_color_gradientn(colors=myPal, limits=c(min(rrPath[,3:5]), max(rrPath[,3:5])),  name='') +
...
ggplot(melt(loopreds[c('Min','Max')]), aes(value)) + facet_wrap(~variable, ncol=2) +
