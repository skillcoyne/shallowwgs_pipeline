...
genomePtCN.plot = cn.mtn.plot(dfpts, labeller(Patient = pt.status))
...
genomePtBECN.plot = cn.mtn.plot(dfpts,  labeller(Patient = ptbe.status )) + labs(title='NDBE samples only')

genomePtCN.plot 
genomePtBECN.plot

cn.mtn.plot(dfpts,  labeller(Patient = ptbe.status ), type='bar') + labs(title='NDBE samples only')
```

## Individual samples
```{r}

a = sample(subset(patient.info, Status == 'NP' & Pathology %in% c('BE'))$Samplename, 1)
b = sample(subset(patient.info, Status == 'P' & Pathology %in% c('BE'))$Samplename, 1)


cn.mtn.plot(subset(globalCNdf, variable %in% c(a) & seg.type != 'arms'), labeller(label_both)) + labs(title="Individual NDBE samples") + theme(strip.text = element_text(size=10, face='bold'))
cn.mtn.plot(subset(globalCNdf, variable %in% c(b) & seg.type != 'arms'), labeller(label_both)) + labs(title="Individual NDBE samples") + theme(strip.text = element_text(size=10, face='bold'))



cn.mtn.plot(subset(globalCNdf, variable %in% c(a,b) & seg.type != 'arms'), labeller(label_both)) + labs(title="Individual NDBE samples")
cn.mtn.plot(subset(globalCNdf, variable %in% c(a,b) & seg.type != 'arms'), labeller(label_both), type='bar') + labs(title="Individual NDBE samples")
...
m = rbind.data.frame(get.roc.stat(p53roc),
...
ggplot(melt(m,id.vars = 'model'), aes(model, value*100, group=variable, fill=variable)) + ylim(0,100) +
...
m = endos %>% group_by(brks, labels) %>% summarise( 'pred.r'= length(which(pred > 0))/length(brks), 'n'=length((Patient)), 'n.samp'=sum(samples) )
...
ggplot(m, aes(brks, pred.r*100, group=brks)) + ylim(0,105) +
  ...
  geom_text(aes(y=pred.r*100-2, label=paste('n=',n,'\ns=',n.samp,sep='')), color='white') + 
...
transform.by.time<-function(pt) {
  ...
  pt = merge(pt, subset(patient.info, Hospital.Research.ID == pt$Hospital.Research.ID[1], select=c('Samplename','Block')), by='Samplename')
...
At each quintile are the percentage of progressor samples that were predicted within that range, and the hypergeometric p-value (Fishers) for the significance within that quintile. Then our predictions are at best a coin flip in the middle, but highly significant at the outer edges.
...
pred.tiles <- function(df) {
  ggplot(df, aes(brks, nl, fill=Prediction)) + 
      geom_tile() + scale_fill_gradient2(high='red3',low='green',mid='orange', limits=c(0,1), midpoint=cutoff, name='P(P)') +
      geom_text(aes(label=labels, color=insig), show.legend=F) + scale_color_manual(values=c('grey','white')) +
...
insig = subset(pred.confidence, p.value > 0.05, select=c('quant','r1','r2'))

back$insig = back$quants != insig$quant
...
plot.cn.chr<-function(df, chrom='17', info=NULL, haz=NULL, genes=NULL) {
    ...
    p = ggplot(df, aes(x=chr.length)) + facet_grid(Status~chr, scales='free_x', labeller=labeller(chr=label_both)) +
        ...
        scale_color_manual(values=cn.colors) + scale_fill_manual(values=cn.fill) 
...
clin = lapply(vpd, function(v) {
  v = arrange(v, Endoscopy.Year)
  tb = table(v$Endoscopy.Year, v$PID)

  first = subset(v, Endoscopy.Year == rownames(tb)[1] & PID == names(which(tb[1,] > 0))[1])
  if (nrow(tb) > 2) {
    penult = subset(v, Endoscopy.Year == rownames(tb)[(nrow(tb)-1)] & PID == names(which(tb[(nrow(tb)-1),] > 0))[1])
  } else if (nrow(tb) == 2) {
    penult = subset(v, Endoscopy.Year == rownames(tb)[(nrow(tb))] & PID == names(which(tb[(nrow(tb)),] > 0))[1])
  } else {
    penult = first
  } 
  cols = c('Hospital.Research.ID','Patient','Status','Path.ID','PID','Endoscopy.Year','Prediction','Samplename')
  fp = unique(rbind.data.frame(first[cols],penult[cols]))
  
  fp$conf = cut(fp$Prediction, breaks=cuts, labels=cuts[-1])
  fp = merge(fp, pred.confidence, by.x='conf', by.y='quant')
  fp$conf = round(fp$perc, 2)
  fp$perc = NULL
  fp$Path.ID = factor(fp$Path.ID)
  
  fp = merge(fp, validation.patient.info[c('Samplename','Block')])
  fp$nl = factor( (fp$Block-1)*2, ordered = T)
  fp$Endoscopy.Year = factor(fp$Endoscopy.Year, ordered = T)
  
  fp
})

df = do.call(rbind.data.frame,clin)
df$conf = factor(df$conf)

ggplot(df, aes(Endoscopy.Year, Prediction, fill=Prediction, color=Prediction)) + 
  geom_point(aes(size=conf)) + facet_grid(~Patient,  scales='free_x', labeller=label_both) +
  #scale_size_continuous(breaks=cuts,  labels=cuts) + 
  scale_fill_gradient2(high='red3',low='green',mid='orange', limits=c(0,1), midpoint=cutoff, name='P(P)') +
scale_color_gradient2(high='red3',low='green',mid='orange', limits=c(0,1), midpoint=cutoff, name='P(P)') +
...
```{r clonality, echo=F, message=F, warning=F}

## Variance in in CN per endoscopy
pt = 'AH0254'
subset(sum.patient.data, Status == 'P')

subset(sum.patient.data, Patient == 34)
