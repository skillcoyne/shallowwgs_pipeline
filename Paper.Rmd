...
cache.dir = paste(data, 'Analysis/5e6_arms_all', sep='/') # QC'd
...
lastEndo = pg.samp %>% group_by(Patient, Hospital.Research.ID, Status,Endoscopy.Year, Path.ID) %>% arrange(desc(Endoscopy.Year), PID)  %>% dplyr::summarise(avg = mean(Prediction) ) %>% ungroup %>% group_by(Patient) %>% dplyr::filter( Endoscopy.Year == max(Endoscopy.Year) ) %>% dplyr::select(-Endoscopy.Year, -PID)
...
transform.by.time<-function(pt, info) {
  #pt = left_join(pt, info %>% dplyr::select(Hospital.Research.ID, Patient,Samplename,Block), by=c('Hospital.Research.ID','Patient','Samplename'))
...
back = do.call(bind_rows,lapply(unique(pg.samp$Patient) %>% map( function(p) pg.samp %>% filter(Patient == p)  ), transform.by.time, info=patient.info))
...
back = back %>% filter(!is.na(Prediction))
...
pred.confidence =
  qt %>% as.data.frame.matrix %>% dplyr::group_by(quant) %>% dplyr::mutate( 
...
pred.confidence = pred.confidence %>% dplyr::mutate(Risk = ifelse(perc<.5, 'Low','High'))
...
pred.confidence = pred.confidence %>% mutate(Risk = as.character(ifelse(p.value > 0.05, 'Moderate', Risk)))
...
pcal = BarrettsProgressionRisk::showPredictionCalibration(pred.confidence)
pcal
...
coefs.per.pt = data.frame(matrix(ncol=length(unique(pg.samp$Patient)), nrow=nrow(ch), dimnames=list( ch$label, unique(pg.samp$Hospital.Research.ID))))
...
ch = as_tibble(coefs$`0.9`[,1,drop=F], rownames='coef')

loo.ch = dplyr::left_join(ch, loo.coefs, by='coef')

library(bootstrap)

ch$jack.se = apply(loo.ch, 1, function(x) {
...
colnames(ch) = c('label','coef','jack.se')
...
p = pred.tiles(back %>% filter(Patient == 6), colors=myPal, probs=T, ncol=1) + labs(title='')
...
ga = grid.arrange(
  pred.tiles(back %>% filter(Patient %in% c(13,20,34,56,60,48) & Status == 'P'), ncol=1, colors=myPal, probs=T) + labs(title='Progressors') + theme(legend.position = 'none'),
  pred.tiles(subset(back, Patient %in% c(15,16,21,3,45,19) & Status == 'NP'), ncol=1, colors=myPal, probs=T) + labs(title='Non-Progressors') + theme(legend.position = 'none'),
ncol=2)
...
p1 = pred.tiles(back %>% filter(Status == 'NP'), colors=myPal, probs=T, nrow=2) + labs(title='Non-Progressors') 
riskLegend = get.legend(p1)
p1 = p1 + theme(legend.position = 'bottom')
riskLegendLong = get.legend(p1)

p1 = p1 + theme(legend.position = 'none')
p2 = pred.tiles(back %>% filter(Status == 'P'), probs=T, colors=myPal,nrow=2) + labs(title='Progressors') + theme(legend.position = 'none')
