...
ggsave(filename='plots/unadj_relrisk.png', plot=unadjRR + ggtitle(''), width=6, height=5, units='in', dpi=300)
rm(unadjRR, myPal)
```
...
rp = plot.risk.by.path(pg.samp %>% ungroup) + labs(title='Discovery cohort samples', y='% samples classified', x='Sample pathology') + theme(strip.text = element_text(size=14))
rp
ggsave(filename='plots/pred_by_path.png', plot=(rp + theme(legend.position = 'none') + ggtitle('')), width=7.5, height=6.5, units='in', dpi=300)
rm(rp)
```
...
rp = ggplot(endos.m, aes(x=brks, y=pred.r*100, ymin=(pred.r-`sem(P)`)*100, ymax=(pred.r+`sem(P)`)*100, group=brks)) +
  ...
  geom_text(aes(label=paste(round(pred.r*100), '%', sep=''), y=(pred.r-`sem(P)`)*100), color='white', nudge_y=-3) +
  geom_text(aes(y=5, label=paste('n=',n.endo,'\ns=',n.samp,sep='')), color='white', nudge_y=2) + 
  labs(x='Months prior to endpoint', y='Sensitivity for high risk samples', title='High risk classifications prior to final endoscopy', subtitle='n = #endoscopies, s = #samples') + coord_cartesian(ylim = c(0,100)) + 
  plot.theme + theme( axis.text.x=element_text(angle=45,hjust=1), text=element_text(size=12) ) 
rp

ggsave(filename='plots/pred_endo.png', plot=rp + ggtitle(''), width=6, height=5, units='in', dpi=300)
rm(endos,endos.m,rp)
```
...
p = ggplot(risk.over.time) + facet_wrap(~Status, scales='free', ncol = 1) +
...
p
ggsave(filename='plots/risks_over_time.png', plot = p + ggtitle(''), height=12, width=7, units='in')
...
rp = plot.risk.by.path(val.preds) + labs(title='Validation cohort samples', y='% samples classified', x='Sample pathology')  + theme(strip.text = element_text(size=12))
rp
ggsave(filename='plots/validation/val_pred_by_path.png', plot=rp + theme(legend.position = 'none') + ggtitle(''), width=6, height=7, units='in', dpi=300)
rm(rp)
...
p = second %>% group_by(Status, Rule) %>% tally %>% spread(Rule,n) %>% dplyr::rename(RFA = `1`, `6-12 month recheck` = `2`, `12-24 month recheck` = `3`, `3-5 year surveillance` = `4`)  %>%
  ...
  ggplot(aes(variable, value, group = Status)) + 
  geom_col(aes(fill=Status), position = position_dodge()) + geom_text(aes(label=paste0('n=',value)), position = position_dodge(width = 1)) + coord_flip() +
  scale_fill_manual(values = c('#4575B4', '#A50026'), name='' ) + plot.theme + labs(x='', y='Number of Patients', title='Second Endoscopy Rx') + theme(legend.position = 'bottom')
p
