...
ind.status <- c('Non-Progressor'='Patient 57 (n=1)',
              'Progressor'='Patient 76 (n=1), excl. HGD/IMC')
...
```{r prediction_cutoff, echo=F, message=F, warning=F, fig.height=5, fig.width=5}
...
ggplot() + geom_line(data=df, aes(specificity, sensitivity)) + scale_x_reverse() + 
...
multi.roc.plot(list(roc, rocEndoM, rocEndoA, rocPtM, rocPtA,rocFirstEndo,rocLastEndo), 'ROC Comparisons')
```
...
ggplot(melt(subset(m, Measure == 'AUC'), measure.vars='value'), aes(x=model, y=value*100, ymin=CI.min*100, ymax=CI.max*100, group=Measure, fill=Measure)) + 
  ...
  theme(legend.position = 'none') + labs(x='', y='Percent', title='ROC Comparison') 
```
...
```{r, echo=F, warning=F, message=F, fig.height=12, fig.width=16}

mtn.spatial<-function(df, b, limits, title='', pal=NULL) {
  if (is.null(pal))
    pal = RColorBrewer::brewer.pal(length(limits), 'Paired')

  df = merge(df, b[,c('Samplename','Endoscopy.Year','months.before.final','Block','nl')], by.x='variable',by.y='Samplename')
  df$nl = factor(df$nl, ordered = T)

  #pal = pal[which(limits %in% levels(df$nl))]
  
  ggplot(df, aes(x=chr.length)) + facet_grid(Endoscopy.Year~chr, scales='free_x', space='free_x') +
    geom_rect(aes(xmin=start, xmax=end, ymin=0, ymax=mean.value, fill=nl), show.legend=T) + 
    scale_fill_manual(values=pal, limits=limits,  name='OGJ Dist.') +
    theme(axis.text.x = element_blank()) +
    plot.theme + labs(title=title)
}

ogjLimits = levels(factor(back$nl, ordered = T))

p1 = mtn.spatial(subset(globalCNdf, Patient == 48 & seg.type != 'arms'), back, ogjLimits, 'Patient 48, (P)')
ogjLegend = get.legend(p1)
p1+theme(legend.position = 'none')

mtn.spatial(subset(globalCNdf, Patient == 60 & seg.type != 'arms'), back,ogjLimits,'Patient 60 (P)' )+theme(legend.position = 'none')

mtn.spatial(subset(globalCNdf, Patient == 3 & seg.type != 'arms'), back,ogjLimits,'Patient 3 (NP)' )+theme(legend.position = 'none')

mtn.spatial(subset(globalCNdf, Patient == 15 & seg.type != 'arms'), back,ogjLimits,'Patient 15 (NP)' )+theme(legend.position = 'none')
...
```{r p53IHC, echo=F, warning=F, message=F, fig.height=6, fig.width=8}

p53Path$Risk[p53Path$Risk == 'High'] = 'Aberrant'
p53Path$Risk[p53Path$Risk == 'Low'] = 'Normal'

col = brewer.pal(11, "PRGn")[c(3,9)]
...
pred.tiles(subset(back, Patient %in% c(6)), ncol=1, cols=myPal, probs=T) + labs(title='') + theme(legend.position = 'none')
...
grid.arrange(
  pred.tiles(subset(back, Patient %in% c(13,20,34,56,60,48) & Status == 'P'), ncol=1, cols=myPal, probs=T) + labs(title='Progressors'),
  pred.tiles(subset(back, Patient %in% c(15,16,21,3,45,19) & Status == 'NP'), ncol=1, cols=myPal, probs=T) + labs(title='Non-Progressors'),
ncol=2)
...
```{r echo=F, warning=F, message=F, fig.height=14,fig.width=28}
grid.arrange(
  pred.tiles(subset(back.v, Status == 'P'), cols=myPal, probs=T) + labs(title='Validation Progressors') + theme(legend.position = 'none'),
  pred.tiles(subset(back.v, Status == 'NP'), cols=myPal, probs=T) + labs(title='Validation Non-Progressors') + theme(legend.position = 'none'),
ncol=2)
```
...
roc$model = 'Discovery'
multi.roc.plot(list(roc, vroc), title='ROC') 
...
# ggplot(subset(back, Status == 'P'), aes(factor(Endoscopy.Year, ordered=T), nl)) + geom_tile(aes(fill=Risk, color=p53.Status), size=1.5)  + facet_wrap(~Patient, scales='free') + 
#   scale_fill_manual(values=riskCols, limits=levels(back$Risk),name='Progression') + 
#   scale_color_manual(values=c('grey22','white'), limits=levels(back$p53.Status), drop=F, name='p53 IHC', labels=c('normal','aberrant')) +
...
#     theme(panel.background = element_rect(fill='grey60', color='grey60'), panel.grid.major = element_line(color='grey70'), panel.grid.minor = element_blank())
# 
# ggplot(subset(back, Status == 'NP'), aes(factor(Endoscopy.Year, ordered=T), nl)) + geom_tile(aes(fill=Risk, color=p53.Status), size=1.5)  + facet_wrap(~Patient, scales='free') + 
#   scale_fill_manual(values=riskCols, limits=levels(back$Risk),name='Progression') + 
#   scale_color_manual(values=c('grey22','white'), limits=levels(back$p53.Status), drop=F, name='p53 IHC', labels=c('normal','aberrant')) +
#     geom_text(aes(label=Pathology), color='black', show.legend = F) +
#     labs(y='Oesophageal Location',x='Months Prior to Endpoint', title='') +
...
## P
vig.plot(df=subset(back, Patient == 34),info= subset(patient.info, Patient == 34),cols=riskCols)
vig.plot(subset(back, Patient == 56), subset(patient.info, Patient == 56),cols=riskCols)
vig.plot(subset(back, Patient == 20), subset(patient.info, Patient == 20),cols=riskCols)
vig.plot(subset(back, Patient == 86), subset(patient.info, Patient == 86),cols=riskCols)


## NP
vig.plot(subset(back, Patient == 3), subset(patient.info, Patient == 3),cols=riskCols)
vig.plot(df=subset(back, Patient == 23),info= subset(patient.info, Patient == 23),cols=riskCols)
vig.plot(subset(back, Patient == 33), subset(patient.info, Patient == 33),cols=riskCols)
vig.plot(subset(back, Patient == 49), subset(patient.info, Patient == 49),cols=riskCols)
vig.plot(subset(back, Patient == 51), subset(patient.info, Patient == 51),cols=riskCols)
           
vig.plot(subset(back, Patient == 6), subset(patient.info, Patient == 6),cols=riskCols)
vig.plot(subset(back, Patient == 54), subset(patient.info, Patient == 54),cols=riskCols)
```
...
### Recommendations
 ...
 ```{r rx, echo=F, warning=F, message=T}
...
In the literature the risk of BE progression to HGD/cancer is estimated to be 0.2-0.7% per year.

So for every 100 BE patients, at most `r 0.07*100` patients will progress in a year.

Lifetime risk is assumed to be ~10% presuming 10 years followup (`r 0.7*10`%).  Two ways to do this, take one year and look at the progression predictions...or take all of them and compare to the lifetime risk.
...
### Lifetime adjusted risk

```{r adjProbRisk, echo=F, fig.height=6, fig.width=10}
#popL = round((table(all$Status)['P']/(0.1*100))*100)
cases = table(sum.patient.data$Status)['P']
popL = round((table(sum.patient.data$Status)['P']/(0.1*100))*100)
...
loopreds$P1 = 1/(1+exp(-loopreds$OR+abs(offset))) # Adjust OR for RR
...
ggplot(loopreds, aes(P1)) + 
    geom_histogram(aes(fill=..x..), breaks=cuts, show.legend = F) + 
    #geom_vline(xintercept = mean(adjRR)) +
    scale_fill_gradientn(colors = myPal,  name='') + 
    plot.theme + theme(legend.position = 'none') + 
    labs(y='n Samples', x='Probability',
         title='Sample Predictions: Lifetime risk adjusted')
        
ggplot(loopreds, aes(OR)) + 
    geom_histogram(aes(fill=..x..), bins=10, show.legend = F) + 
    scale_fill_gradientn(colors = myPal,  name='') + 
    #scale_fill_distiller(palette = 'RdYlBu', name='P(P)') + 
    plot.theme + theme(legend.position = 'none') + 
    labs(y='n Samples', x='Adjusted Relative Risk',
         title='Sample Predictions: Lifetime risk adjusted')
...
```{r lifetimeFPR, echo=F, fig.height=6, fig.width=6}
...
## GLM regions only
coxDF = cbind(coxDF[,1:3],pathology[,c('BE','ID')], coxDF[,grep('^\\d+:', ch$label, value=T)])
