...
features = features %>% filter(label != 'cx') %>% separate(label, c('chr','start','end'), sep=':|-', remove=F) %>% 
  ...
  mutate(chr = factor(chr, levels=c(1:22), ordered=T)) %>% 
  ...
  left_join(chr.lengths %>% dplyr::select(chrom, chr.length), by=c('chr'='chrom')) %>% mutate(chr = factor(chr, levels=c(1:22), ordered=T))
...
i.thresholds = do.call(bind_rows, purrr::map(insilico.rocs, function(r) pROC::coords(r, 'best') %>% as_tibble %>% 
                                               mutate(AUC = as.numeric(r$auc), model=r$model, is=as.character(r$is))) ) %>%
...
rm(rocEndoM, rocEndoA, rocLastEndo, rocFirstEndo, rocRandEndo, rocPtM, rocPtA)
```
...
load(list.files(model.dir, 'loo_0.9.Rdata',recursive = T, full.names = T)[2], verbose=T)
...
load(list.files(model.dir, 'all.pt.alpha',recursive = T, full.names = T), verbose=T)
d.coefs = coefs[['0.9']]
...
est = info  %>% dplyr::filter(!grepl('SNP', Cohort)) %>%
  ...
  arrange(n.patients) %>% ungroup %>%
  dplyr::mutate(idx = row_number())
# 
# ggplot(est, aes(n.patients, auc)) + geom_point() + geom_smooth(method = 'lm') + xlim(20, 300) + ylim(0.6,1)
# ggplot(est, aes(n.patients, specificity)) + geom_point() + geom_smooth(method = 'lm')  + ylim(0.6,1)
...
predict(fit, data.frame(n.patients = c(180, 190, 200, 220, 250, 275, 300)), interval = 'confidence', type = 'response' )
# 
fit = lm((specificity)~n.patients, data=est)
predict(fit, data.frame(n.patients = c(180, 190, 200, 220, 250, 275, 300)), interval = 'confidence', type = 'response' )
# 
fit = lm(auc~n.patients, data=est)
predict(fit, data.frame(n.patients = c(180, 190, 200, 220, 250, 275, 300)), interval = 'confidence', type = 'response' )

fit = lm(n.coefs~n.patients, data=est)
autoplot(fit)
predict(fit, data.frame(n.patients = c(180, 190, 200, 220, 250, 275, 300)), interval = 'confidence', type = 'response' )

# 
#fit = lm(n.patients~auc, data=est)
#predict(fit, data.frame(auc = c(0.7, 0.89, 0.92, 0.95, 0.97)), interval = 'confidence' )
