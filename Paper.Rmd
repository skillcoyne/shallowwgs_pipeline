...
#length(performance.at.1se)
#length(unique(pg.sampNOLGD$Patient))

#perf = cbind.data.frame(perf=performance.at.1se, pt=unique(pg.sampNOLGD$Patient))
...
#pROC::coords(roc, 0.3, input='threshold', transpose=T)
...
#p = multi.roc.plot(list(roc,rocNOHGD), threshold = 0.3, palette = 'Dark2' ) + labs(title='LOO model comparisons', subtitle = 'threshold = 0.3')
p
...
multi.roc.plot(list(roc,rocEndoM, rocEndoA), threshold = 0.3, palette = 'Dark2' ) + labs(title='Aggregated per Endoscopy ', subtitle = 'threshold = 0.3')
...
#   mutate(Group = case_when( grepl('Endoscopy', model, ignore.case=T) ~ 'Endoscopy', grepl('Patient',model,ignore.case=T) ~ 'Patient', TRUE ~ 'Sample')) %>%
#   mutate(order = row_number(), Group = factor(Group, levels=c('Sample','Endoscopy','Patient'), ordered=T))
# p = ggplot(mm, aes(x=reorder(model, order), y=value*100, ymin=CI.min*100, ymax=CI.max*100, group=Measure, fill=Group)) + 
...
ggplot(pg.samp, aes(Prediction)) + geom_histogram(aes(fill=..x..), breaks=cuts, show.legend = F) + 
  scale_fill_gradientn( colors=myPal) +
  #scale_fill_distiller(palette = 'RdYlBu', name='P(P)') +
  #scale_fill_distiller(palette = myPal) +
...
unadjRR = ggplot(pg.samp, aes(RR, fill=Prediction)) + 
  geom_histogram(aes(fill=..x..), bins=10, show.legend = F) +
...
wtA = signif(wilcox.test(filter(el2, Status == 'Progressor')$`diff(arms)`, filter(el2, Status == 'Non-Progressor')$`diff(arms)`)$p.value,3)
...
# }
p20 = mtn.spatial(globalCNdf %>% filter(Patient == 20 & seg.type != 'arms'), pg.samp, ogjLimits )
ggsave(filename='plots/spatial/patient_20_P.png', plot=grid.arrange(p20$grob), width=16, height=6, units='in', dpi=300)

p51 = mtn.spatial(globalCNdf %>% filter(Patient == 51 & seg.type != 'arms'), pg.samp, ogjLimits,'Patient 51 (NP)' )
...
#### By pathology?
```{r, echo=F, warning=F, message=F, fig.height=10, fig.width=12}
rp = plot.risk.by.path(pg.samp %>% ungroup) + labs(title='Discovery cohort samples', y='% Classified')
rp
...
rp = ggplot(endos.m, aes(x=brks, y=pred.r*100, ymin=(pred.r-`sem(P)`)*100, ymax=(pred.r+`sem(P)`)*100, group=brks)) +
  ...
  labs(x='Months prior to endpoint', y='Sensitivity', title='High risk classifications prior to final endoscopy', subtitle='n = #endoscopies, s = #samples') + coord_cartesian(ylim = c(0,100)) + 
...
```{r pred_by_time}
pts = pg.samp %>% dplyr::group_by(Status, Patient, months.before.final, brks, PID)%>% dplyr::select(Patient, Status, months.before.final, brks, PID, Block, Endoscopy.Year, Pathology, Prediction, RR, Risk) %>% ungroup

pts %<>% mutate(Patient = factor(Patient)) %>% group_by(Patient) %>% arrange(Endoscopy.Year)
...
#time.pred = left_join(time.pred,time.pred %>% group_by(Status, Patient) %>% dplyr::summarise(max.path = max(endo.path)), by=c('Status','Patient'))
...
p = ggplot(risk.over.time) + facet_wrap(~Status, scales='free', ncol = 1) +
  ...
  plot.theme 
p
ggsave(filename='plots/risks_over_time.png', plot = p, height=12, width=7, units='in')
#ggsave(filename='plots/risks_over_time.png', plot = p, width=12, height=7, units='in')
```
...
plist = purrr::map(sum.patient.data$Patient, function(pt) {
  ...
  patient = pg.samp %>% filter(Patient == pt) %>% dplyr::select(Patient,Status,Pathology,Endoscopy.Year,Block,Risk,months.before.final) %>% dplyr::rename(Endoscopy = Endoscopy.Year, GEJ.Distance = Block) %>% ungroup
    ...
    p = BarrettsProgressionRisk::patientRiskTilesPlot(patient, 'months.before.final','rev', 0.5) + theme(legend.position = 'none') + labs(x='months',y='location') + theme(text = element_text(size = 10), plot.title = element_text(face = 'bold', size=16), axis.text = element_text(size = 10))
...
17p11 loss and p53 IHC correlation  Fisher's `r broom::tidy(ct) %>% kable() %>% kable_styling(full_width=F)`
...
```{r}
rp = plot.risk.by.path(val.preds) + labs(title='Validation cohort samples', y='% Classified')  
rp
ggsave(filename='plots/validation/val_pred_by_path.png', plot=rp + theme(legend.position = 'none'), width=5, height=6, units='in', dpi=300)
...
p34 = vig.plot(df=filter(pg.samp, Patient == pt),cols=riskCols) + 
    theme(legend.position = 'bottom', text = element_text(size=12)) + labs(title = filter(sum.patient.data, Patient == pt)$desc) 
...
info = do.call(bind_rows, purrr::map(list.files(cache.dir, 'merged.Rdata', recursive = T, full.names = T), function(f) {
  ...
  left_join(model.info, do.call(bind_rows, purrr::map(rocs, function(roc) {
    add_column(auc=as.numeric(pROC::auc(roc)), pROC::coords(roc, 'best', transpose=F))  
  })), by='auc')
...
info = add_row(info, n.patients = d.n.patient, cv.performance = d.perf, n.coefs = nrow(d.coefs), n.samples = 773, auc = d.roc$auc,
               threshold = pROC::coords(d.roc, 'best', transpose=F)$threshold,
               specificity = pROC::coords(d.roc, 'best', transpose=F)$specificity,
               sensitivity = pROC::coords(d.roc, 'best', transpose=F)$sensitivity,
...
info = add_row(info, n.patients = dv.n.patient, cv.performance = dv.perf, n.coefs = nrow(dv.coefs), n.samples = dv.n.samples, auc = dv.roc$auc,
               threshold = pROC::coords(dv.roc, 'best', transpose=F)$threshold,
               specificity = pROC::coords(dv.roc, 'best', transpose=F)$specificity,
               sensitivity = pROC::coords(dv.roc, 'best', transpose=F)$sensitivity,
...
info = add_row(info, n.patients = dvsnp.patient, cv.performance = dvsnp.perf, n.coefs = nrow(dvsnp.coefs), n.samples = nrow(dvsnp.preds), auc = dvsnp.roc$auc,
               threshold = pROC::coords(dvsnp.roc, 'best', transpose=F)$threshold,
               specificity = pROC::coords(dvsnp.roc, 'best', transpose=F)$specificity,
               sensitivity = pROC::coords(dvsnp.roc, 'best', transpose=F)$sensitivity,
...
# Estimate BE population 

From the d only model, patient estimates at the second endoscopy

```{r}
pg.samp %>% filter(Patient == 3 & Endoscopy.Year == 1998) %>% dplyr::select(Pathology)
second %>% 

get.path<-function(p,y) {
  max( filter(pg.samp, Patient == p & Endoscopy.Year == y)$Pathology )
}
  
nps = second %>% group_by(Patient, Endoscopy.Year) %>% 
  mutate( max.Path = get.path(Patient,Endoscopy.Year) ) %>% ungroup %>% 
  dplyr::select(Status, max.Path) %>% filter(Status == 'NP') %>% mutate(max.Path = droplevels(max.Path))
nps = nps$max.Path  
#nps = rep(nps$max.Path,24)

# Estimate false positive of LGD recommendations
table(nps)['LGD']/length(nps)

second %>% dplyr::mutate(Risk = case_when(Probability <= 0.3 ~ 'Low', Probability >= 0.5 ~ 'High', TRUE ~ 'Moderate')) %>%
  group_by(Status, Risk) %>% tally %>% spread(Status,n) %>%
  mutate(Total = sum(NP)+sum(P)) %>%
  dplyr::mutate_at(vars(NP,P), list(~./Total)) %>% dplyr::select(-Total)


## Get estimates at multiple timepoints during followup?

per.endo.max = pg.samp %>% group_by(Patient, Status, PID, months.before.final, Endoscopy.Year) %>% 
  dplyr::summarise(max.Path = max(Pathology), max.Risk = max(Risk), max.P = max(Prediction))


pt = per.endo.max %>% dplyr::filter(Status == 'NP' & max.Risk == 'High') %>% dplyr::select(Patient) %>% pull

lgd = per.endo.max %>% filter(Patient %in% pt) %>% filter(max.Path == 'LGD')
per.endo.max %>% filter(Patient %in% lgd$Patient) %>% group_by(Patient) %>% filter(months.before.final == max(months.before.final)) 
```
...
est = tibble(
  Pathology = c(rep('NDBE',372), rep('ID',46), rep('LGD',77),rep('HGD',101)),
  Risk = c(rep('Low',372), rep('Low',46), rep('High',77),rep('High',101)),
  Status = c(rep('NP',372+46), rep('P',101+77))
)

est %>% group_by(Status,Risk) %>% tally %>% spread(Status,n)

x %>% mutate_at(vars(NP,P), list(~round(.*nrow(est))))
...
# Estimated starting numbers from the BEST2 BE cases
est.lgd = round(i*(lgd/n))
est.hgd = round(i*(hgd/n))
est.be = i - est.lgd - est.hgd

# BE->HGD rate per person is 0.006
beHGD = round(est.be*0.006)
beLGD = round(est.be*0.03)

# HGD or high risk triggers RFA so the est.hgd cases are immediately at RFA

# From the all model 68% of all LGDs are HIGH risk
round(est.lgd * 0.68) 
# but 23% are false positives
round(est.lgd * 0.68) * 0.23
