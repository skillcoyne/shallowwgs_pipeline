...
back$quants = with(back, cut(Prediction, breaks=cuts, include.lowest = T))
...
vpd = left_join(vpd %>% dplyr::select(-Pathology, -matches('P53')), validation.patient.info %>% dplyr::select(Samplename, Patient, Pathology, p53.Status, Block, Initial.Endoscopy, Final.Endoscopy, Status ), by=c('Sample'='Samplename')) %>% 
  dplyr::rename(Endoscopy.Year = Endoscopy, Samplename = Sample, RR = `Relative Risk`, Prediction = Probability)
...
```{r validation_globalplot, message=F, warning=F, echo=F, fig.height=8, fig.width=8, eval=F}
...
back.v = do.call(bind_rows, lapply(unique(vpd$Patient) %>% map( function(p) vpd %>% filter(Patient == p) ), transform.by.time, info=validation.patient.info)) %>% dplyr::mutate(Patient = factor(Patient))
...
back.v = back.v %>% dplyr::mutate(Risk = factor(Risk, levels=c('High','Moderate','Low')), nl = factor(nl, ordered = T))
