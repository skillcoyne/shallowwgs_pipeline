...
ggsave(filename='plots/discovery_roc.png', plot=droc, width=6, height=4, units='in', dpi=300)
...
ir = ggplot(insilico.rocs, aes(is, auc, color=split)) + ylim(0,1) +
  geom_hline(data=m, aes(yintercept=auc, linetype=line), color='grey32', show.legend=F) + 
  geom_point(size=2) + geom_errorbar(aes(ymin=auc.ci.min, ymax=auc.ci.max), width=0.3) + 
  geom_text(data=m, aes(label=paste(variable, round(auc, 2), sep='\n')), color='grey32', nudge_y=0, show.legend=F) + 
  scale_linetype_manual(values=c('dotdash','longdash')) +
  plot.theme + scale_color_brewer(palette='Set1', name='Type') +
  theme(axis.text.x=element_blank(), legend.position = 'bottom') +
  labs(y='AUC', x='Data splits', title='AUC per split', subtitle='20 insilico 20:69 splits')
ir
ggsave(filename='plots/auc_splits.png', plot=ir, width=7, height=8, units='in', dpi=300)
...
ggsave('plots/compare_auc.png', plot=comp, width=6, height=8, units="in", limitsize=F, dpi = 300)
...
ggsave(filename='plots/pred_calib.png', plot=pcal, width=6, height=6, units='in', dpi=300)
```
...
ggsave(filename='plots/chr17.png', plot=g17, width=10, height=5, units='in', dpi=300)
...
cn.mtn.plot(subset(valCNdf, seg.type != 'arms'), labeller(Status=n.status, chr=label_value)) + theme(legend.position = 'none')
