...
rp = ggplot(endos.r, aes(x=brks, y=pred.r*100, ymin=(pred.r-`sem(P)`)*100, ymax=(pred.r+`sem(P)`)*100, group=brks)) + ylim(0,110) +
  facet_grid(~Status) +
  geom_bar(stat='identity', show.legend = F, fill=riskCols[2]) + 
...
rp
ggsave(filename='thesis/pred_endo.png', plot=rp, width=11, height=8, units='in', dpi=300, scale=1)
...
cleaned = list.files(path=paste(data, 'Cleaned', sep='/'), pattern='raw', full.names=T, recursive=T)
cleaned = grep(paste(unique(validation.patient.info$Hospital.Research.ID), collapse='|'), cleaned, value=T)

chr.len = get.chr.lengths()

tmp = 'thesis/thesis_validationdf.Rdata'
if (file.exists(tmp)) {
  load(tmp)
} else {
  validation.df = do.call(rbind, lapply(cleaned, function(f) {
    df = read.table(f, sep='\t', header=T)
    tiled = segment.matrix(tile.segmented.data(df, chr.info=chr.len))
    for (i in 1:ncol(tiled)) 
      tiled[,i] = unit.var(tiled[,i]) 
    tiled[is.na(tiled)] = 0
    tiled
  }))
...
table(subset(vpd, Prediction > 0.5)$Status)
table(subset(vpd, Prediction < 0.5)$Status)
...
tmp = 'thesis/thesis_repeats.df.Rdata'
if (file.exists(tmp)) {
  load(tmp)
} else {
  repeat.df = do.call(rbind, lapply(cleaned, function(f) {
    df = read.table(f, sep='\t', header=T)
    df = df[, c(1:4, which(colnames(df) %in% repeats$Samplename))]
...
repeats$Probability = round(repeats$Probability, 2)

write.table(repeats, sep='\t', quote=F, row.names=F, file='thesis/tech_repeats.txt')

write.table(subset(back, Patient %in% unique(repeats$`Patient ID`)), sep='\t', row.names=F, file='thesis/original_rep_predictions.txt')
