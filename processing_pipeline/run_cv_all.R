...
info = bind_rows(d.info %>% dplyr::select(Status, Patient, matches('Hospital'),PID,Endoscopy,Pathology,Samplename) %>% mutate(cohort='discovery'),
                 v.info %>% dplyr::select(Status, Patient, matches('Hospital'),PID,Endoscopy,Pathology,Samplename) %>% mutate(cohort='validation')) %>% 
