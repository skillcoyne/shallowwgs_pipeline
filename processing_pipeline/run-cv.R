...
cache.dir = paste(outdir, '5e6_arms_all_norm_exAHM0320', sep='/')
...
sampleStatus = patient.info[,c('Samplename','Status')]
...
tiles = grep('arms', cleaned, invert=T, value=T)
arms = grep('arms', cleaned, value=T)




tiled.segs = do.call(rbind, lapply(tiles, function(f) {
  segs = read.table(f, sep='\t', header=T, stringsAsFactors = F)
  segment.matrix(segs)
}))
dim(tiled.segs)
tiled.segs = tiled.segs[sampleStatus$Samplename,]
...
dim(tiled.arms)
tiled.arms = tiled.arms[sampleStatus$Samplename,]
...
norm.segs = t(apply(tiled.segs, 1, function(x) {
  x[which(x == 0 | is.na(x))] = mean(x) ## I'm not sure what is better here, but setting these unknowns to 0 pushes the low extremes out a bit so best to avoid?
  x-mean(x, na.rm=T)
}))

#segsList = prep.matrix(tiled.segs)
segsList = prep.matrix(norm.segs)
...
norm.arms = t(apply(tiled.arms, 1, function(x) {
  x[which(x == 0 | is.na(x))] = mean(x) ## I'm not sure what is better here, but setting these unknowns to 0 pushes the low extremes out a bit so best to avoid?
  x-mean(x, na.rm=T)
}))

#armsList = prep.matrix(tiled.arms) # unit.var in here
armsList = prep.matrix(norm.arms) # unit.var in here
