...
#} else {
sets = create.sample.sets(patient.info[c('Hospital.Research.ID','Samplename','Status')], 'Samplename', folds, splits, 0.2)  
#}
...
if (file.exists(file)) {
  ...
  for (a in alpha.values) {
    ...
    cv.patient = crossvalidate.by.patient(x=dysplasia.df, y=labels, sampleID = 'Samplename', lambda=l, pts=sets, a=a, nfolds=folds, splits=splits, fit=fit0, select='deviance', opt=-1, standardize=F)
