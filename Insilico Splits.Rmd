...
model.dir = paste(data, 'Analysis/5e6_arms_all_exAHM0320', sep='/')
split.dir = '/Volumes/research1/gerstung/skillcoyne/Ellie/Analysis/5e6_arms_splits_exAHM0320'

performance = data.frame(); rocs = list()
select.alpha = '0.9'

file = paste(model.dir, 'insilico_splits.Rdata', sep='/')
if (file.exists(file)) {
  load(file, verbose=T)
  rocs = insilico.rocs
  ...
  save(insilico.rocs, insilico.perf, file=paste(model.dir, 'insilico_splits.Rdata', sep='/'))
}

n.splits = nrow(performance)

load(list.files(model.dir, full.names = T, pattern = 'all.*Rdata'))

performance = rbind(performance, performance.at.1se[[select.alpha]])
performance$split = c(paste('split',1:n.splits,sep='.'), 'discovery')
rm(plots,coefs,performance.at.1se,models,cvs,dysplasia.df,labels)

load(list.files(model.dir,full.names = T, pattern='loo.Rdata'))
rm(plots, performance.at.1se,coefs,nzcoefs,fits)
