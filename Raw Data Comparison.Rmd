...
# Adjusted to nearly integer ploidy?

```{r}


files = list.files('~/Data/BarrettsProgressionRisk/Analysis/IntPloidy/multipcf_perPatient/', '5e06_tiled', recursive=T, full.names = T)
training.tiles.int = do.call(bind_rows, purrr::map(files, function(f) {
  read_tsv(f,col_types=c(.default=col_double()))
})) %>% dplyr::rename('Sample' = 'X1')

files = list.files('~/Data/BarrettsProgressionRisk/Analysis/IntPloidy/multipcf_perPatient/', 'arms_tiled', recursive=T, full.names = T)
training.arms.int = do.call(bind_rows, purrr::map(files, function(f) {
  read_tsv(f,col_types=c(.default=col_double()))
})) %>% dplyr::rename('Sample' = 'X1')

files = list.files('~/Data/BarrettsProgressionRisk/Analysis/validation/IntPloidy/', '5e06_cleaned_tiled', recursive=T, full.names = T)
val.tiles.int = do.call(bind_rows, purrr::map(files, function(f) {
  read_tsv(f,col_types=c(.default=col_double()))
})) %>% dplyr::rename('Sample' = 'X1')

files = list.files('~/Data/BarrettsProgressionRisk/Analysis/validation/IntPloidy/', 'arms_cleaned_tiled', recursive=T, full.names = T)
val.arms.int = do.call(bind_rows, purrr::map(files, function(f) {
  read_tsv(f,col_types=c(.default=col_double()))
})) %>% dplyr::rename('Sample' = 'X1')



t.pt = per.pt.tile(training.tiles.int) %>% mutate(cohort='train')
v.pt = per.pt.tile(val.tiles.int) %>% mutate(cohort='val')
...
mean(tile.dist.measure(training.tiles.int)$mean)
mean(tile.dist.measure(training.tiles.int)$sd)


mean(tile.dist.measure(val.tiles.int)$mean)
mean(tile.dist.measure(val.tiles.int)$sd)
