...
## Normalize Validation to itself

Instead scaling using z-values from the trianing data, I can do it from within the validation cohort. This looks better here, but when I use this for predictions it doesn't significantly change the results.
...
preds = left_join(all.val, preds, by=c('Samplename'='Sample', 'Hospital Research ID', 'SLX-ID', 'Index Sequence', 'Block ID'))

# Non progressors
preds %>% dplyr::filter(Status == 'NP') %>% group_by(Risk) %>% tally() %>% spread(Risk, n) 

# FPR is nearly 40% still
nrow(preds %>% filter(Status == 'NP' & Risk == 'High'))/nrow(preds %>% filter(Status == 'NP'))
...
nrow(preds %>% filter(Status == 'P' & Risk == 'Low' & !grepl('OCCAMS',`Hospital Research ID`)))/nrow(preds %>% filter(Status == 'P' &  !grepl('OCCAMS',`Hospital Research ID`)))
```
...
How does it change if I use a global variance, or a rolling-bin variance (to preserve some local info) to adjust?   
...
wt.sds = sapply(sds, function(x) weighted.mean(c(x,global.sd),w=c(0.5,0.5)))
