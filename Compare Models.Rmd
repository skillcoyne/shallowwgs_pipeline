...
p = ggplot(melt(rc9, id.vars='kb'), aes(variable, value, group=kb)) + ylim(0,1) + 
  geom_bar(aes(fill=kb), stat='identity', position='dodge') + 
  geom_text(aes(label=round(value,2)), position=position_dodge(width=1)) + 
  scale_fill_brewer(palette='Set1') + 
  labs(title='Discovery ROC', subtitle = 'alpha=0.9', x='', y='%') + 
  theme_minimal() + theme(text=element_text(size=12))
p
ggsave(filename='plots/parameters/training_kb_auc.png', plot = p, width = 6, height = 6, units = 'in', dpi = 300)
