...
```{r pca, echo=F, warning=F, message=F, fig.width=8, fig.height=4}
...
```{r echo=F, message=F, warning=F, pcaplots, fig.align='left', fig.height=30, fig.width=20}
...
#### EM
```{r clust, echo=F, message=F, warning=F, fig.height=4, fig.width=4}
...
```{r clust2, echo=F, message=F, warning=F, fig.height=4, fig.width=4}
llk = as.data.frame(matrix(nrow=nrow(sum.patient.data),ncol=3,dimnames=list(c(sum.patient.data$Patient), c(2:4))))
for (pt in sum.patient.data$Patient) {
    status = subset(sum.patient.data, Patient == pt)$Status
...
#### Kmeans
```{r kmeansnorm, echo=F, message=F, warning=F, fig.height=6, fig.width=6} 
for (pt in sum.patient.data$Patient) {
    status = subset(sum.patient.data, Patient == pt)$Status
    x1 = patient.data[[pt]]$norm.seg.vals
    rownames(x1) = apply(patient.data[[pt]]$seg.vals[,c(1:4)], 1, function(x) strip.whitespace(paste(x[['chrom']], ':', x[['start.pos']], '-', x[['end.pos']], sep=''))) 
...
ggplot(subset(hgmx, x > 0), aes(x=chr,y=P.x,color=factor(cluster))) + ylim(0,0.5)+ 
  geom_point() + 
  geom_line(alpha=0.1) + 
...
```{r xval_pt, echo=F, warning=F, message=F, fig.align='left', fig.height=4, fig.width=4}
# Leave out patients
