...
```{r clust, echo=T, message=T, warning=F, eval=F}
...
for (pt in subset(sum.patient.data, Status == 'P')$Patient) {
  ...
  x1 = patient.data[[pt]]$seg.vals[,-(1:5)]
  
  model = Mclust(t(x1))
  print(model$loglik)
  
  tryCatch({
    mod1dr = MclustDR(model, lambda = 0)
    #summary(mod1dr)
...
```{r em, echo=F, message=F, warning=F, eval=F} 
## EM
...
```{r apclust, echo=T, message=T, warning=F, fig.height=8, fig.align='left'}
q=0.2
...
for (pt in sum.patient.data$Patient) {
  ...
  last = ncol(x1)
  if (patient.data[[pt]]$info$Status == 'P')
    last = sort(grep('HGD', patient.data[[pt]]$info$Pathology), decreasing=T)[1]
  if (ncol(x1) <= 15) {
...
## (xval) Re run clusters on the Progressors without the final sample
...
for (pt in sum.patient.data$Patient) {
  x1 = apclust.data(patient.data[[pt]]$seg.vals, patient.data[[pt]]$info$Samplename)
  if (subset(sum.patient.data, Patient == pt)$Status == 'P' & length(which(grepl('HGD', patient.data[[pt]]$info$Pathology))) > 0) {
    final = grep('HGD', patient.data[[pt]]$info$Pathology)
...
```{r loc_pca, echo=F, warning=F, message=T, fig.height=12, fig.width=20}
