...
sum.patient.data = as.data.frame(patient.info %>%
  group_by(Patient, Status) %>%
  summarise(
    years=paste(range(Endoscopy.Year), collapse='-'),
    n.samples=length(Samplename), med.cellularity=median(Barretts.Cellularity,na.rm=T),
    mean.reads=mean(Total.Reads),
...
ttr = t.test(subset(patient.info, Status == 'NP')$Total.Reads, subset(patient.info, Status == 'P')$Total.Reads)
ttrbe = t.test(subset(be, Status == 'NP')$Total.Reads, subset(be, Status == 'P')$Total.Reads)
ttrlgd = t.test(subset(lgd, Status == 'NP')$Total.Reads, subset(lgd, Status == 'P')$Total.Reads)

tbclgd = t.test(subset(lgd, Status == 'NP')$Barretts.Cellularity, subset(lgd, Status == 'P')$Barretts.Cellularity)
tbcbe = t.test(subset(be, Status == 'NP')$Barretts.Cellularity, subset(be, Status == 'P')$Barretts.Cellularity)

t.test(subset(patient.info, Pathology == 'BE')$Barretts.Cellularity, 
               subset(patient.info, Pathology == 'LGD')$Barretts.Cellularity)

t.test(subset(patient.info, Pathology == 'LGD')$Barretts.Cellularity, 
               subset(patient.info, Pathology == 'HGD')$Barretts.Cellularity)


## No differences between the number of reads by pathology
t.test(subset(patient.info, Pathology == 'BE')$Total.Reads, subset(patient.info, Pathology == 'LGD')$Total.Reads)
t.test(subset(patient.info, Pathology == 'BE')$Total.Reads, subset(patient.info, Pathology == 'HGD')$Total.Reads)
t.test(subset(patient.info, Pathology == 'LGD')$Total.Reads, subset(patient.info, Pathology == 'HGD')$Total.Reads)

t.test(subset(patient.info, Pathology == 'BE')$Barretts.Cellularity, subset(patient.info, Pathology == 'LGD')$Barretts.Cellularity)
t.test(subset(patient.info, Pathology == 'BE')$Barretts.Cellularity, subset(patient.info, Pathology == 'HGD')$Barretts.Cellularity)
t.test(subset(patient.info, Pathology == 'LGD')$Barretts.Cellularity, subset(patient.info, Pathology == 'HGD')$Barretts.Cellularity)

```

There are a few things we need to be careful of and a couple of questions to check here:

1. The total number of reads per sample between NP and P might show a difference, but currently with multi-test correction it's not significant.
`r  signif(ttr$p.value,2)*11`
So are those samples with lower numbers of reads showing less complexity overall?  When sequencing NP are we likely to get fewer reads than with P? If we look only at the samples with BE (no LGD or HGD samples) the difference disappears `r signif(ttrbe$p.value, 2)*11`. If we include LGD with the BE samples we again see the difference.  `r signif(ttrlgd$p.value, 2)*11`.  This is suggestive that changes in copy number result in more reads being output. 


2. There is a significant difference in the cellularity of the Barretts sample, with progressors having a higher mean cellularity, even when only comparing samples that were either LGD or BE.  `r signif(tbclgd$p.value, 2)` However, even the BE only samples show a higher cellularity overall `r signif(tbcbe$p.value, 2)*11`  That said, currently we have a high rate of missing information, with about `r length(which(is.na(subset(patient.info, Status == 'P')$Barretts.Cellularity)))/length(which(patient.info$Status == 'P'))*100`% of the cellualarities missing for progressors and `r length(which(is.na(subset(patient.info, Status == 'NP')$Barretts.Cellularity)))/length(which(patient.info$Status == 'NP'))*100`% of the NP cellularities missing.

3. With regards to p53 staining, non-progressors have no p53 positive tests, while progressors had `r p53Status[2,2]` overall.
...
plot.pca<-function(pt) {
  ...
  for (sv in c('seg.vals','norm.seg.vals')) {
    ...
    screeplot(pca, main=paste(pt, df, 'scree'))
...
plot.pca.norm<-function(pt) {
  
  info = arrange(patient.data[[pt]]$info, Endoscopy.Year)
  
  x1 = patient.data[[pt]]$norm.seg.vals[, patient.data[[pt]]$info$Samplename]
  x1 = x1[,patient.data[[pt]]$info$Samplename]
  colnames(x1) = paste(patient.data[[pt]]$info$Endoscopy.Year, '(', patient.data[[pt]]$info$Pathology, ')', sep='' )
...
simX = simX[, c('Status', 'clusters', 'ns','ss','gain','loss')]
...
ggplot(m, aes(x=qv, y=value, col=variable)) + geom_point() +  
  geom_errorbar(aes(ymin=value-sd, ymax=value+sd), width=.01, alpha=0.3) +
  geom_hline(yintercept=5, alpha=0.3) + geom_hline(yintercept=15, alpha=0.3, col='red') + 
  labs(title='Clusters vs q parameters', x='q parameter', y='mean(clusters)')


ggplot(melt(pvals, measure.vars=c('p.clusters'), id.vars='qv'),aes(x=qv,y=value, color=variable)) + geom_point() +
