...
patient.info = read.patient.info(patient.file, demo.file)$info
#head(patient.info)

patient.info = arrange(patient.info, Status, Patient, Endoscopy.Year, Pathology)
patient.info = patient.info[-which(patient.info$Hospital.Research.ID == 'AH0222'),]
...
for (pt in sum.patient.data$Hospital.Research.ID) {
  ...
  gg = ggplot(melt(x1), aes(x=Var2, y=value, group=Var2, fill=Var2, color=Var2)) + 
...
```{r echo=F, message=F, warning=F, fig.width=5, fig.height=5}
g2 + labs(x='Status', y='Complexity (# clusters) per patient', title='AP Clustering', subtitle=paste("p-value", signif(ttpc, 3))) + theme(text=element_text(size=14, face='bold'), legend.position = 'non')
```
...
```{r apclustN, echo=F, message=F, warning=F, fig.height=5, fig.width=5, fig.align='left', eval=F}
...
for (i in 1:nrow(sum.patient.data)) {
  ...
  for (pt in sum.patient.data$Hospital.Research.ID[-i]) {
    x1 = apclust.data(patient.data[[pt]]$seg.vals, patient.data[[pt]]$info$Samplename)
...
```{r xval_smp, echo=F, warning=F, message=F, fig.align='left', fig.height=4, fig.width=4, eval=F}
# Leave out samples?
...
for (pt in sum.patient.data$Hospital.Research.ID) {
  x1 = apclust.data(patient.data[[pt]]$seg.vals, patient.data[[pt]]$info$Samplename)
  if (subset(sum.patient.data, Hospital.Research.ID == pt)$Status == 'P' & length(which(grepl('HGD|IMC', patient.data[[pt]]$info$Pathology))) > 0) {
...
subset(sum.patient.data, Hospital.Research.ID %in% pts)
