...
```{r}
pg.samp = do.call(bind_rows,lapply(unique(pg.samp$Patient) %>% 
                                     map( function(p) pg.samp %>% filter(Patient == p)), transform.by.time, info=patient.info)) 
...
```{r, echo=F, warning=F, message=F, fig.width=10, fig.height=6, eval=F}
...
val.preds = do.call(bind_rows, 
                    lapply(unique(val.preds$Patient) %>% 
...
kmp = 
  survminer::ggsurvplot(sfit, size=1, linetype="strata",risk.table=F, risk.table.col='strata',
