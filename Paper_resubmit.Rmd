...
gp = globalCN.plot + theme(legend.position = 'none', axis.text = element_blank(), strip.text.y = element_blank(), strip.text.x = element_text(size=8)) + 
...
ct = cor.test(pi$p11Loss, (as.numeric(pi$p53.Status)-1))
```

17p11 loss and p53 IHC correlation  R=`r round(ct$estimate,2)`. 
...
kmp = survminer::ggsurvplot(sfit, size=1, linetype="strata",risk.table=T, risk.table.col='strata',
                      conf.int=T, censor=T, legend='none', title='Progression risk over time', xlab='Time (months prior to progression)',
...
kmp = survminer::ggsurvplot(sfit, size=1, linetype="strata",risk.table=T, risk.table.col='strata',
                      conf.int=T, censor=T, legend='none', title='Progression Risk', 
                      subtitle='Training & Validation predictions', xlab='Time (months prior to progression)',
                      ylab = 'Progression risk',
                      ggtheme=theme_minimal(), palette=as.vector(BarrettsProgressionRisk::riskColors()), font.legend=c(14,'bold'))
...
pg.samp = pg.samp %>% mutate(p53.Status = factor(p53.Status, levels=c(0,1), ordered=T)) 

maxP = pg.samp %>% dplyr::group_by(Patient, Status, brks) %>% 
  dplyr::summarise( 'maxP'=max(Prediction), 'Risk'=max(Risk), 'Pathology'=max(Pathology), 'p53'=max(p53.Status,na.rm=T) )

pt.counts = maxP %>% group_by(Patient, Status) %>% dplyr::filter( length(which(grepl('Endpoint', brks))) > 0  & length(brks) > 1) %>% 
  ungroup %>% dplyr::select(Patient, Status) %>% distinct %>% group_by(Status) %>% tally


rules = do.call(bind_rows, purrr::map(unique(maxP$Patient), function(pt) {
  ...
  return(as_tibble(r))
})) %>% dplyr::mutate(t1 = factor(t1, levels=levels(pg.samp$brks), ordered=T), t2 = factor(t2, levels=levels(pg.samp$brks), ordered=T)) %>% arrange(Patient, t2)

rules %>% group_by(Patient, Status) %>% dplyr::filter(t2 == 'Endpoint' & length(t1) > 2) %>% 
  dplyr::select(Patient, Status) %>% distinct %>%  group_by(Status) %>% tally


ids = rules %>% group_by(Patient, Status) %>% dplyr::filter(t2 == 'Endpoint' & length(t1) > 2) %>% dplyr::select(Patient) %>% pull

penult = rules %>% dplyr::filter(Patient %in% ids & t2 != 'Endpoint') %>% arrange(Patient, desc(t1)) %>% group_by(Patient, Status) %>% 
  dplyr::mutate(gid = 1:n() ) %>% dplyr::filter(gid == 1) 

#penult = filter(rules, t2 != 'Endpoint' & (t1 %in% rev(levels(pg.samp$brks))[2] | t2 == rev(levels(pg.samp$brks))[2] ))

penult %>% dplyr::select(Patient, Status) %>% distinct %>% group_by(Status) %>% tally

penult %>% filter(Status == 'P' & rule %in% c(3,4)) %>% kable(caption='Patients that would be missed if we moved Moderate to Low risk') %>% kable_styling(full_width = F)

penult %>% filter(Status == 'NP' & rule == 1) %>% kable(caption='NP patients for RFA') %>% kable_styling(full_width = F)

penult %>% group_by(Status) %>% tally(name='total')
...
path.rx<-function(Patient, brks, Pathology) {
  ...
  for (i in 1:length(brks)) {
    ...
    if ( length(which(grepl('HGD|IMC', Pathology[i:(i+1)]))) > 0  |
      ...
      rule = 2
    } else if ( length(which(grepl('BE|NDBE|ID', Pathology[i:(i+1)]))) > 1  ) {
      rule = 3
    }
  rules = rbind(rules, cbind( as.character(brks[i]), as.character(brks[(i+1)]), rule))
  }  
...
rules2$t1 = factor(rules2$t1, levels=levels(pg.samp$brks), ordered=T)
rules2$t2 = factor(rules2$t2, levels=levels(pg.samp$brks), ordered=T)
...
second %>% group_by(Status, rule) %>% tally %>% spread(rule,n) %>% 
  kable(caption='Patient recommendation by PATH alone in penult endoscopy' ) %>% kable_styling(full_width = F)

second %>% group_by(Status, rule) %>% tally %>% spread(rule,n) %>% 
  left_join(second %>% group_by(Status) %>% tally(name='total'), by='Status') %>%
  mutate_if(is.numeric, funs(  round((./total)*100,1)  )) %>% dplyr::select(-total) %>%
  kable(caption='Final endoscopy: % patients by recommendation rule based on the max (PATH) per endoscopy, not (RISK)') %>% kable_styling(full_width=F)
