...
p11Loss %>% filter(value < -1) %>% group_by(Status) %>% tally(name='p11 Loss') %>% full_join(
  p11 %>% group_by(Status) %>% tally(name='Total Samples') ) %>% 
  kable(caption='Chr17 p11 loss') %>% kable_styling(full_width = F)

p11Loss %>% group_by(Status, Hospital.Research.ID) %>% tally
...
patient.info %>% filter(Samplename %in% p11Loss$variable) %>% group_by(forcats::fct_explicit_na(p53.Status)) %>% tally %>% 
  set_names(c('p53 Status', 'n samples')) %>% kable(caption='p53 status for samples with p11 loss') %>% kable_styling(full_width = F)
...
val.info = readxl::read_xlsx(patient.file, 'Final Validation Samples') %>% 
  ...
  mutate(Status = factor(Status, levels=levels(pg.samp$Status))) %>% ungroup %>% group_by(`Hospital Research ID`) %>% 
...
val.preds = left_join(all.val.preds, val.info, by=c('Hospital Research ID', 'Sample'='Samplename')) %>% dplyr::select(Patient, everything())
...
pt = 34
p34 = vig.plot(df=subset(pg.samp, Patient == pt),info= subset(patient.info, Patient == pt),cols=riskCols) + 
...
pt = 3
p3 = vig.plot(df=subset(pg.samp, Patient == pt),info= subset(patient.info, Patient == pt),cols=riskCols) + 
    theme(legend.position = 'bottom') + labs(title = filter(sum.patient.data, Patient == pt)$desc) + 
    theme(legend.position = 'none', text = element_text(size=12))
...
pt = 68
p68 = vig.plot(df=subset(pg.samp, Patient == pt),info= subset(patient.info, Patient == pt),cols=riskCols) + 
    theme(legend.position = 'bottom') + labs(title = filter(sum.patient.data, Patient == pt)$desc) + 
    theme(legend.position = 'none', text = element_text(size=12))
...
rules$t1 = factor(rules$t1, levels=levels(pg.samp$brks), ordered=T)
rules$t2 = factor(rules$t2, levels=levels(pg.samp$brks), ordered=T)

penult = filter(rules, t2 != 'Endpoint' & (t1 == rev(levels(pg.samp$brks))[2] | t2 == rev(levels(pg.samp$brks))[2] ))
...
penult %>% group_by(Status, rule) %>% tally %>% spread(rule,n) %>% 
  left_join(penult %>% group_by(Status) %>% tally(name='total'), by='Status') %>%
...
endpoint = subset(rules,  t2 != 'Endpoint') %>% 
  dplyr::group_by(Patient, Status) %>% 
  dplyr::summarise(  max(t2), 'rule'=rule[t2 == max(t2)]  )


endpoint %>% group_by(Status, rule) %>% tally %>% spread(rule,n) %>% 
  left_join(endpoint %>% group_by(Status) %>% tally(name='total'), by='Status') %>%
  mutate_if(is.numeric, funs(  round((./total)*100,1)  )) %>% dplyr::select(-total) %>%
  kable(caption='Final endoscopy: % patients by recommendation rule based on the max (P) per endoscopy') %>% kable_styling(full_width=F)
