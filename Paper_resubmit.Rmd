...
rm(dysplasia.df, plots, pg.samp, models, nzcoefs, models, performance.at.1se)
...
info = do.call(bind_rows, purrr::map(list.files(cache.dir, 'merged.Rdata', recursive = T, full.names = T), function(f) {
  load(f,verbose=F)
  model.info
})) %>% mutate('Cohort' = 'Discovery (patient subsets)')

info = add_row(info, n.patients = d.n.patient, cv.performance = d.perf, n.coefs = nrow(d.coefs), n.samples = 773, auc = d.roc$auc, Cohort = 'Discovery (all)') 

info = add_row(info, n.patients = dv.n.patient, cv.performance = dv.perf, n.coefs = nrow(dv.coefs), n.samples = dv.n.samples, auc = dv.roc$auc, Cohort = 'Discovery+Validation')

x = info %>% group_by(n.patients, Cohort) %>% dplyr::summarise_at(vars(-n.samples), list(~mean(.), ~sd(.))) %>% as_tibble %>% mutate(n.patients = factor(n.patients), Cohort = factor(Cohort, levels=c('Discovery (patient subsets)', 'Discovery (all)', 'Discovery+Validation'), ordered=T))


p = ggplot(x, aes(x=n.patients, y=cv.performance_mean, group=n.patients, fill=Cohort)) + 
  ...
  labs(title='Cross validation performance', y='mean', x='# patients used to train')

legend = get_legend(p)
p = p + theme(legend.position = 'none')

p1 = ggplot(x, aes(x=n.patients, y=n.coefs_mean, group=n.patients, fill=Cohort)) + 
  facet_grid(~Cohort, scales = 'free_x', space='free_x') + 
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0,125)) + scale_fill_brewer(palette = 'Set2') + 
  plot.theme + theme(legend.position = 'none', strip.text = element_blank(), panel.spacing = unit(0.5, 'lines')) +
  labs(title='# Coefficients',y='mean', x='# patients used to train')
...
p2 = ggplot(x, aes(x=n.patients, y=auc_mean, group=n.patients, fill=Cohort)) +
  ...
  plot.theme + theme(legend.position = 'none', strip.text = element_blank(), panel.spacing = unit(0.5, 'lines')) +
  labs(title='AUC', y='mean', x='# patients used to train')

grid.arrange(arrangeGrob(p,p1,p2,nrow=1), arrangeGrob(legend), heights=c(4,1))


ggsave(filename='plots/performance/training_n_pts.png',
       plot=grid.arrange(arrangeGrob(p,p1,p2,nrow=1), arrangeGrob(legend), heights=c(4,1)), width=12, height=8, units='in', dpi=300)
