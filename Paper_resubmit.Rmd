...
val.info %>% group_by(Status) %>% dplyr::summarise(
  `Total Patients` = length(unique(Patient)),
  `Total Samples` = length(Samplename),
`max follow-up` = max(`Weeks Follow Up`/52),
`min follow-up` = min(`Weeks Follow Up`/52),
  `mean follow-up` = mean(`Weeks Follow Up`/52),
  `sd follow-up` = sd(`Weeks Follow Up`/52)
)

val.info %>% group_by(Status, Pathology) %>% tally
...
val.info %>% filter(Samplename %in% failedQC$samplename)
