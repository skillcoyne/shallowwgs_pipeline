...
i.thresholds = do.call(bind_rows, purrr::map(insilico.rocs, function(r) pROC::coords(r, 'best', transpose=F) %>% as_tibble %>% mutate(AUC = r$auc, model=r$model, is=as.character(r$is)))) %>%
...
rocRandEndo = pROC::roc(Status ~ r.Prob, data=pendo.rand, ci=T, of='thresholds', transpose = T)
...
```{r}
pg.samp = do.call(bind_rows,lapply(unique(pg.samp$Patient) %>% map( function(p) pg.samp %>% filter(Patient == p)), transform.by.time, info=patient.info)) 
...
rp = plot.risk.by.path(pg.samp %>% ungroup)
rp
...
p11Loss %>% filter(value <= -1) %>% group_by(Status) %>% tally(name='p11 Loss') %>% 
...
v.tiles = do.call(bind_rows, purrr::map(list.files(vdir, '5e06_tiled_segvals.txt', full.names = T, recursive = T), function(f) {
  read_tsv(f, col_types = cols(.default=col_double(), 'X1'=col_character()))
})) %>% dplyr::rename(Samplename = 'X1')
v.tiles = filter(v.tiles, Samplename %in% val.info$Samplename)
v.tiles = v.tiles[-which(duplicated(v.tiles$Samplename))[2],]


v.arm.tiles = do.call(bind_rows, purrr::map(list.files(vdir, 'arms_tiled_segvals.txt', full.names = T, recursive = T), function(f) {
  read_tsv(f, col_types = cols(.default=col_double(), 'X1'=col_character()))
})) %>% dplyr::rename(Samplename = 'X1')
v.arm.tiles = filter(v.arm.tiles, Samplename %in% val.info$Samplename)
v.arm.tiles = v.arm.tiles[-which(duplicated(v.arm.tiles$Samplename))[2],]

for (i in 1:length(z.mean))
  v.tiles[, i+1] = BarrettsProgressionRisk:::unit.var(v.tiles[,i+1], z.mean[i], z.sd[i])

for (i in 1:length(z.arms.mean))
  v.arm.tiles[, i+1] = BarrettsProgressionRisk:::unit.var(v.arm.tiles[,i+1], z.arms.mean[i], z.arms.sd[i])

vt = dplyr::select(v.tiles, -Samplename) %>% as.matrix; rownames(vt) = v.tiles$Samplename
at = dplyr::select(v.arm.tiles, -Samplename) %>% as.matrix; rownames(at) = v.arm.tiles$Samplename

v.df = BarrettsProgressionRisk:::subtractArms(vt, at)

valCNdf = as_tibble(t(v.df), rownames='locs') %>% set_names(c('locs',val.info$Samplename)) %>%
  separate(locs, c('chr','start','end'), sep=':|-', remove=F) %>%   
  dplyr::mutate_at(vars(start, end), as.numeric) %>%
  #dplyr::mutate( seg.type = '5MB' ) %>%
  dplyr::mutate( seg.type = ifelse( (start %in% arm.loc$start & end %in% arm.loc$end), 'arms','5MB'  ) ) %>%
  melt(id.vars=c('locs','chr','start','end', 'seg.type')) %>% as_tibble() %>%
  left_join(chr.lengths %>% dplyr::select(chrom,chr.length), by=c('chr'='chrom')) %>%
  left_join(val.info %>% dplyr::select(matches('Hospital|Patient|Samplename|Pathology|Status')), by=c('variable'='Samplename')) %>%
  dplyr::mutate(Status = factor(recode(Status, NP='Non-Progressor',P='Progressor'), ordered=T)) %>% 
  dplyr::mutate( 
    cn = factor(unlist(purrr::map(.x=value, .f=get.cn.state)),  levels=c('loss','neutral','gain'), ordered = T),
    chr = factor(chr, levels = chr.lengths$chrom, ordered=T))

n.status <- c('Progressor'=paste0('Progressor (n=',length(unique(subset(valCNdf, Status == 'Progressor')$variable)), ')'),
              'Non-Progressor'=paste0('Non-Progressor (n=',length(unique(subset(valCNdf, Status == 'Non-Progressor')$variable)),')'))

#cn.mtn.plot(filter(valCNdf, seg.type == '5MB'), labeller(Status=n.status, chr=label_value)) + ylim(-30,20)
...
residuals = do.call(bind_rows, purrr::map(list.files( dirname(vdir), 'residuals.tsv', full.names = T, recursive = T ), function(f) {
  read_tsv(f, col_types = 'ccddddl')
}))

failedQC = residuals %>% filter(varMAD_median > 0.008)
...
all.val.preds = all.val.preds %>% filter( !Sample %in% failedQC$samplename &  
                                            !`Hospital Research ID` %in% unique(oac.info$`Hospital Research ID`) )

val.preds = left_join(all.val.preds, val.info, by=c('Hospital Research ID', 'Sample'='Samplename')) %>% 
  dplyr::select(Patient, everything()) %>% mutate(Pathology = factor(Pathology, levels=c('NDBE','ID','LGD'), ordered=T)) %>% 
...
```{r}
rp = plot.risk.by.path(val.preds) + labs(title='Validation samples classified by pathology')  
rp
...
val.preds = do.call(bind_rows, lapply(unique(val.preds$Patient) %>% 
                            map( function(p) val.preds %>% filter(Patient == p) %>% 
...
kmp = survminer::ggsurvplot(sfit, size=1, linetype="strata",risk.table=T, risk.table.col='strata',
                      ...
                      ggtheme=theme_minimal(), palette=as.vector(BarrettsProgressionRisk::riskColors()), font.legend=c(14,'bold'))
...
```{r}
vp = val.preds %>% dplyr::select(Patient, Status, months.before.final, Risk) %>% mutate(Patient = as.factor(Patient)) %>% 
  group_by(Patient) %>% mutate(Diagnosis = as.integer(Status)-1) 

merged = bind_rows(ps,vp)


sfit = survfit( Surv(months.before.final, Diagnosis, type='right') ~ Risk, data=merged, conf.int=0.95, conf.type='log', na.action=na.omit)
names(sfit$strata) = sub('.*=','', names(sfit$strata))

kmp = survminer::ggsurvplot(sfit, size=1, linetype="strata",risk.table=T, risk.table.col='strata',
                      conf.int=T, censor=T, legend='none', 
                      title='Risks over time', 
                      subtitle='Training & Validation predictions',
                      xlab='Time (months prior to final sample)',
                      ylab = 'Progression risk',
                      ggtheme=theme_minimal(), palette=as.vector(BarrettsProgressionRisk::riskColors()), font.legend=c(14,'bold'))
...
max.rules = purrr::map(unique(pg.samp$Patient), function(pt) {
    ...
    endo = BarrettsProgressionRisk:::.setUpRxTablePerEndo(preds %>% dplyr::mutate(Endoscopy = months.before.final, Sample = 'Samplename'), 'max', verbose=F) %>% mutate(Endoscopy = Endoscopy*-1)
...
pids = names(which(sapply(progressors, function(x) {
  nrow(x) >= 1 & grepl('Endpoint', x[nrow(x),'Time 2'])
})))

nids = names(which(sapply(nonprog, function(x) {
  nrow(x) >= 1 & grepl('Endpoint', x[nrow(x),'Time 2'])
})))
...
second %>% dplyr::select(Patient, Status) %>% distinct %>% group_by(Status) %>% tally %>% kable(caption='n patients with at least two years follow-up') %>%
  kable_styling(full_width = F)
...
rocCox = pROC::roc(Status ~ FittedRR, data=pg.samp, auc=T, ci=T, of='thresholds', transpose=T)
...
rocCox2 = pROC::roc(Status ~ FittedRR, data = first, auc = T, ci = T, of='thresholds', transpose=T)
roc.plot(rocCox2) + labs(title = 'Cox model ROC, first endo')

last = filter(pg.samp, Endoscopy.Year == Final.Endoscopy)
rocCox3 = pROC::roc(Status ~ FittedRR, data = last, auc = T, ci = T, of='thresholds', transpose=T)
roc.plot(rocCox3) + labs(title = 'Cox model ROC, last endo')

rocPath = pROC::roc(Status ~ Pathology, data=pg.samp, auc=T, ci=T, of='thresholds', transpose=T)
...
d.roc = pROC::roc(Status ~ Prediction, data = pg.samp, ci=T, of='thresholds', transpose=T)
...
dv.roc = pROC::roc(Status ~ Probability, data = pg.samp, ci=T, of='thresholds', transpose=T)
...
x = info %>% group_by(n.patients) %>% dplyr::summarise_at(vars(-n.samples), list(~mean(.), ~sd(.))) %>% as_tibble %>% mutate(n.patients = factor(n.patients))

p = ggplot(x, aes(x=n.patients, y=cv.performance_mean, group=n.patients, fill=n.patients)) +
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0,0.8)) + theme(legend.position = 'none') +
  plot.theme + labs(title='Cross validation performance', subtitle='10 random samples', y='mean', x='# patients used to train')

p1 = ggplot(x, aes(x=n.patients, y=n.coefs_mean, group=n.patients, fill=n.patients)) +
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0,90)) + theme(legend.position = 'none') +
  plot.theme + labs(title='# Coefficients selected by model', subtitle='10 random samples', y='mean', x='# patients used to train')

p2 = ggplot(x, aes(x=n.patients, y=auc_mean, group=n.patients, fill=n.patients)) +
  geom_col() + geom_errorbar(aes(ymin=auc_mean-auc_sd/2, ymax=auc_mean+auc_sd/2), width=0.2, color='grey39') +
  geom_text(aes(label=round(auc_mean,2)), y=0.05) +
  scale_y_continuous(expand=c(0,0), limits=c(0,1)) + theme(legend.position = 'none') +
  plot.theme + labs(title='AUC from leave-one-out analysis', subtitle='10 random samples', y='mean', x='# patients used to train')


ggsave(filename='plots/performance/training_n_pts.png',plot=grid.arrange(p,p1,p2,nrow=1), width = 10, height=8, units='in', dpi=300)
