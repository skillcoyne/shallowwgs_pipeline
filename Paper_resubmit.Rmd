...
p = ggplot(df, aes(Specificity, Sensitivity)) + 
    ...
    scale_x_reverse() +
    geom_label(data=filter(i.thresholds, AUC %in% range(AUC) | is == 'discovery'), aes(label=paste0('AUC\n',round(AUC*100))), color='black') +
    #scale_color_brewer(palette='Blues') +
    labs(title='ROC AUC', subtitle='in-silico splits', x='Specificity (FPR)', y='Sensitivity (TPR)')  + plot.theme +
    theme(legend.position = 'none')
p
...
#cn.mtn.plot(filter(valCNdf, seg.type == '5MB'), labeller(Status=n.status, chr=label_value))
...
ggsave('plots/validation/validatation_roc.png', plot=roc.plot(roc) + labs(title='Validation cohort'), width=6, height=4, units='in',dpi=300)
...
rp
ggsave(filename='plots/validation/val_pred_by_path.png', plot=rp, width=9, height=7, units='in', dpi=300)
...
for (pt in c(34,56,20,86,3,23,33,49,51,6,25,68)) {
...
second %>% group_by(Status, rule) %>% tally %>% spread(rule,n) %>% 
  ...
  mutate_if(is.numeric, funs(  round((./total)*100,1)  )) %>% dplyr::select(-total) %>%
  kable(caption='Final endoscopy: % patients by recommendation rule based on the max (PATH) per endoscopy') %>% kable_styling(full_width=F)
