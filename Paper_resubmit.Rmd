...
```{r}
cache.dir = '~/Data/BarrettsProgressionRisk/Analysis/models_5e6_all/supp_train_n/insilico_splits/'

purrr::map(list.files(cache.dir, 'loo.Rdata',recursive = T, full.names = T), function(f) {
  load(f,verbose=T)
  
  length(unique(pg.samp$Patient))
