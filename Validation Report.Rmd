...
all.val = do.call(bind_rows, lapply(sheets, function(s) {
  readxl::read_xlsx(val.file, s) %>% select(`Hospital Research ID`, matches('Status'), `Block ID`,`Sample Type`, `SLX-ID`, `Index Sequence`, Cohort, Batch, RA, matches('Collection')) %>% mutate_at(vars(`SLX-ID`, `Block ID`), list(as.character)) %>% fncols('Collection', 'Biopsy')
}))
...
})) %>% select(`Patient ID`, sample, varMAD_median, n.segs) %>% mutate(varMAD_median = round(varMAD_median, 3)) %>% mutate(sample = str_replace(sample, '_', '-'))


all.val %>% filter(!Samplename %in% val$sample )


val = val %>% filter(sample %in% (all.val %>% select(Samplename) %>% pull) ) %>% 
...
sval = val %>% group_by(Batch) %>% dplyr::mutate(Cohort = Cohort[1]) %>% group_by(Batch, Cohort) %>% dplyr::summarise_at(vars(varMAD_median), funs(min,mean,median,max),na.rm=T)

vt = val %>% group_by(Batch) %>% dplyr::mutate(Cohort = Cohort[1]) %>% group_by(Batch, `Sample Type`, Cohort) %>% dplyr::summarise(
...
val %>% group_by(as.integer(RA),`Sample Type`, Collection) %>% dplyr::summarise(
  `Total Samples` = length(Batch),
  Pass=length(which(varMAD_median<=qc.cutoff)), Fail=length(which(varMAD_median>qc.cutoff))
) %>% rename('as.integer(RA)' = 'Processing') %>% kable(caption='Pass/Fail by sample type') %>% kable_styling('striped')
...
speedvac = left_join(val %>% ungroup %>% filter(Batch %in% c(5)), readxl::read_xlsx(val.file, sheet='Batch 5') %>% select(`Hospital Research ID`, `Block ID`, `Original Cohort`, `Speedvac`))
...
oldp = val %>% ungroup %>% filter(Batch %in% c(9))
...
oldp.preds = oldp.preds %>% filter(Sample %in% (oldp %>% filter(!grepl('OCCAMS',`Hospital Research ID`)) %>% select(sample) %>% pull))
...
new.np = val %>% ungroup %>% filter(Batch %in% c(10))
...
Again, all passed QC though on manual QC I might have failed 2-3.  There were 3 samples that just squeaked in under the cutoff threshold.  But they did.

### Predictions

```{r}

if (file.exists('batch_10.txt')) {
  new.np.preds = read_tsv('batch_10.txt')
} else {

  segFiles = grep(paste(unique(new.np$`Patient ID`),collapse='|'), list.files('~/Data/BarrettsProgressionRisk/Analysis/validation', '2_segObj',  full.names = T, recursive = T), value=T)

  info = readxl::read_xlsx(val.file, 'Batch 10') %>% dplyr::select(`Hospital Research ID`, `Block ID`, Endoscopy, Pathology, `SLX-ID`, `Index Sequence`) %>% mutate(Sample = paste0(`SLX-ID`,'.',`Index Sequence`))
  ...
  write_tsv(new.np.preds, 'batch_10.txt')
}
...
new.np.preds %>% group_by(`Hospital Research ID`) %>% dplyr::summarise(
  `n samples` = n(), 
  'High Risk' = length(which(Risk == 'High')),
  'Mod. Risk' = length(which(Risk == 'Moderate')),
  'Low Risk' =   length(which(Risk == 'Low'))
