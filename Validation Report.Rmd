...
alpha = '0.9'
fit = models[[alpha]]
lambda = performance.at.1se[[alpha]]$lambda
...
file = paste0(pred.dir, '/batch8_preds_alpha', alpha,'.txt')

if (file.exists(file)) {
  batch8_preds = read_tsv(file)
} else {
  ...
  write_tsv(batch8_preds, path=file)
}
...
sheets = readxl::excel_sheets(val.file)[9:13]
...
file = paste0(pred.dir, '/oldp_preds_alpha', alpha, '.txt')
if (file.exists(file)) {
  oldp.preds = read_tsv(file)
} else {
...
#  info = readxl::read_xlsx(val.file, 'Batch 9') %>% dplyr::select(`Hospital Research ID`, `Block ID`, Endoscopy, Pathology, `SLX-ID`, `Index Sequence`, `Path Notes`) %>% mutate(Sample = paste0(`SLX-ID`,'.',`Index Sequence`))
  ...
  write_tsv(oldp.preds, path=file)
}
...
file = paste0(pred.dir, '/val_np_alpha', alpha, '.txt')
if (file.exists(file)) {
  new.np.preds = read_tsv(file)
} else {
  ...
  write_tsv(new.np.preds, path=file)
}
