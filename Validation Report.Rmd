...
on = left_join(val %>% ungroup %>% filter(Batch %in% c(8)),
               readxl::read_xlsx(val.file, sheet='Batch 8') %>% dplyr::select(`Block ID`, `Endoscopy`, `Sample Type`))
...
batch8_preds = do.call(bind_rows,
  purrr::map(grep( paste(unique(on$`Patient ID`), collapse='|'), list.files(preds.dir, full.names = T, recursive = T), value=T), function(f) {
             read_tsv(f, col_types = cols(
                ...
                `Path Notes` = col_logical()
      ))
})) %>% dplyr::mutate_if(is.numeric, list(round), 2)

batch8_preds %>% dplyr::select(`Hospital Research ID`, Sample, everything(), -matches('SLX|Index') ) %>% kable() %>% kable_styling('striped')
...
oldp.preds = do.call(bind_rows,
  purrr::map(grep( paste(unique(oldp$`Patient ID`), collapse='|'), list.files(preds.dir, full.names = T, recursive = T), value=T), function(f) {
             read_tsv(f,col_types = cols(
                Sample = col_character(),
                Probability = col_double(),
                `Relative Risk` = col_double(),
...
new.np.preds = do.call(bind_rows,
  purrr::map(grep( paste(unique(new.np$`Patient ID`), collapse='|'), list.files(preds.dir, full.names = T, recursive = T), value=T), function(f) {
             read_tsv(f,col_types = cols(
                Sample = col_character(),
                Probability = col_double(),
                `Relative Risk` = col_double(),
