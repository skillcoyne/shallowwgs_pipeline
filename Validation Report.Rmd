...
pred.dir = '~/Data/BarrettsProgressionRisk/Analysis/validation/predictions'
dir.create(pred.dir, showWarnings = F)

model = '~/Data/BarrettsProgressionRisk/Analysis/5e6_arms/all.pt.alpha.Rdata'
load(model, verbose=T)
...
if (file.exists(paste0(pred.dir, '/batch8_preds.txt'))) {
  batch8_preds = read_tsv(paste0(pred.dir, '/batch8_preds.txt'))
} else {
  ...
  write_tsv(batch8_preds, paste0(pred.dir, '/batch8_preds.txt'))
}
...
if (file.exists(paste0(pred.dir, '/oldp_preds.txt'))) {
  oldp.preds = read_tsv(paste0(pred.dir, '/oldp_preds.txt'))
} else {
...
#  info = readxl::read_xlsx(val.file, 'Batch 9') %>% dplyr::select(`Hospital Research ID`, `Block ID`, Endoscopy, Pathology, `SLX-ID`, `Index Sequence`, `Path Notes`) %>% mutate(Sample = paste0(`SLX-ID`,'.',`Index Sequence`))
  ...
  write_tsv(oldp.preds, paste0(pred.dir, '/oldp_preds.txt'))
}
...
) %>% kable(format='html',escape = F,caption = 'Classified predictions per patient (all P)') %>% kable_styling('striped', full_width = F)


oldp.preds = oldp.preds %>% rowwise %>% mutate(PID = unlist(strsplit(`Block ID`, '(-|\\s)\\w+$', perl = T))[1]) 

ggplot(oldp.preds, aes(`Hospital Research ID`, Probability, color=Probability)) + geom_point() +
  scale_color_distiller(palette = 'RdYlBu', name='P(P)') + plot.theme + theme(axis.text.x = element_text(angle=45,hjust=1))
  

oldp.per.endo = oldp.preds %>% group_by(`Hospital Research ID`, PID) %>% 
  dplyr::mutate('n.samples'=length(PID),'Samples'=paste(Sample,collapse=',')) %>% 
  dplyr::summarise_at(vars(matches('Prob|Relative|Endo')), list(max)) %>% dplyr::select(-matches('^Sample$')) %>% 
  arrange(`Hospital Research ID`, Endoscopy, PID) 
...
if (file.exists(paste0(pred.dir, '/val_np.txt'))) {
  new.np.preds = read_tsv(paste0(pred.dir, '/val_np.txt'))
} else {
  segFiles = grep(paste(sub('\\(.*\\)','', new.np$`Patient ID`),collapse='|'), list.files('~/Data/BarrettsProgressionRisk/Analysis/validation', '(2|3)_segObj',  full.names = T, recursive = T), value=T)
  ...
  write_tsv(new.np.preds, paste0(pred.dir, '/val_np.txt'))
}
...
ggplot(new.np.preds, aes(Probability)) + geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1), show.legend = F) + scale_fill_distiller(palette = 'RdYlBu', name='P(P)') +
    plot.theme + labs(title='Sample predictions for validation NP samples', y='n Samples', x='Probability') + theme(legend.position = 'none')

new.np.preds = new.np.preds %>% rowwise %>% mutate(PID = unlist(strsplit(`Block ID`, '(-|\\s)\\w+$', perl = T))[1]) 

ggplot(new.np.preds, aes(`Hospital Research ID`, Probability, color=Probability)) + geom_point() +
  scale_color_distiller(palette = 'RdYlBu', name='P(P)') + plot.theme + theme(axis.text.x = element_text(angle=45,hjust=1))
  

np.per.endo = new.np.preds %>% group_by(`Hospital Research ID`, PID) %>% 
  dplyr::mutate('n.samples'=length(PID),'Samples'=paste(Sample,collapse=',')) %>% 
  dplyr::summarise_at(vars(matches('Prob|Relative|Endo')), list(max)) %>% dplyr::select(-matches('^Sample$')) %>% 
  arrange(`Hospital Research ID`, Endoscopy, PID) 
#back.per.endo = back.per.endo %>% mutate(quants = cut(Prediction, breaks=cuts, include.lowest = T)) %>% left_join(pred.confidence %>% dplyr::select(quants, Risk), by='quants') %>% dplyr::select(-matches('quant')) %>% dplyr::mutate(Risk = as.character(Risk))

#np.per.endo %>% filter(`Relative Risk` > 0)
