...
```{r echo=F}
pander(
  as.data.frame(sum.patient.data %>% summarise(
...
```{r echo=F}
cohortSummary = as.data.frame(sum.patient.data %>% group_by(Status) %>% summarise(
  'Total Patients'=length(unique(Patient)),
  'Total Samples'=sum(total.samples),
...
m = rbind.data.frame(
  ...
  get.roc.stat(roc))
m$model = factor(m$model, levels=c('HGD Only','HGD & LGD','CN Model'), ordered = T)

rocList = list(roc,pathroc,hgdpathroc)
...
qt = cbind(qt, back %>% group_by(quants) %>% summarise ( 'mn'=mean(Prediction), 'sd'=sd(Prediction) ))#, 
...
# x = back %>% group_by(Status, Pathology) %>% summarise( mean(Prediction), 'count'=length(Pathology) )
# ggplot(x, aes(Pathology, count)) + geom_bar(stat='identity',aes(fill=`mean(Prediction)`)) + 
#     scale_fill_gradientn(colors = myPal,  name='') + facet_grid(~Status) + plot.theme
...
ggsave('pred_heatmap_ALL.png', plot=grid.arrange(p2,p1,nrow=2), width=52, height=16, units="in", limitsize=F, dpi = 300)
