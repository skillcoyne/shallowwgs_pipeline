...
pander(
  ...
  'Median length'=median(M,na.rm=T),
...
# pg.sampNOHGD = subset(pg.sampNOHGD, !is.na(Prediction))
# cuts = seq(0,1,0.1) 
...
# noHGD.pred.confidence = cbind(noHGD.pred.confidence, pg.sampNOHGD %>% dplyr::group_by(quants) %>% dplyr::summarise ( 'mn'=mean(Prediction), 'sd'=sd(Prediction) ))
...
if (file.exists(file)) {
  ...
  hgd.samples[which(hgd.samples$Samplename %in% rownames(dysplasia.df)), 'Prediction'] = pred
...
p = ggplot(classification.rate, aes(model, mean, group=model, fill=model)) + geom_col() +
  geom_text( aes(label=round(mean, 3)), vjust=2, size=5, color='gray39', show.legend=F) +
...
hazards = apply( exp(t(dysplasia.df[,rownames(coefs[[select.alpha]])]) *  coefs[[select.alpha]][,1]), 1, function(x) {
  sd(x)/mean(x)
  #mad(x) # The CV(RR) is pretty sensitive to extremes I think. But I'm not sure if MAD(RR would be better)
})
...
p = ggplot(ch, aes(coef, hazards, col=hazards >= qq[length(qq)])) + 
  ...
  labs(title='Features by CV(relative risk)', x='Coefficient value', y='CV(RR)') + plot.theme 
p
...
insilico.rocs = list()
ifile = paste(cache.dir, 'insilico_splits.Rdata', sep='/')
if (file.exists(ifile)) {
  ...
  rr = ggplot(subset(df, is == 'all' | auc %in% c(range(auc))), aes(Specificity, Sensitivity)) + 
    ...
    theme(legend.position = 'none')
  rr  
  ggsave(filename='plots/auc/roc_CI.png', plot=rr, width=6, height=6, units='in', dpi=300, scale=1.5)
} else {
  message("No insilico split file.")
}
...
#ptendo = do.call(rbind, pg.samp) %>% group_by(Patient, Hospital.Research.ID, Status) %>% dplyr::summarise('max'=max(Prediction), 'avg'=mean(Prediction), 'n'=length(Patient))
...
transform.by.time<-function(pt, info) {
  pt = pt[,c('Status','Patient','Hospital.Research.ID','Path.ID','PID','Pathology','Maximal','Samplename','Final.Endoscopy','Endoscopy.Year','Prediction', 'OR')] %>% group_by(PID, Samplename) %>% dplyr::mutate('months.before.final' = (Final.Endoscopy - Endoscopy.Year)*12)
...
ft.fun<-function(NP,P) {
  ...
  f = chisq.test(c(NP,P))
  cbind.data.frame('p.value'= signif(f$p.value,4))
  #cbind.data.frame('p.value'=f$p.value, 'odds.ratio'=f$estimate)
}

excluded = subset(back, is.na(Prediction))
back = subset(back, !is.na(Prediction))

qt = cbind(qt, back %>% group_by(quants) %>% dplyr::summarise ( 'mn'=mean(Prediction), 'sd'=sd(Prediction) ))
pred.confidence = qt %>% as.data.frame.matrix %>% rowwise() %>% dplyr::mutate( 
  'perc'=P/sum(NP,P), 'p.value'=ft.fun(NP,P)$p.value, 'conf'=ifelse(round(p.value,2) <= 0.05, '*', '') )
...
pred.confidence$Risk[ which(with(pred.confidence, round(p.value,2) <= 0.05 & perc < .3)) ] = 'Low'
pred.confidence$Risk[ which(with(pred.confidence, round(p.value,2) <= 0.05 & perc > .5)) ] = 'High'
...
annotation.table = dplyr::arrange(pred.confidence %>% dplyr::group_by(Risk) %>% dplyr::summarise('n(NP)'=sum(NP), 'n(P)'=sum(P), 'p.value'=format.pval(mean(p.value), 3), 'min'=min(r1), 'max'=max(r2)), min)
...
pcfit = lm(perc~mn, data=pred.confidence)

rsid = rstandard(pcfit)
pred.confidence[which( rsid >= 1 |  rsid <= -1 ), 'Risk'] = 'Moderate'


pcal = ggplot(pred.confidence, aes(mn, perc)) + geom_smooth(method='lm',formula=y~x, color='grey39', linetype='dashed', size=0.5,fullrange=T) + 
  ...
  geom_point() + 
  # geom_abline(slope=1, intercept=0, color='grey39', linetype='dotted', size=0.5) + #geom_text(aes(label=paste(r1,r2,sep='-')), nudge_x=0.05) +
  geom_errorbar(aes(ymin=ci.low, ymax=ci.high), size=0.5, width=0.01) + labs(x='P(P)', y='P:NP', title='') +
  scale_color_manual(values=riskCols,limits=levels(pred.confidence$Risk) ) +   coord_cartesian(xlim=c(0,1), ylim=c(0,1)) +
  scale_x_continuous(expand=c(0,0),limits=c(-0.5,1.5), breaks=cuts, labels=cuts) +  scale_y_continuous(expand=c(0,0),limits=c(-0.5,1.5), breaks=cuts, labels=cuts) +
...
```{r, echo=F, warning=F, message=F, fig.height=5, fig.width=7}
back$ogj = back$nl
...
df2 = df %>% group_by(Status, Patient, Endoscopy.Year) %>% dplyr::summarise( 
...
df2 = df2 %>% group_by(Status, Patient) %>% dplyr::mutate(
...
el2 = el %>% group_by(Status, Patient) %>% dplyr::summarise( diff(arms), diff(CNA) )
...
p = ggplot(subset(df2, early|late), aes(Endoscopy.Year, arms, color=Patient)) + facet_grid(~Status) +
  geom_point(aes(shape=late), show.legend = F, size=2) + geom_line() + 
...
get.w.scale <- function(b) {
  n = sum((b %>% group_by(Patient) %>% dplyr::summarise(unique(n.endos)))[,2])
  #round(n*0.5)+1
  n
}
...
write.table(chh, sep='\t', quote=F, row.names=F, file='features.txt')

```

Clearly need to interpret those "hazards" carefully.  Some of the really high ones are driven by some of the extreme samples.

```{r, fig.width=10, fig.height=10}

gains = subset(chh, coef > 0)

y = cbind.data.frame(labels, dysplasia.df[,gains$label])
y$labels = factor(y$labels)
y$labels = plyr::revalue(y$labels, c('0'='NP','1'='P'))

m = melt(y, id.vars='labels')
colnames(m) = c('status','label','value')
m = base::merge(m, gains, by='label')
m$chr = factor(m$chr, levels=chr.lengths$chrom, ordered = T)

p = ggplot(m) + facet_wrap(chr~label, scales='free_y') +
  geom_jitter(aes(status, value, group=status, color=status), width=0.01) + 
  geom_violin(aes(status, value, group=status, fill=status, color=status), alpha=0.5) + 
  geom_hline(yintercept=c(1,-1), color='grey39', linetype='dotted') +
  geom_label(data=subset(gains, hazards >= 1), aes('P',0, label=round(hazards,2)), color='red') + 
  geom_label(data=subset(gains, hazards < 1), aes('P',0, label=round(hazards,2)), color='grey') + 
  labs(y='',x='',title='Gains') +
  plot.theme + theme(legend.position = 'none', strip.text=element_text(size=9)) 
p
ggsave('plots/gains.png', plot=p, width=10, height=10, unit='in', dpi=300)


losses = subset(chh, coef < 0)

y = cbind.data.frame(labels, dysplasia.df[,losses$label])
y$labels = factor(y$labels)
y$labels = plyr::revalue(y$labels, c('0'='NP','1'='P'))

m = melt(y, id.vars='labels')
colnames(m) = c('status','label','value')
m = base::merge(m, losses, by='label')
m$chr = factor(m$chr, levels=chr.lengths$chrom, ordered = T)
...
p = ggplot(m) + facet_wrap(chr~label, scales='free_y') +
  ...
  geom_violin(aes(status, value, group=status, fill=status, color=status), alpha=0.5) + 
  geom_hline(yintercept=c(1,-1), color='grey39', linetype='dotted') +
  geom_label(data=subset(losses, hazards >= 1), aes('P',3, label=round(hazards,2)), color='red') + 
  geom_label(data=subset(losses, hazards < 1), aes('P',3, label=round(hazards,2)), color='grey') + 
  labs(y='',x='',title='Losses') +
  plot.theme + theme(legend.position = 'none', strip.text=element_text(size=9)) 
p
ggsave('plots/losses.png', plot=p, width=10, height=10, unit='in', dpi=300)
...
down.dir = '~/Data/Ellie/Analysis/downsampled'
dsids = read_xlsx(paste(data,'Analysis/downsampled_ids.xlsx',sep='/'))
load(list.files(down.dir, 'Rdata', full.names = T), verbose=T)

dspred = merge(dspred, dsids, by.x='row.names',by.y='Illumina ID')
rownames(dspred) = dspred$Row.names
dspred$Row.names = NULL
head(dspred)
...
colnames(prg) = colnames(dspred)[c(1:2,ncol(dspred))]

dspred = rbind(dspred[colnames(prg)],prg)

dspred= subset(dspred, Type != 'LGD') # These are pre-progressors most likely

dspred$Type = factor(dspred$Type, levels=c('Normal', 'NDBE','Non-Progressor','Progressor','Tumor Adjacent BE'), ordered = T) 

colors = myPal[seq(2, length(myPal), round(length(myPal)/length(levels(dspred$Type))))]
...
m = subset(dspred, !Type %in% c('Normal','LGD','NDBE'))
m$Type = droplevels(m$Type)

colors = myPal[seq(2, length(myPal), round(length(myPal)/length(levels(m$Type))))]
pr = ggplot(m, aes(Type, RR, fill=Type)) + 
  geom_jitter(width=0.2, aes(color=RR)) + 
  scale_color_gradientn(colors = colors,  name='') + 
  geom_boxplot(outlier.colour = NA) + 
...
rrPath = back %>% group_by(Status, Pathology) %>% dplyr::summarise( 'mean.RR'=mean(OR),'ci.min'=mean(OR)-(1.96*sd(OR)), 'ci.max'=mean(OR)+(1.96*sd(OR)) )
...
```{r 5yrrisk, echo=F, fig.height=8, fig.width=12, eval=F}
...
risksL = subset(bd, Status == 'NP') %>% group_by(Patient) %>% dplyr::summarise( 'n.samp'=length(Patient), 'high.risk'=length(which(Risk == 'High')), 'mod.risk'=length(which(Risk == 'Moderate')) )
...
for (yr in names(which(table(bd$Endoscopy.Year) > 10))) {
  risks = subset(bd, Status == 'NP' & Endoscopy.Year == yr) %>% group_by(Patient) %>% dplyr::summarise( 'n.samp'=length(Patient), 'high.risk'=length(which(Risk == 'High')), 'mod.risk'=length(which(Risk == 'Moderate')) )
