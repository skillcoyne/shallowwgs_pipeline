...
```{r loonohgd, echo=F, message=F, warning=T, fig.width=5, fig.height=5, eval=F}
...
```{r, echo=F, fig.width=6, fig.height=6,eval=F}
...
load(file='~/Data/Ellie/Analysis/5e6_arms_all/insilico_splits.Rdata', verbose=T)
...
ggsave('plots/auc/path_p53_cn_auc.png', plot=comp, width=2.7, height=3.7, units="in", limitsize=F, dpi = 300, scale=2)
...
pcal = ggplot(pred.confidence, aes(mn, perc)) + 
  ...
  scale_color_manual(values=riskCols,limits=levels(pred.confidence$Risk) ) + plot.theme + theme(legend.position = 'none')
pcal

ggsave(filename='plots/pred_calib.png', plot=pcal, width=2.2, height=2.2, units='in', dpi=300, scale=2)
...
unadjRR
ggsave(filename='plots/unadj_relrisk.png', plot=unadjRR, width=5.5, height=4.3, units='in', dpi=300)
...
ggsave(filename='plots/feature_jacknife.png', plot=p, width=6, height=8, unit='in', dpi=300)
...
rp = ggplot(riskPath, aes(Pathology, ratio*100)) + 
  ...
  labs(title='Samples predicted by pathology', y='% Predicted', x='Pathology') + theme(legend.position = 'bottom')
rp

ggsave(filename='plots/pred_by_path.png', plot=rp, width=5.8, height=4.5, units='in', dpi=300, scale=1.5)
...
rp = ggplot(p53Path, aes(Pathology, ratio*100, group=Pathology, fill=p53.Status)) + geom_bar(stat='identity', position = position_stack()) +
  ...
  geom_text(aes(label=paste('n=',nPath,sep=''), x=Pathology, y=102)) +
...
rp
ggsave(filename='plots/p53_pred.png', plot=rp, width=4.5, height=3.6, units='in', dpi=300, scale=1.5)
...
rp = ggplot(endos.m, aes(x=brks, y=pred.r*100, ymin=(pred.r-`sem(P)`)*100, ymax=(pred.r+`sem(P)`)*100, group=brks)) + ylim(0,105) +
  geom_bar(stat='identity', fill='orangered4', show.legend = F) + 
...
rp
ggsave(filename='plots/pred_endo.png', plot=rp, width=5.6, height=4.4, units='in', dpi=300, scale=1.5)
...
p1 = pred.tiles(subset(back, Status == 'NP'), cols=myPal, probs=T, nrow=2, scales='free', drop=T) + labs(title='Non-Progressors') 
#p1 = pred.tiles(subset(back, Status == 'NP'), cols=riskCols, probs=F, OR=F, nrow=2) + labs(title='Non-Progressors') 
riskLegend = get.legend(p1)
p1 = p1 + theme(legend.position = 'bottom')
riskLegendLong = get.legend(p1)

p1 = p1 + theme(legend.position = 'none')
p2 = pred.tiles(subset(back, Status == 'P'), probs=T, cols=myPal,nrow=2,scales='free',drop=T) + labs(title='Progressors') + theme(legend.position = 'none')

ggsave('plots/pred_heatmap_ALL.png', plot=grid.arrange(p2 + labs(x='', y='',title=''),p1 + labs(x='', y='',title=''),nrow=2), width=11, height=3.4, units="in", limitsize=F, dpi = 300, scale = 3)
...
p1 = pred.tiles(subset(back, Status == 'NP'), cols=myPal, probs=T, nrow=2, scales='free_x', drop=T) + labs(title='Non-Progressors') + theme(legend.position = 'none') 
p2 = pred.tiles(subset(back, Status == 'P'), probs=T, cols=myPal,nrow=2,scales='free_x',drop=T) + labs(title='Progressors') + theme(legend.position = 'none')
ggsave('plots/pred_heatmap_ALL_widthadj.png', plot=grid.arrange(p2 + labs(x='', y='',title=''),p1 + labs(x='', y='',title=''),nrow=2), width=11, height=3.4, units="in", limitsize=F, dpi = 300, scale = 3)
```
...
dirs = c('~/Data/Ellie/Analysis/downsampled5G_BEAdjacent',
         '~/Data/Ellie/Analysis/downsampled5G_BEAdjacent_normal/',
          '~/Data/Ellie/Analysis/downsampled5G_NDBE')
...
for (i in 1:length(predfiles)) {
  if (length(predfiles[[i]]) > 0) {
    ...
    dspred$Type = basename(names(predfilels[i]))
    predObj[[i]] = dspred
...
ggsave('plots/vignette/vig_68.png', plot=p, width=6, height=4,dpi=300)
...
rules2$rule = factor(rules$rule, levels=c('None',4,3,2,1), ordered = T)
...
pander(rbind('Progressors'=table(subset(second, Status == 'P')$rule),
            'Non-Progressors'=table(subset(second, Status == 'NP')$rule)), caption='Two endos, pathology rules')
