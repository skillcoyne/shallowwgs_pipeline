...
ggsave(filename='plots/globalCN.png', plot=globalCN.plot, width=16, height=4.6, units='in', dpi=300)
globalCN.plot

globalBECN.plot = globalBECN.plot + theme(legend.position = 'none') + labs(y='mean adj.seg.', title="Non-Dysplastic Barrett's Samples") 
ggsave(filename='plots/globalBECN.png', plot=globalBECN.plot, width=16, height=4.6, units='in', dpi=300)
globalBECN.plot
...
```{r noLGD, echo=F, warning=F, message=F, fig.width=6, fig.height=6, eval=F}
...
```
# TODO number endoscopies so the images are scaled 
...
predA = list.files(dirA, 'predictions_all_exAHM0320.Rdata', full.names = T)
predB = list.files(dirB, 'predictions_all_exAHM0320.Rdata', full.names = T)
...
if (length(files) > 0) {
  ...
  predA = sub('_raw.txt','',basename(c(list.files(dirA, pattern='raw', full.names=T))))
  predB = sub('_raw.txt','',basename(c(list.files(dirB, pattern='raw', full.names=T))))
  ...
  for (n in 1:length(files)) {
    ...
    dspred$Type = NA
    dspred[predA,'Type'] = 'AdjacentBE'
    dspred[predB,'Type'] = 'BEST2 NDBO'
    pred = dspred
    
    dspred = subset(pred, Type == 'AdjacentBE')
    
    save(dspred, file=paste(dirA, 'predictions_all_exAHM0320.Rdata', sep='/'))

    dspred = subset(pred, Type == 'BEST2 NDBO')
    save(dspred, file=paste(dirB, 'predictions_all_exAHM0320.Rdata', sep='/'))
