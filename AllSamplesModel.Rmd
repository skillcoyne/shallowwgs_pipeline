...
ggsave(filename='plots/globalCN.png', plot=globalCN.plot + labs(title=''), width=16, height=4.6, units='in', dpi=300)
globalCN.plot
...
ggsave(filename='plots/globalBECN.png', plot=globalBECN.plot + labs(title=''), width=16, height=4.6, units='in', dpi=300)
globalBECN.plot
...
min.theme = theme(text=element_text(size=12), panel.background=element_blank(), strip.background=element_rect(fill="grey97"), strip.text.y = element_text(size=6),
...
ggsave(filename='plots/spatial/patient_20.png', plot=grid.arrange(p2$grob), width=14, height=6, units='in', dpi=300)
ggsave(filename='plots/spatial/patient_51.png', plot=grid.arrange(p6$grob), width=14, height=6, units='in', dpi=300)
...
rp = ggplot(riskPath, aes(Pathology, ratio*100)) + 
  ...
  scale_color_manual(values=c('white','black','white'), limits=levels(pred.confidence$Risk) ) +
...
#  geom_text(data=endos.m, aes(y=round(pred.r*100+2), label=paste(round(pred.r*100), '%', sep='')), color='grey45') +
  ...
  geom_text(aes(y=pred.r*100-8, label=paste('n=',n.endo,'\ns=',n.samp,sep='')), color='white') + 
...
table(rev(arrange(subset(back, Patient == 19), brks)$brks))
...
#back = back %>% group_by(Patient) %>% dplyr::mutate(endo.num = as.integer(droplevels(brks)) )
#back$endo.num = factor(back$endo.num, ordered = T)

#back$endo.num = factor(as.integer(back$brks))
#back = back %>% group_by(Patient) %>% dplyr::mutate(endo.num = ifelse(endo.num == max(endo.num), 'End', endo.num)   )
...
lim = c(1:4)
p = pred.tiles(b, cols=riskCols, ylim=lim,  probs=F, scales='free', space='free') + labs(title='', x='', y='') + theme(legend.position = 'none', strip.text = element_blank(), plot.margin = unit(c(0,0.2,0,0), 'cm'))
  ggsave(paste('plots/heatmaps/P/pt_32.png', sep=''), plot=p, height=4, width=get.w.scale(b), unit='in', dpi=300 )
...
for (i in subset(sum.patient.data, Status == 'NP')$Patient) {
  ...
  p = pred.tiles(b, cols=riskCols, ylim=lim,  probs=F, scales='free', space='free') + labs(title='', x='', y='') + theme(legend.position = 'none', strip.text = element_blank(), plot.margin = unit(c(0,0.2,0,0), 'cm'))
  ggsave(paste('plots/heatmaps/NP/pt_',i,'.png', sep=''), plot=p, height=get.h.scale(b,9), width=get.w.scale(b), unit='in', dpi=300 )
}


all = pred.tiles(back, cols=riskCols,  probs=F, scales='free', space='free') + labs(title='', x='', y='') + theme(legend.position = 'bottom')
