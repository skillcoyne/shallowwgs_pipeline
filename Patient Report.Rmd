...
if ( file.exists(cachefile) ) {
  ...
  for (patient.name in sum.patient.data$Patient  )  {
    ...
    if (length(sample.cols) > 0) {
      ...
      patient = arrange(subset(patient.info, Patient == patient.name),  Endoscopy.Year, Pathology, Samplename)
    }
  }
save(patient.data, file=cachefile)
}
print(paste("Missing patients:", paste(setdiff(sum.patient.data$Patient, names(patient.data)), collapse=', ')))
...
example.pt = subset(sum.patient.data, Status == 'P' & total.samples > 2)$Patient[5]
...
ggplot( melt(patient.data[[example.pt]]$seg.vals[,-(1:5)]), aes(x=value) ) + 
        facet_wrap(~variable,scales = "free_x") + 
        geom_histogram( fill=I("blue"),col=I("lightblue"), binwidth=0.03) + labs(title=paste(example.pt,' segmented value distribution'), x='', y='')
```
...
ggplot( melt(patient.data[[example.pt]]$norm.seg.vals), aes(x=value) ) + 
        facet_wrap(~variable,scales = "free_x") + 
        geom_histogram( fill=I("blue"),col=I("lightblue"), binwidth=0.3) + labs(title=paste(example.pt,' normalized segmented value distribution'), x='', y='')
...
lapply(patient.data, function(pt) {
  ...
  do.call('grid.arrange', list(g1,g2, top=paste(name, status, sep=" - "), ncol=2))
})
...
sum.var = rbind('Progressors'=do.call(rbind.data.frame, var.seg.lengths[prog.pt]) %>% summarize( 'n.segments' = length(seg.len), 'median.length'=median(seg.len), 'mean.length'=mean(seg.len), 'sd'=sd(seg.len)),
  'Non-Progressors'=do.call(rbind.data.frame, var.seg.lengths[np.pt]) %>% summarize( 'n.segments' = length(seg.len), 'median.length'=median(seg.len), 'mean.length'=mean(seg.len), 'sd'=sd(seg.len)))

pander(sum.var, caption='Variable regions between progressors and non-progressors')

sum.seg = rbind('Progressors'=do.call(rbind.data.frame, segment.lengths[prog.pt]) %>% summarize( 'n.segments' = length(seg.len), 'median.length'=median(seg.len), 'mean.length'=mean(seg.len), 'sd'=sd(seg.len)),
  'Non-Progressors'=do.call(rbind.data.frame, segment.lengths[np.pt]) %>% summarize( 'n.segments' = length(seg.len), 'median.length'=median(seg.len), 'mean.length'=mean(seg.len), 'sd'=sd(seg.len)))
...
ggplot(df, aes(status, seg.len)) + 
  geom_boxplot(aes(group=status, fill=status), show.legend=F) + 
  ...
  labs(y='Variable segment length', x='', title='Variable regions', caption=paste('Wilcox p-value', ifelse (lenWT$p.value < .001, '< .001', format.pval(lenWT$p.value)) ))
...
ggplot(df, aes(status, n, group=status)) + 
  geom_boxplot(aes(fill=status), position = 'dodge', show.legend = F) + 
  geom_label(data=subset(df, status == 'Progressor'), aes(status, max(n)-2*sd(n), label=paste('median\n',median(n),sep='') )) +
  geom_label(data=subset(df, status == 'Non-Progressor'), aes(status, max(n), label=paste('median\n',median(n),sep=''))) +
  labs(y='Number of segments', title="Total number of segments", caption=paste('Wilcox p-value', ifelse (lenWT$p.value < .001, '< .001', format.pval(nWT$p.value)) ))
...
var.seg.lengths = lapply(var.seg.lengths, function(x) merge(x, chr.info, by.x='chr', by.y='chr') )

# All segments
segment.lengths = lapply(segment.lengths, function(x) merge(x, chr.info, by.x='chr', by.y='chr') )
...
melted = merge(sum.patient.data[,c('Patient', 'Status')], melted, by.x='Patient', by.y='L1')

chr.ratio.tt = t.test(unlist(chr.ratios[prog.pt]), unlist(chr.ratios[np.pt]))

ggplot(melted, aes(Status, value, fill=Status)) + geom_boxplot() + 
  labs(title=paste("Variable region length, p-value", ifelse(chr.ratio.tt$p.value < 0.001, "<0.001", signif(chr.ratio.tt$p.value,2))),y='Segment Length/Chr length')
...
ft = fisher.test((df %>% group_by(status) %>% summarize( 'gains'=sum(gain), 'losses'=sum(loss)))[,c('gains','losses')])

ggplot(melt(df), aes(status, value)) + facet_grid(.~variable) + 
  geom_bar(aes(fill=status),stat='identity') + 
  labs(title='Total gains & Losses excluding HGD/IMC', y='counts', x='', caption=paste('Gains and losses are signficantly enriched in Progressors vs Non. Fisher odds ', round(ft$estimate,2), '(p=', format.pval(ft$p.value, digits=1), ')', sep='' )) 
...
```{r, hmm, echo=F, message=F, warning=F, fig.height=100, fig.width=10}
...
m$brks <- cut(m$value, include.lowest=T, right=F,ordered_result=T,
                   breaks=c(0, 1, 3, 5, 8, 10, 12),
                   labels=c('0','1-3', '4-5', '6-8','9-10', '10-12'))
