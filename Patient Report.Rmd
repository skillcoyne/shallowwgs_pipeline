...
bad_batch = which(patient.info$Batch.Name %in% c('Batch_8', 'Batch_9'))
patient.info = patient.info[-(bad_batch),]
...
for (patient.name in sum.patient.data$Patient  )  {
  ...
  message( paste(probes, ' probes (', round(probes/nrow(segvals))*100, '%)',' below the minimum probe count (',min.probes,')', sep=''))
  ...
  if (nrow(segvals) < 100) {
    warning(paste(patient.name, 'has fewer than 100 genomic reginos with segmented values:', nrow(segvals)))
    next
  }
