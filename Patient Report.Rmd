...
cor.scores = as.data.frame(matrix(nrow=length(patient.data$Patient), ncol=3, dimnames=list(patient.data$Patient, c('similar','different','n.probes'))))
  ...
  if(nrow(segvals)>0) {
      ...
      if(nrow(segvals)>=3) {
        ...
        for (i in 1:ncol(rc$r)) {
          ...
          sim = sim + signif( sum(rc$r[which(rc$P[,i] < 0.05 & rc$r[,i] > 0),i ]), 2)
          diff = diff + signif(sum(rc$r[which(rc$P[,i] < 0.05 & rc$r[,i] < 0),i ]),2)
        }  
        cor.scores[patient.name,] = c(sim,diff,nrow(sample.normalised.segvals))
