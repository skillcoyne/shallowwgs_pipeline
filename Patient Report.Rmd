...
m2 = m %>% group_by(status) %>% summarise( median=median(value), n=length(status), value=max(value) )
...
ggplot(df, aes(status, n, group=status)) + geom_jitter(aes(color=status), width=0.2) +
  ...
  geom_label(data=(df %>% group_by(status) %>% summarise(y=max(n), med=median(n))), 
             aes(status, y, label=paste('median\n',sprintf("%.1f", med, sep='')))) +
  labs(y='Number of segments', title="Number of segments per patient", subtitle=paste('t-test p-value', signif(nWT$p.value, digits=3)) ) +
  theme(legend.position = 'none')


nohgd.var.seg.lengths = lapply(patient.data, function(pt) {
  samples = subset(pt$info, Pathology %in% c('BE','ID','LGD'))$Samplename
  cols = c('chrom','start.pos','end.pos', grep(paste(samples, collapse='|'), colnames(pt$variable.regions), value=T))
  vr = pt$variable.regions[,cols]
