...
roc.plot(rocBE) + labs(title='SNP ROC, BE biopsies')

rocBE$model = 'per Biopsy'

ggsave(filename = 'plots/snp/be_roc.png', plot = roc.plot(rocBE, 'SNP ROC: BE biopsies') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)
```
...
# Timepoint 2 only
roct2 = pROC::roc(Status ~ Prob, data=subset(pred.qc, Timepoint != 'T1' & !lowsca & !wgd.NP), auc=T, ci=T, of='thresholds')
