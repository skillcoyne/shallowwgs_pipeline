...
# pred.qc = pred.qc %>% group_by(Status, Samplename) %>% dplyr::mutate(
#   lowsca = ( `mean(SCA.Ratio)` < 0.01 & `mean(Purity)` >= 0.95)
# )
...
ggplot(pred.qc, aes(Prob)) + geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1) , show.legend = F) +
  scale_fill_gradientn(colors = myPal,  name='') + 
  labs(title='Predictions, all samples model', y='n Samples', x='Unadjusted Probability')
...
ggplot(subset(pred.qc, !exclude & !wgd & !lowsca), aes(`mean(Purity)`,Prob, color=Risk, shape=Status)) + 
  facet_grid(~Status) + geom_point()
