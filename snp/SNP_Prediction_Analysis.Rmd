...
qcdata$ASCAT.SCA.ratio = apply(qcdata,1,function(s) {
  smp = filter( segments.list[[ s$PatientID ]], sample == s$Samplename & chr %in% c(1:22) )
  smp = smp %>% #rowwise %>% 
    dplyr::mutate(
      #'Total' = nAraw + nBraw,
      'Total' = nMajor + nMinor,
  ...
  sum(x$endpos - x$startpos)/as.numeric(chr.info[22,'genome.length'])
})
...
patient.info = readxl::read_xlsx('~/Data/BarrettsProgressionRisk/Analysis/SNP/metadata_T1T2.xlsx') %>% 
  mutate( UniqueSampleID = paste(PatientID, `Timepoint Code`, sep='_'), Path.Status = Status) %>% 
...
sample.list = sample.list %>% mutate(SCA.Ratio = Total.SCA/max(chr.info$genome.length))
...
preds = preds %>% mutate(Status = as.character(ifelse(Pathology %in% c('HGD','IMD'), 'P', Status)))

preds %>% filter(grepl('BLD|gastric',Level,ignore.case=T)) %>% group_by(Risk, Level) %>% tally %>% spread(Risk,n)

preds %>% filter(!grepl('BLD|gastric',Level,ignore.case=T)) %>% group_by(Status, Risk, Pathology) %>% tally %>% spread(Risk,n)
...
preds.normal = preds %>% filter(grepl('BLD|gastric', Level, ignore.case=T)) 

preds.normal %>% group_by(Risk) %>% tally
...
preds = preds %>% 
  mutate(Risk = ifelse(Prob > 0.5, 'High', 'Low')) %>% 
  mutate(Risk = factor(Risk, levels=c('Low','High'), ordered=T))



preds %>% group_by(Status, Risk) %>% tally %>% spread(Risk, n) %>%
  left_join(preds %>% group_by(Status) %>% tally, by='Status') %>%
  mutate_if(is.numeric, funs( round(./n,2))) %>% 
  dplyr::select(-n)
...
rocR = pROC::roc(Status ~ Risk, data=preds, auc=T, ci=T, of='thresholds',transpose=T)
