...
ggplot(filter(qcdata,SampleType == 'BE'), aes(Timepoint, ASCAT.SCA.ratio, fill=Timepoint, group=Timepoint)) + geom_jitter(width=0.2) +
  geom_boxplot(alpha=0.5) + facet_wrap(~Status, scales='free_x', ncol=2) + labs(title='SCA Ratio by Timepoint') + theme(legend.position = 'none')
...
ggplot(subset(qcdata, SampleType == 'BE'), aes(Ploidy, ASCAT.SCA.ratio, color=Status)) + facet_grid(~Status) + geom_point() + 
  geom_hline(yintercept = x, color='red') + labs(title='Barretts only')
...
```{r echo=F}
preds = preds %>% mutate(Pathology = factor(Pathology, levels=c('NDBE','ID','LGD','HGD','IMC'), ordered = T))
rp = plot.risk.by.path(preds) 
rp
...
roc.plot(rocBE) + labs(title='SNP ROC', subtitle='BE samples only')
pROC::ci.auc(rocBE$auc)
pROC::coords(rocBE, 'best', transpose=F)
ggsave(filename = 'plots/snp/be_roc.png', plot = roc.plot(rocBE) + labs(title='SNP ROC', subtitle='BE samples only') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)
```
...
p1 = ggplot(prog, aes(Prob)) + facet_wrap(~Timepoint, scales='free_y',nrow=1) + 
  ...
  scale_fill_gradientn(colors = myPal,  name='') + 
  labs(title='Predictions on progressors', y='n Samples', x='Unadjusted Probability') + theme(text=element_text(size=12))

nonprog = subset(pred.qc, Status == 'NP')
p2 = ggplot(nonprog, aes(Prob)) + facet_wrap(~Timepoint, scales='free_y', nrow=1) + 
  geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1) , show.legend = F) +
  scale_fill_gradientn(colors = myPal,  name='') + 
  labs(title='Predictions on non-progressors', y='n Samples', x='Unadjusted Probability') + theme(text=element_text(size=12))
...
p3 = ggplot(normals, aes(Prob)) + facet_wrap(~Type, ncol=2, scales = 'free') +
  geom_histogram(aes(fill=..x..), breaks=seq(0,1,0.1) , show.legend = F) +
  scale_fill_gradientn(colors = myPal,  name='') +
  labs(title='Predictions on Blood/Gastric normals', y='n Samples', x='Unadjusted Probability') + theme(text=element_text(size=12))
...
roc.plot(rocEX) + labs(title='BE samples', subtitle='Path status (HGD/IMC)')
pROC::ci.auc(rocEX)
ggsave(filename='plots/snp/path_status_roc.png', plot = roc.plot(rocEX) + labs(title='BE samples', subtitle='Path status (HGD/IMC)') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)
...
roc.plot(rocpp) + labs(title='BE samples', subtitle='excluding low SCA samples')
ggsave(filename='plots/snp/auc_ex_lowsca_roc.png', plot = roc.plot(rocpp) + labs(title='BE samples', subtitle='excluding low SCA samples') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)
...
roc.plot(rocexall) + labs(title='BE samples', subtitle='excluding low SCA & WGD NP')
ggsave(filename='plots/snp/auc_ex_lowsca_wgs_roc.png', plot =roc.plot(rocexall) + labs(title='BE samples', subtitle='excluding low SCA & WGD NP') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)

# Timepoint 1 only
roct1 = pROC::roc(Status ~ Prob, data=filter(pred.qc, Timepoint == 'T1' & !lowsca & !wgd.NP), auc=T, ci=T, of='thresholds', transpose=T)
roc.plot(roct1) + labs(title='BE samples', subtitle='Timepoint T1')
ggsave(filename='plots/snp/auc_t1_roc.png', plot=roc.plot(roct1) + labs(title='BE samples', subtitle='Timepoint T1') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)

# Timepoint 2 only
roct2 = pROC::roc(Status ~ Prob, data=subset(pred.qc, Timepoint != 'T1' & !lowsca & !wgd.NP), auc=T, ci=T, of='thresholds', transpose=T)
roc.plot(roct2) + labs(title='BE samples', subtitle='Timepoint T2')
ggsave(filename='plots/snp/auc_t2_roc.png', plot=roc.plot(roct2) + labs(title='BE samples', subtitle='Timepoint T2') + theme(text=element_text(size=12)), width=4, height=4, units='in', dpi=300)
